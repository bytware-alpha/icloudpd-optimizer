...
check:
    ...
    CARGO_TARGET_DIR=target cargo test --locked --manifest-path vendor/smb2-0.13.0/Cargo.toml --lib session_setup_retains_exact_guest_and_null_flags
    CARGO_TARGET_DIR=target cargo test --locked --manifest-path vendor/smb2-0.13.0/Cargo.toml --lib session_setup_binds_exact_connection_diagnostics
    cargo test --locked
...
macos-app:
    ...
    keychain="${ICLOUDPD_OPTIMIZER_KEYCHAIN:-}"
    config="${ICLOUDPD_OPTIMIZER_MONITOR_CONFIG:-}"
    authority="${ICLOUDPD_OPTIMIZER_AUTHORITY:-policies/authorization-policy-disabled.json}"
    if [ "$(jq -r '.mode // empty' "$authority")" = "production" ] && [ -z "$keychain" ]; then
      printf '%s\n' 'production authority requires ICLOUDPD_OPTIMIZER_KEYCHAIN' >&2
      exit 2
    fi
    set -- --bin target/release/icloudpd-optimizer --output "$output" --authority "$authority"
    if [ -n "$identity" ]; then
      set -- "$@" --sign "$identity"
    fi
    if [ -n "$keychain" ]; then
...
macos-app-service-install config:
    #!/usr/bin/env sh
    set -eu
    app="$HOME/Library/Application Support/iCloudPD Optimizer/Service/iCloudPD Optimizer Service.app"
...
macos-app-verify:
    ...
    service_app="$HOME/Library/Application Support/iCloudPD Optimizer/Service/iCloudPD Optimizer Service.app"
