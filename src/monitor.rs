...
use crate::legacy_upload_migration::{
    LegacyUploadMigrationError, classify_legacy_upload_migration_records,
};
...
pub struct MonitorConfig {
    ...
    #[serde(default)]
    pub smb_url_authority_file: Option<PathBuf>,
    #[serde(default)]
    pub smb_url_authority_sha256: Option<String>,
    #[serde(default)]
    pub smb_expected_mount_root: Option<PathBuf>,
}
...
impl MonitorConfig {
    pub fn new(
        ...
        Self {
            ...
            rolling_original_resolve_batch_multiplier:
                default_rolling_original_resolve_batch_multiplier(),
            smb_url_authority_file: None,
            smb_url_authority_sha256: None,
            smb_expected_mount_root: None,
        }
    }
    ...
    pub fn validate(&self) -> Result<(), MonitorError> {
        ...
        if !matches!(
            (
                &self.smb_url_authority_file,
                &self.smb_url_authority_sha256,
                &self.smb_expected_mount_root,
            ),
            (None, None, None) | (Some(_), Some(_), Some(_))
        ) {
            return Err(MonitorError::InvalidConfig {
                message: "SMB authority bootstrap requires file, SHA-256, and expected mount root together".to_string(),
            });
        }
        if let (Some(expected_sha256), Some(expected_mount_root)) = (
            &self.smb_url_authority_sha256,
            &self.smb_expected_mount_root,
        ) {
            let canonical_mount_root = expected_mount_root.components().collect::<Vec<_>>();
            let exact_volumes_child = matches!(
                canonical_mount_root.as_slice(),
                [Component::RootDir, Component::Normal(volumes), Component::Normal(_)]
                    if *volumes == OsStr::new("Volumes")
            );
            ...
            {
                return Err(MonitorError::InvalidConfig {
                    message: "SMB authority bootstrap SHA-256 or expected mount root is invalid"
                        .to_string(),
                });
            }
        }
...
pub fn run_monitor_once(
    ...
    config.validate()?;
    let preclassified_state =
        if AssetStateStore::db_path_for_manifest(&config.manifest_path).exists() {
            let state_store = Arc::new(guard.state_store(&config.manifest_path)?.clone());
            let authoritative_manifest = state_store.load_or_import()?;
            let (manifest, sealed_migration_asset_ids) =
                classify_legacy_upload_migration_records(&authoritative_manifest)?.into_parts();
            Some((state_store, manifest, sealed_migration_asset_ids))
        } else {
            None
        };
    ...
    let (state_store, mut manifest, sealed_migration_asset_ids) = match preclassified_state {
        Some(preclassified) => preclassified,
        None => {
            let state_store = Arc::new(guard.state_store(&config.manifest_path)?.clone());
            let authoritative_manifest = state_store.load_or_import()?;
            let (manifest, sealed_migration_asset_ids) =
                classify_legacy_upload_migration_records(&authoritative_manifest)?.into_parts();
            (state_store, manifest, sealed_migration_asset_ids)
        }
    };
    ...
    if new_work_skip_reason.is_none() {
        ...
        if pending_capacity > 0 {
            ...
            visit_raw_paths(&download_root, config.scan_recursive, &mut |raw_path| {
                ...
                if sealed_migration_asset_ids.contains(&asset_id) {
...
impl MonitorLeaseHeartbeat {
    fn start(state_store: AssetStateStore, lease_ttl: Duration) -> Self {
        let (stop_tx, stop_rx) = mpsc::channel();
        let interval = Duration::from_millis(
            u64::try_from((lease_ttl.as_millis() / 3).max(1))
...
pub fn acquire_monitor_run_guard(config: &MonitorConfig) -> Result<MonitorRunGuard, MonitorError> {
    acquire_monitor_run_guard_with_lease_ttl(config, MONITOR_STATE_LEASE_TTL)
}

fn acquire_monitor_run_guard_with_lease_ttl(
    config: &MonitorConfig,
    lease_ttl: Duration,
) -> Result<MonitorRunGuard, MonitorError> {
    ...
    pub(crate) fn state_store(
        ...
        match self.state_store.as_ref() {
            ...
            None => {
                ...
                self.heartbeat = Some(MonitorLeaseHeartbeat::start(
                    state_store.clone(),
                    self.lease_ttl,
                ));
...
pub fn launchd_plist(
    ...
    service_hard_stop_path: Option<&Path>,
    ...
    let service_hard_stop_argument = service_hard_stop_path.map_or_else(String::new, |path| {
        format!(
            "    <string>--service-hard-stop</string>\n    <string>{}</string>\n",
    ...
    Ok(format!(
        ...
        service_hard_stop_argument = service_hard_stop_argument,
...
pub fn write_service_launchd_plist(
    label: &str,
    binary: &Path,
    config: &Path,
    stdout_path: &Path,
    ...
    let hard_stop_path = output.with_extension("hard-stop");
    let plist = launchd_plist(
        label,
        binary,
        config,
...
fn run_rolling_lifecycle_passes(
    ...
    let mut admitted_delete_asset_ids = BTreeSet::new();
...
fn run_rolling_lifecycle_pass(
    ...
    admitted_delete_asset_ids: &mut BTreeSet<String>,
) -> Result<(), MonitorError> {
...
fn run_rolling_lifecycle_delete_batch(
    ...
    let admitted = admit_rolling_lifecycle_delete_asset_ids(
        manifest,
        config,
        active_lifecycle_asset_ids,
        admitted_delete_asset_ids,
    );
    delete_original_assets(config, state_store, manifest, summary, &admitted)
}

fn admit_rolling_lifecycle_delete_asset_ids(
    manifest: &Manifest,
    config: &MonitorConfig,
    active_lifecycle_asset_ids: &[String],
    admitted_delete_asset_ids: &mut BTreeSet<String>,
) -> Vec<String> {
    let remaining_delete_capacity = config
        .max_lifecycle_per_scan
        .saturating_sub(admitted_delete_asset_ids.len());
    if remaining_delete_capacity == 0 {
        return Vec::new();
    }
    let admitted = delete_lifecycle_asset_ids(manifest, config, active_lifecycle_asset_ids)
        .into_iter()
        .filter(|asset_id| !admitted_delete_asset_ids.contains(asset_id))
        .take(remaining_delete_capacity)
        .collect::<Vec<_>>();
    admitted_delete_asset_ids.extend(admitted.iter().cloned());
    admitted
}
...
fn validate_authoritative_adjusted_source_conflict_record(
    ...
    let destination = CloudKitLibraryDestination {
        ...
        owner_record_name: durable_original.owner_record_name,
    };
    ...
    F: FnOnce(&Path, &str, &AssetRecord, &Path, u64) -> Result<Vec<u8>, MonitorError>,
{
    ...
    let immutable_state_snapshot = state_store.create_immutable_read_snapshot()?;
    let child_result = child_runner(
        immutable_state_snapshot.manifest_path(),
        asset_id,
        &record,
        session_path,
        config.upload_timeout_seconds,
    );
    drop(immutable_state_snapshot);

    match child_result.and_then(|response| {
...
fn reconcile_delete_window<T: CloudKitDeleteTransport>(
    ...
    for (destination, items) in grouped {
        ...
        destination_session.zone = destination;
        let lookup_started = Instant::now();
        let lookup = match client.lookup_delete_states(&destination_session, &batch_request) {
            Ok(lookup) => lookup,
            Err(error) => {
    ...
    F: FnMut() -> SystemTime,
{
    ...
    let mut destination_session = session.clone();
    destination_session.database_scope = database_scope;
    destination_session.zone = CloudKitLibraryDestination {
        database_scope,
        zone_name: zone_name.clone(),
        owner_record_name: first_request.owner_record_name.clone(),
    };
    ...
    let outcomes = match client.delete_originals_batch_with_preflight(
        ...
        Err(CloudKitDeleteBatchSendError::Remote(modify_error)) => {
            ...
            let lookup_started = Instant::now();
            match client.lookup_delete_states(&destination_session, &batch_request) {
                Ok(reconciliation) => {
...
pub struct AdjustedSourceRequiredProof {
    ...
    original_owner_record_name: Option<String>,
...
fn validate_adjusted_source_required_queue_common(
    ...
    if marker.original_record_name != original.record_name
        ...
        || marker.original_owner_record_name != original.owner_record_name
...
fn validate_adjusted_source_required_common(
    ...
    if marker.nas_proof_digest != source.nas_proof_digest
        ...
        || marker.original_owner_record_name != source.original.owner_record_name
...
fn build_adjusted_source_required_proof(
    ...
    Ok(AdjustedSourceRequiredProof {
        ...
        original_owner_record_name: source.original.owner_record_name,
...
pub(crate) fn verify_converted_heic(
    manifest: &Manifest,
    asset_id: &str,
    timeout_seconds: u64,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct ReferenceNormalizationIdentity {
    pub(crate) orientation: u16,
    pub(crate) width: u32,
    pub(crate) height: u32,
    pub(crate) decoded_pixel_sha256: String,
}

pub(crate) fn reference_normalization_identity(
    path: &Path,
    timeout_seconds: u64,
) -> Result<ReferenceNormalizationIdentity, MonitorError> {
    let metadata = read_media_metadata(
        path,
        timeout_seconds,
        HeicMetadataFailure::ReferenceOrientationInvalid,
    )?;
    let [orientation] = metadata.orientations.as_slice() else {
        return Err(metadata_verification_error(
            HeicMetadataFailure::ReferenceOrientationInvalid,
        ));
    };
    let orientation = u16::try_from(*orientation).map_err(|_| {
    ...
    let decoded_pixel_sha256 = format!("{:x}", Sha256::digest(rgb.into_raw()));
    if metadata.dimensions != (u64::from(width), u64::from(height)) {
        return Err(metadata_verification_error(
            HeicMetadataFailure::DimensionMismatch,
        ));
    }
    Ok(ReferenceNormalizationIdentity {
        orientation,
        width,
        height,
...
/// Mutates only a caller-owned private temporary file. Callers must validate
/// and atomically install that file; public reference paths are never passed here.
pub(crate) fn normalize_private_reference_orientation_temp(
    path: &Path,
    timeout_seconds: u64,
) -> Result<(), MonitorError> {
    ...
    command
        ...
        .arg(path);
    let output = run_external_command_with_timeout("exiftool", command, timeout_seconds)?;
    if output.status.success() {
        Ok(())
    } else {
...
#[derive(Clone, Debug)]
pub(crate) struct VerifiedHeic {
    pub(crate) proof: HeicVerificationInput,
...
#[cfg(target_os = "macos")]
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
struct VisualVerificationWorkspaceIdentity {
    device: u64,
    inode: u64,
    owner: u32,
    mode: u32,
...
#[cfg(target_os = "macos")]
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
struct VisualVerificationOutputIdentity {
    device: u64,
    inode: u64,
    owner: u32,
    mode: u32,
...
#[cfg(target_os = "macos")]
struct VisualVerificationOutput {
    file: File,
    name: CString,
    identity: VisualVerificationOutputIdentity,
}

#[cfg(target_os = "macos")]
struct VisualVerificationOutputPlan {
    path: PathBuf,
    name: CString,
}

#[cfg(target_os = "macos")]
struct VisualVerificationWorkspace {
    parent_path: PathBuf,
    parent: File,
    parent_identity: VisualVerificationWorkspaceIdentity,
    name: CString,
    path: PathBuf,
    directory: File,
    identity: VisualVerificationWorkspaceIdentity,
    planned_outputs: Vec<VisualVerificationOutputPlan>,
    active: bool,
}

#[cfg(target_os = "macos")]
impl VisualVerificationWorkspace {
    fn create() -> Result<Self, MonitorError> {
        use std::os::fd::AsRawFd;
        use std::os::unix::ffi::OsStrExt;
        use std::os::unix::fs::OpenOptionsExt;

        let parent_path = Self::trusted_parent_path()?
            .canonicalize()
            .map_err(|_| MonitorError::VisualVerificationWorkspace)?;
        if let Some(configured_tmpdir) = env::var_os("TMPDIR") {
            let configured_tmpdir = PathBuf::from(configured_tmpdir)
                .canonicalize()
                .map_err(|_| MonitorError::VisualVerificationWorkspace)?;
            if configured_tmpdir != parent_path {
                return Err(MonitorError::VisualVerificationWorkspace);
            }
        }
        let parent = fs::OpenOptions::new()
            ...
            .open(&parent_path)
            .map_err(|_| MonitorError::VisualVerificationWorkspace)?;
        let parent_identity = Self::validated_directory_identity(
            &parent
                .metadata()
                .map_err(|_| MonitorError::VisualVerificationWorkspace)?,
        )?;
        if parent_identity.owner != unsafe { libc::geteuid() } || parent_identity.mode != 0o700 {
            return Err(MonitorError::VisualVerificationWorkspace);
        }
        for _ in 0..16 {
            let name = CString::new(format!(
                ".icloudpd-visual-verification-{}.workspace",
                Uuid::new_v4()
            ))
            .map_err(|_| MonitorError::VisualVerificationWorkspace)?;
            ...
            let directory = match Self::open_directory_at(&parent, &name) {
            ...
            let identity = match Self::validated_directory_identity(&metadata) {
        ...
        Err(MonitorError::VisualVerificationWorkspace)
    }

    fn trusted_parent_path() -> Result<PathBuf, MonitorError> {
        use std::os::unix::ffi::OsStrExt;
    ...
    fn plan_output(&mut self, path: PathBuf) -> Result<PathBuf, MonitorError> {
        use std::os::unix::ffi::OsStrExt;
        ...
        let name = CString::new(
            path.file_name()
                .ok_or(MonitorError::VisualVerificationWorkspace)?
                .as_bytes(),
        )
        .map_err(|_| MonitorError::VisualVerificationWorkspace)?;
        if !self.child_is_missing(&name)? {
            return Err(MonitorError::VisualVerificationWorkspace);
        }
        self.planned_outputs.push(VisualVerificationOutputPlan {
            path: path.clone(),
            name,
        });
        Ok(path)
    ...
    fn validate(&self) -> Result<(), MonitorError> {
        ...
        let named_parent = fs::symlink_metadata(&self.parent_path)
            .map_err(|_| MonitorError::VisualVerificationWorkspace)?;
        if Self::validated_directory_identity(&held_parent)? != self.parent_identity
            || Self::validated_directory_identity(&named_parent)? != self.parent_identity
        {
            return Err(MonitorError::VisualVerificationWorkspace);
        }
        let held = self
            ...
            .metadata()
            .map_err(|_| MonitorError::VisualVerificationWorkspace)?;
        let named = Self::open_directory_at(&self.parent, &self.name)?;
        if Self::validated_directory_identity(&held)? != self.identity
            || Self::validated_directory_identity(
                &named
                    .metadata()
                    .map_err(|_| MonitorError::VisualVerificationWorkspace)?,
            )? != self.identity
    ...
    fn open_output(&self, path: &Path) -> Result<VisualVerificationOutput, MonitorError> {
        self.validate()?;
        let plan = self
            .planned_outputs
            .iter()
            .find(|planned| planned.path == path)
            .ok_or(MonitorError::VisualVerificationWorkspace)?;
        let file = Self::open_regular_at(&self.directory, &plan.name)?;
        let identity = Self::validated_output_identity(
            &file
                .metadata()
                .map_err(|_| MonitorError::VisualVerificationWorkspace)?,
        )?;
        Ok(VisualVerificationOutput {
            file,
    ...
    fn revalidate_output(&self, output: &VisualVerificationOutput) -> Result<(), MonitorError> {
        self.validate()?;
        let held = Self::validated_output_identity(
            &output
                .file
                .metadata()
                .map_err(|_| MonitorError::VisualVerificationWorkspace)?,
        )?;
        let named = Self::open_regular_at(&self.directory, &output.name)?;
        let named_identity = Self::validated_output_identity(
            &named
                .metadata()
                .map_err(|_| MonitorError::VisualVerificationWorkspace)?,
        )?;
        if held != output.identity || named_identity != output.identity {
            return Err(MonitorError::VisualVerificationWorkspace);
        }
        Ok(())
    }

    fn cleanup(&mut self) -> Result<(), MonitorError> {
        use std::os::fd::AsRawFd;
        ...
        self.validate()?;
        for plan in &self.planned_outputs {
            let output = match Self::open_regular_at(&self.directory, &plan.name) {
                Ok(file) => file,
                Err(_) if self.child_is_missing(&plan.name)? => continue,
                Err(_) => return Err(MonitorError::VisualVerificationWorkspace),
            };
            Self::validated_output_identity(
                &output
                    .metadata()
                    .map_err(|_| MonitorError::VisualVerificationWorkspace)?,
            )?;
            if unsafe { libc::unlinkat(self.directory.as_raw_fd(), plan.name.as_ptr(), 0) } != 0 {
                return Err(MonitorError::VisualVerificationWorkspace);
            }
        }
        self.validate()?;
    ...
    fn open_directory_at(parent: &File, name: &CString) -> Result<File, MonitorError> {
    ...
    fn open_regular_at(parent: &File, name: &CString) -> Result<File, MonitorError> {
        use std::os::fd::{AsRawFd, FromRawFd};
    ...
    fn child_is_missing(&self, name: &CString) -> Result<bool, MonitorError> {
    ...
    fn validated_directory_identity(
        metadata: &fs::Metadata,
    ) -> Result<VisualVerificationWorkspaceIdentity, MonitorError> {
        use std::os::unix::fs::MetadataExt;

        let mode = metadata.mode() & 0o7777;
        if !metadata.is_dir() || metadata.file_type().is_symlink() || metadata.nlink() < 2 {
            return Err(MonitorError::VisualVerificationWorkspace);
        }
        Ok(VisualVerificationWorkspaceIdentity {
            device: metadata.dev(),
            inode: metadata.ino(),
            owner: metadata.uid(),
            mode,
    ...
    fn validated_output_identity(
        metadata: &fs::Metadata,
    ) -> Result<VisualVerificationOutputIdentity, MonitorError> {
        use std::os::unix::fs::MetadataExt;
        ...
        Ok(VisualVerificationOutputIdentity {
            device: metadata.dev(),
            inode: metadata.ino(),
            owner: metadata.uid(),
...
#[cfg(target_os = "macos")]
impl Drop for VisualVerificationWorkspace {
    fn drop(&mut self) {
        let _ = self.cleanup();
    }
}
...
fn visual_metrics_for_conversion_with_sips(
    ...
    #[cfg(target_os = "macos")]
    let mut workspace = VisualVerificationWorkspace::create()?;
    #[cfg(target_os = "macos")]
    let workspace_candidate = workspace.path().join("candidate.heic");
    #[cfg(target_os = "macos")]
    let candidate_preview_path =
        workspace.plan_output(verification_preview_path(&workspace_candidate, "heic"))?;
    #[cfg(not(target_os = "macos"))]
    let candidate_preview_path = verification_preview_path(candidate, "heic");
    #[cfg(target_os = "macos")]
    let reference_preview_path =
        workspace.plan_output(verification_preview_path(&workspace_candidate, "raw"))?;
    #[cfg(not(target_os = "macos"))]
    let reference_preview_path = verification_preview_path(candidate, "raw");
    #[cfg(target_os = "macos")]
    let codec_normalized_reference_path =
        workspace.plan_output(codec_normalized_reference_path(&workspace_candidate))?;
    #[cfg(not(target_os = "macos"))]
    let codec_normalized_reference_path = codec_normalized_reference_path(candidate);
    #[cfg(target_os = "macos")]
    let codec_normalized_candidate_preview_path = workspace.plan_output(
        verification_preview_path(&workspace_candidate, "codec-normalized-candidate"),
    )?;
    #[cfg(not(target_os = "macos"))]
    let codec_normalized_candidate_preview_path =
        verification_preview_path(candidate, "codec-normalized-candidate");
    #[cfg(target_os = "macos")]
    let codec_normalized_reference_preview_path = workspace.plan_output(
        verification_preview_path(&codec_normalized_reference_path, "heic"),
    )?;
    #[cfg(not(target_os = "macos"))]
    let codec_normalized_reference_preview_path =
        verification_preview_path(&codec_normalized_reference_path, "heic");
    let reference_exists = reference.exists();
    let result = (|| {
        #[cfg(target_os = "macos")]
        workspace.validate()?;
        if reference_exists {
        ...
        #[cfg(target_os = "macos")]
        let candidate_preview_output = workspace.open_output(&candidate_preview_path)?;
        #[cfg(target_os = "macos")]
        let candidate_preview =
            read_rgb_preview_file(&candidate_preview_path, &candidate_preview_output.file)?;
        #[cfg(not(target_os = "macos"))]
        ...
        let Some(direct_reference_error) = reference_exists
            .then(|| {
                #[cfg(target_os = "macos")]
                let reference_preview_output = workspace.open_output(&reference_preview_path)?;
                #[cfg(target_os = "macos")]
                let reference_preview =
                    read_rgb_preview_file(&reference_preview_path, &reference_preview_output.file)?;
                #[cfg(not(target_os = "macos"))]
        ...
        #[cfg(target_os = "macos")]
        let codec_normalized_reference_output =
            workspace.open_output(&codec_normalized_reference_path)?;
        render_visual_preview_pair_with_sips(
            sips_program,
            candidate,
        ...
        #[cfg(target_os = "macos")]
        workspace.revalidate_output(&codec_normalized_reference_output)?;
        #[cfg(target_os = "macos")]
        let codec_normalized_candidate_preview_output =
            workspace.open_output(&codec_normalized_candidate_preview_path)?;
        #[cfg(target_os = "macos")]
        let codec_normalized_candidate_preview = read_rgb_preview_file(
            &codec_normalized_candidate_preview_path,
            &codec_normalized_candidate_preview_output.file,
        )?;
        #[cfg(not(target_os = "macos"))]
        let codec_normalized_candidate_preview =
            read_rgb_preview(&codec_normalized_candidate_preview_path)?;
        #[cfg(target_os = "macos")]
        let codec_normalized_reference_preview_output =
            workspace.open_output(&codec_normalized_reference_preview_path)?;
        #[cfg(target_os = "macos")]
        let codec_normalized_reference_preview = read_rgb_preview_file(
            &codec_normalized_reference_preview_path,
            &codec_normalized_reference_preview_output.file,
        )?;
        #[cfg(not(target_os = "macos"))]
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        match (result, cleanup) {
            ...
            (Err(_), Err(_)) => Err(MonitorError::VisualVerificationAndCleanupFailed),
...
#[cfg(target_os = "macos")]
fn read_rgb_preview_file(path: &Path, file: &File) -> Result<RgbPreview, MonitorError> {
    let file = file
        .try_clone()
        .map_err(|source| MonitorError::PreviewDecode {
            path: path.to_path_buf(),
            source: image::ImageError::IoError(source),
        })?;
    let image =
        image::load(io::BufReader::new(file), image::ImageFormat::Png).map_err(|source| {
            MonitorError::PreviewDecode {
                path: path.to_path_buf(),
                source,
            }
        })?;
...
pub enum MonitorError {
    ...
    #[error("legacy upload migration classification error: {0}")]
    LegacyUploadMigration(#[from] LegacyUploadMigrationError),
    ...
    #[error("visual verification workspace validation failed")]
    VisualVerificationWorkspace,
    #[error("visual verification failed and workspace cleanup also failed")]
    VisualVerificationAndCleanupFailed,
...
mod tests {
    ...
    #[test]
    fn service_launchd_plist_renders_stable_associated_bundle_identifier() {
        let temporary = tempfile::tempdir().expect("temporary service plist directory");
        let plist_path = temporary
            .path()
            .join("com.icloudpd-optimizer.monitor.plist");
        let hard_stop_path = plist_path.with_extension("hard-stop");
        let plist = launchd_plist(
            "com.icloudpd-optimizer.monitor",
            &temporary.path().join("ICloudPDOptimizerApp"),
            &temporary.path().join("monitor.json"),
            &temporary.path().join("monitor.stdout.log"),
            &temporary.path().join("monitor.stderr.log"),
            Some("com.icloudpd-optimizer.monitor"),
            Some(&hard_stop_path),
        )
        .expect("stable service plist should render");

        assert!(plist.contains("<key>AssociatedBundleIdentifiers</key>"));
        assert!(plist.contains("<string>com.icloudpd-optimizer.monitor</string>"));
        assert!(!plist.contains("io.github.bytware-alpha.icloudpd-optimizer"));
        assert!(plist.contains("<key>Disabled</key>\n  <true/>"));
        assert!(plist.contains("--service-hard-stop"));
        assert!(plist.contains(&hard_stop_path.to_string_lossy().to_string()));
    }
...
"#,
    ...
    #[cfg(unix)]
    const FAKE_NATIVE_VISUAL_VERIFIER_TIMEOUT_SECONDS: u64 = 15;
...
"#,
    ...
    #[cfg(target_os = "macos")]
    struct FakeSealedReverifyVisualVerifier {
        _tempdir: tempfile::TempDir,
        program: PathBuf,
        output_log: PathBuf,
        write_error_log: PathBuf,
    }

    #[cfg(target_os = "macos")]
    impl FakeSealedReverifyVisualVerifier {
        fn new() -> Self {
            use std::os::unix::fs::PermissionsExt;

            let tempdir = tempfile::tempdir().expect("tempdir should be created");
            let program = tempdir.path().join("sips");
            let preview = tempdir.path().join("preview.png");
            let output_log = tempdir.path().join("outputs.log");
            let write_error_log = tempdir.path().join("write-errors.log");
            write_rgb_preview_png(
                &preview,
                &RgbPreview {
                    width: 2,
...
[ -n "$out" ] || exit 64
printf '%s\n' "$out" >> '{}'
if ! /bin/cp '{}' "$out" 2>> '{}'; then
  exit 73
fi
"#,
                    output_log.display(),
                    preview.display(),
                    write_error_log.display(),
                ),
            )
            .expect("fake sips script should be written");
            let mut permissions = fs::metadata(&program)
        ...
        fn output_paths(&self) -> Vec<PathBuf> {
            fs::read_to_string(&self.output_log)
                .unwrap_or_default()
                .lines()
                .map(PathBuf::from)
                .collect()
        ...
        fn write_errors(&self) -> String {
            fs::read_to_string(&self.write_error_log).unwrap_or_default()
        }
    }

    #[cfg(target_os = "macos")]
    struct FakeVisualWorkspaceOutcomeVerifier {
        _tempdir: tempfile::TempDir,
        program: PathBuf,
        reference: PathBuf,
        candidate: PathBuf,
        workspace_log: PathBuf,
    }

    #[cfg(target_os = "macos")]
    impl FakeVisualWorkspaceOutcomeVerifier {
        const CLEANUP_BLOCKER: &'static str = ".injected-cleanup-blocker";

        fn new(fail_verification: bool, fail_cleanup: bool) -> Self {
            use std::os::unix::fs::PermissionsExt;

            let tempdir = tempfile::tempdir().expect("tempdir should be created");
            let program = tempdir.path().join("sips");
            let reference = tempdir.path().join("reference.jpg");
            let candidate = tempdir.path().join("candidate.heic");
            let preview = tempdir.path().join("preview.png");
            let workspace_log = tempdir.path().join("workspaces.log");
            fs::write(&reference, b"reference").expect("reference should be written");
...
workspace="${{out%/*}}"
printf '%s\n' "$workspace" >> '{}'
[ '{}' = "1" ] && printf 'block cleanup' > "$workspace/{}"
[ '{}' = "1" ] && exit 73
/bin/cp '{}' "$out"
"#,
                    workspace_log.display(),
                    if fail_cleanup { "1" } else { "0" },
                    Self::CLEANUP_BLOCKER,
                    if fail_verification { "1" } else { "0" },
                    preview.display(),
                ),
            )
            .expect("fake sips should be written");
            let mut permissions = fs::metadata(&program)
                .expect("fake sips metadata should be readable")
                .permissions();
            permissions.set_mode(0o755);
            fs::set_permissions(&program, permissions).expect("fake sips should be executable");
            Self {
                _tempdir: tempdir,
                program,
                reference,
        ...
        fn run(&self) -> Result<VisualMetrics, MonitorError> {
            let result = visual_metrics_for_conversion_with_sips(
                &self.reference,
                &self.candidate,
                FAKE_NATIVE_VISUAL_VERIFIER_TIMEOUT_SECONDS,
                &self.program,
            );
            self.remove_injected_cleanup_artifacts();
            result
        }

        fn observed_workspaces(&self) -> String {
            fs::read_to_string(&self.workspace_log).unwrap_or_default()
        }

        fn remove_injected_cleanup_artifacts(&self) {
            let workspaces = fs::read_to_string(&self.workspace_log).unwrap_or_default();
            let unique = workspaces
                .lines()
                .map(PathBuf::from)
                .collect::<BTreeSet<_>>();
            for workspace in unique {
                let blocker = workspace.join(Self::CLEANUP_BLOCKER);
                if blocker.exists() {
                    fs::remove_file(&blocker).expect("injected cleanup blocker should be removed");
                }
                if workspace.exists() {
                    fs::remove_dir(&workspace)
                        .expect("injected cleanup workspace should be removed");
                }
            }
        }
    }
    ...
    #[test]
    fn rolling_upload_snapshots_isolate_64_concurrent_immutable_children() {
        const WORKERS: usize = 64;
        let tempdir = tempfile::tempdir().expect("temporary directory should exist");
        let download_root = tempdir.path().join("download");
        fs::create_dir_all(&download_root).expect("download root should exist");
        let mut config = MonitorConfig::new(
            download_root.clone(),
            tempdir.path().join("manifest.json"),
            tempdir.path().join("heic"),
        ...
        config.upload_session_path = Some(tempdir.path().join("upload-session.json"));

        let mut initial = Manifest::new();
        for index in 0..WORKERS {
            let asset_id = format!("asset-{index:02}");
            let mut record = upload_verified_delete_ready_record(&asset_id, &download_root);
            record.state = State::ConversionVerified;
            record.proofs.remove("upload");
            record.proofs.remove("icloudpd_local_mirror");
            fs::create_dir_all(&config.heic_output_dir).expect("HEIC output root should exist");
            let heic_path = config.heic_output_dir.join(format!("{asset_id}.HEIC"));
            let heic_bytes = vec![index as u8; 10];
            fs::write(&heic_path, &heic_bytes).expect("HEIC fixture should exist");
            let heic_sha256 = format!("{:x}", Sha256::digest(&heic_bytes));
            for proof_key in ["conversion", "heic"] {
                let proof = record.proofs.get_mut(proof_key).expect("HEIC proof");
                proof["heic_path"] = json!(heic_path);
                proof["heic_sha256"] = json!(heic_sha256);
            }
            initial.upsert_trusted(record);
        }
        let state_store = Arc::new(
            AssetStateStore::open_writer(
                &config.manifest_path,
                "rolling-upload-concurrency-test",
                Duration::from_secs(60),
            )
            .expect("state store should open"),
        );
        state_store
            .persist_manifest_records_trusted(&initial)
        ...
        let children_ready = Arc::new(std::sync::Barrier::new(WORKERS));
        ...
        for index in 0..WORKERS {
            ...
            handles.push(thread::spawn(move || {
                run_rolling_asset_upload_with_child(
                    ...
                    &manifest,
                    &summary,
                    move |snapshot_path, child_asset_id, record, _, _| {
                        let snapshot = AssetStateStore::open_immutable_read_only(snapshot_path)
                            .expect("child snapshot should open immutably");
                        let durable = snapshot.load().expect("child snapshot should load");
                        assert_eq!(
                            durable
                                .get(child_asset_id)
                                .expect("snapshot asset should exist")
                                .updated_at,
                            record.updated_at
                        );
                        children_ready.wait();
                        let heic_sha256 = record
                            .proofs
                            .get("heic")
                            .and_then(|proof| proof.get("heic_sha256"))
                            .and_then(Value::as_str)
                            .expect("HEIC hash should be present");
                        serde_json::to_vec(&json!({
                            "version": UPLOAD_PROOF_CHILD_PROTOCOL_VERSION,
                            "asset_id": child_asset_id,
                            "outcome": {
                                "response": {
                                    "asset_id": format!("uploaded-{child_asset_id}"),
                                    "filename": null,
                                    "master_id": null,
                                    "database_scope": "private",
                                    "zone_name": "PrimarySync"
                                },
                                "streamed_heic_sha256": heic_sha256,
                                "streamed_size_bytes": 10,
                                "timings": UploadTimings::default()
                            }
                        }))
                        .map_err(|source| MonitorError::CommandFailed {
                            program: "test-upload-child",
        ...
        for handle in handles {
            handle
                .join()
                .expect("rolling upload worker should not panic")
                .expect("rolling upload worker should complete");
        }

        let summary = summary.lock().expect("summary should not be poisoned");
        assert_eq!(summary.uploads_attempted, WORKERS as u64);
        assert_eq!(summary.uploads_completed, WORKERS as u64);
        drop(summary);
        let durable = state_store.load().expect("durable state should load");
        assert_eq!(durable.records().len(), WORKERS);
        assert!(
            durable
                .records()
                .values()
                .all(|record| record.state == State::UploadVerified)
        );
        assert!(
            durable
                .records()
                .values()
                .all(|record| record.proofs.contains_key("upload"))
        );
    }
    ...
    fn rolling_adjusted_source_success_exact_cas_recovers_and_binds_private_or_shared_source() {
        for (database_scope, zone_name) in [
            ...
            (CloudKitDatabaseScope::Shared, "SharedSync-test"),
        ] {
            let tempdir = tempfile::tempdir().expect("tempdir should be created");
            let mut config = MonitorConfig::new(
                tempdir.path().join("download"),
            ...
            let owner_record_name = (database_scope == CloudKitDatabaseScope::Shared)
                .then(|| "_shared-owner".to_string());
            if let Some(owner_record_name) = &owner_record_name {
                record
                    .proofs
                    .get_mut("original_asset")
                    .expect("original proof")["owner_record_name"] = json!(owner_record_name);
            }
            manifest.upsert(record);
            admit_adjusted_source_required_assets(&mut manifest, 1, 1, 1_000)
    ...
    #[test]
    fn rolling_delete_admissions_never_refill_beyond_per_scan_cap() {
        let tempdir = tempfile::tempdir().expect("tempdir should be created");
        let mut config = MonitorConfig::new(
            tempdir.path(),
            tempdir.path().join("manifest.json"),
        ...
        let mut manifest = Manifest::new();
        for index in 0..12 {
            let asset_id = format!("delete-ready-{index:02}");
            manifest.upsert_trusted(upload_verified_delete_ready_record(
                &asset_id,
                &config.download_root,
            ));
        }
        let first_window = (0..6)
            .map(|index| format!("delete-ready-{index:02}"))
            .collect::<Vec<_>>();
        let second_window = (6..12)
            .map(|index| format!("delete-ready-{index:02}"))
            .collect::<Vec<_>>();
        let mut admitted = BTreeSet::new();

        let first = admit_rolling_lifecycle_delete_asset_ids(
            &manifest,
            &config,
            &first_window,
            &mut admitted,
        );
        let second = admit_rolling_lifecycle_delete_asset_ids(
            &manifest,
            &config,
            &second_window,
            &mut admitted,
        );
        let exhausted = admit_rolling_lifecycle_delete_asset_ids(
            &manifest,
            &config,
            &second_window,
            &mut admitted,
    ...
    fn batched_delete_lifecycle_splits_cloudkit_modify_by_library_destination() {
        ...
        assert_eq!(
            transport.payloads[1]["zoneID"]["ownerRecordName"],
            "_shared-owner"
        );
        assert_eq!(manifest.get("asset-a").unwrap().state, State::Deleted);
        assert_eq!(manifest.get("asset-b").unwrap().state, State::Deleted);
    ...
    #[test]
    fn monitor_failure_event_redacts_nested_legacy_migration_json_details() {
        let sentinels = [
            "SENTINEL_ASSET_ID",
            "SENTINEL_PATH",
            "SENTINEL_REMOTE_ID",
        ...
        let sensitive_value = sentinels.join("__");
        let source =
            format!(r#"{{"schema_version":1,"entries":[{{"phase":"{sensitive_value}"}}]}}"#);
        let nested = serde_json::from_str::<
            crate::legacy_upload_migration::LegacyUploadMigrationJournal,
        >(&source)
        .expect_err("sentinel migration journal must be invalid");
        let error = MonitorError::LegacyUploadMigration(LegacyUploadMigrationError::from(nested));

        let event = monitor_failure_event(&error, 123, None);
        let encoded = serde_json::to_string(&event).unwrap();
        assert_eq!(
            event["fields"]["error"],
            "legacy upload migration classification error: legacy upload migration journal JSON is invalid"
        );
        for sentinel in sentinels {
            assert!(
                !encoded.contains(sentinel),
                "monitor failure report leaked {sentinel}"
            );
        }
    }
        ...
        for proof_key in ["original_asset", "upload"] {
            ...
            if database_scope == "shared" {
                record
                    ...
                    .as_object_mut()
                    .expect("proof should be object")
                    .insert("owner_record_name".to_string(), json!("_shared-owner"));
            }
        }
    }
    ...
    fn test_adjusted_source_lookup_response(original: &OriginalAssetProof, jpeg: &[u8]) -> Value {
        json!({
            "records": [{
                ...
                "zoneID": {
                    "zoneName": original.zone_name,
                    "ownerRecordName": original.owner_record_name
                },
                "fields": {
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn native_reverify_visual_outputs_use_private_workspace_while_staged_inputs_stay_sealed() {
        use std::os::unix::fs::PermissionsExt;

        let source = tempfile::tempdir().expect("source tempdir should be created");
        let source_root = source.path().canonicalize().expect("canonical source root");
        let reference = source_root.join("reference.jpg");
        let final_path = source_root.join("final.heic");
        let mirror = source_root.join("mirror.heic");
        fs::write(&reference, b"reference pixels").expect("reference source should be written");
        fs::write(&final_path, b"final pixels").expect("final source should be written");
        fs::write(&mirror, b"final pixels").expect("mirror source should be written");
        let final_sha256 = format!("{:x}", Sha256::digest(b"final pixels"));
        let mut snapshot = crate::adjusted_source::materialize_reverify_heic_snapshot(
            &reference,
            &final_path,
            &mirror,
            &final_sha256,
        ...
        .expect("immutable reverify snapshot should materialize");
        let reference_staging = snapshot
            .reference_path()
            .parent()
            .expect("reference staging directory")
            .to_path_buf();
        let final_staging = snapshot
            .final_path()
            .parent()
            .expect("final staging directory")
            .to_path_buf();
        for staging in [&reference_staging, &final_staging] {
            assert_eq!(
                fs::metadata(staging)
                    .expect("staging metadata should be readable")
                    .permissions()
                    .mode()
                    & 0o777,
                0o500
        ...
        snapshot
            .revalidate()
            .expect("staged inputs should begin sealed");
        let fake = FakeSealedReverifyVisualVerifier::new();
        ...
        .unwrap_or_else(|error| {
            panic!(
                "reverify output planning must not write beside immutable evidence: {error}; outputs: {:?}; write failure: {}",
                fake.output_paths(), fake.write_errors()
            )
        });

        assert_eq!(metrics.match_basis, VisualMatchBasis::Direct);
        let output_paths = fake.output_paths();
        assert_eq!(output_paths.len(), 2);
        assert!(output_paths.iter().all(|output| {
            output.parent() != Some(reference_staging.as_path())
                && output.parent() != Some(final_staging.as_path())
        }));
        assert!(output_paths.iter().all(|output| !output.exists()));
        snapshot
            .revalidate()
            .expect("visual verification must preserve sealed staged inputs");
        for staging in [&reference_staging, &final_staging] {
            assert_eq!(
                fs::metadata(staging)
                    .expect("staging metadata should remain readable")
                    .permissions()
                    .mode()
                    & 0o777,
                0o500
        ...
        snapshot.cleanup().expect("snapshot cleanup should succeed");
    }

    #[cfg(target_os = "macos")]
    #[test]
    fn visual_workspace_preserves_primary_failure_when_cleanup_succeeds() {
        let fake = FakeVisualWorkspaceOutcomeVerifier::new(true, false);
        let error = fake
            .run()
            .expect_err("verification failure should be returned");
        ...
        assert!(
            ...
            "unexpected visual verification error: {error:?}; observed workspaces: {}",
            fake.observed_workspaces()
        );
    }

    #[cfg(target_os = "macos")]
    #[test]
    fn visual_workspace_reports_cleanup_failure_after_success() {
        let fake = FakeVisualWorkspaceOutcomeVerifier::new(false, true);
        let error = fake
            .run()
            .expect_err("cleanup failure must fail a successful verification");

        assert!(
            matches!(error, MonitorError::VisualVerificationWorkspace),
            "unexpected visual workspace cleanup error: {error:?}; observed workspaces: {}",
            fake.observed_workspaces()
        );
    }
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn visual_workspace_reports_combined_verification_and_cleanup_failure() {
        let fake = FakeVisualWorkspaceOutcomeVerifier::new(true, true);
        let error = fake.run().expect_err("both failures must be reported");

        assert_eq!(
            error.to_string(),
            "visual verification failed and workspace cleanup also failed",
            "unexpected combined visual failure: {error:?}; observed workspaces: {}",
            fake.observed_workspaces()
        );
    }
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn visual_workspace_returns_metrics_when_verification_and_cleanup_succeed() {
        let fake = FakeVisualWorkspaceOutcomeVerifier::new(false, false);
        let metrics = fake.run().unwrap_or_else(|error| {
            panic!(
                "verification and cleanup should succeed: {error:?}; observed workspaces: {}",
                fake.observed_workspaces()
            )
        });
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn visual_workspace_rejects_unsafe_tmpdir_parent() {
        use std::os::unix::fs::PermissionsExt;

        const CHILD_ENV: &str = "ICLOUDPD_OPTIMIZER_UNSAFE_VISUAL_TMPDIR_TEST";
        if env::var_os(CHILD_ENV).is_none() {
            let parent = tempfile::tempdir().expect("unsafe parent tempdir should be created");
            let mut permissions = fs::metadata(parent.path())
                .expect("unsafe parent metadata should be readable")
                .permissions();
            permissions.set_mode(0o777);
            fs::set_permissions(parent.path(), permissions)
                .expect("unsafe parent mode should be installed");
            let output = Command::new(env::current_exe().expect("current test executable"))
                .args([
                    "--exact",
                    "monitor::tests::visual_workspace_rejects_unsafe_tmpdir_parent",
                    "--nocapture",
                ])
                .env(CHILD_ENV, "1")
                .env("TMPDIR", parent.path())
                .output()
                .expect("isolated unsafe TMPDIR test should run");
            assert!(
                output.status.success(),
                "isolated unsafe TMPDIR test failed:\nstdout:\n{}\nstderr:\n{}",
        ...
        assert!(
            matches!(
                VisualVerificationWorkspace::create(),
                Err(MonitorError::VisualVerificationWorkspace)
            ),
            "world-writable non-sticky TMPDIR must be rejected"
        );
    }

    #[cfg(target_os = "macos")]
    #[test]
    fn visual_workspace_preview_read_does_not_follow_substituted_named_workspace() {
        use std::os::unix::fs::{DirBuilderExt, PermissionsExt};

        let mut workspace = VisualVerificationWorkspace::create()
            .expect("visual verification workspace should be created");
        let output_path = workspace
            .plan_output(workspace.path().join("candidate.heic-verify-preview.png"))
            .expect("preview output should be planned");
        let original_preview = RgbPreview {
            width: 2,
            height: 1,
            pixels: vec![0, 0, 0, 255, 255, 255],
        };
        let substituted_preview = RgbPreview {
            width: 2,
            height: 1,
            pixels: vec![255, 0, 0, 0, 255, 0],
        };
        write_rgb_preview_png(&output_path, &original_preview);
        let original_output = workspace
            .open_output(&output_path)
            .expect("original output should validate and remain held");

        let displaced_workspace = workspace.path.with_extension("displaced");
        fs::rename(&workspace.path, &displaced_workspace)
            .expect("owned workspace should be displaced for the substitution test");
        let mut builder = fs::DirBuilder::new();
        builder.mode(0o700);
        builder
            .create(&workspace.path)
            .expect("substituted workspace should be created");
        write_rgb_preview_png(&output_path, &substituted_preview);

        let observed = read_rgb_preview_file(&output_path, &original_output.file)
            .expect("the previously validated preview should remain readable by exact identity");
        assert_eq!(
            observed.pixels, original_preview.pixels,
            "preview decoding must not reopen the substituted named path"
        );
        assert!(
            workspace.cleanup().is_err(),
            "cleanup must reject the substituted workspace identity"
        );
        assert!(
            output_path.exists(),
            "cleanup must not unlink a substituted named child"
        );

        fs::remove_file(&output_path).expect("substituted output should be removed");
        fs::remove_dir(&workspace.path).expect("substituted workspace should be removed");
        fs::rename(&displaced_workspace, &workspace.path)
            .expect("original workspace name should be restored");
        workspace
            .cleanup()
            .expect("original workspace should clean up");
        assert_eq!(
            fs::metadata(workspace.path().parent().expect("workspace parent"))
    ...
    fn monitor_guard_heartbeat_keeps_short_lease_live_and_surfaces_fencing() {
        ...
        let takeover_deadline = Duration::from_secs(3);
        let tempdir = tempfile::tempdir().expect("tempdir should be created");
        let config = MonitorConfig::new(
            tempdir.path().join("download"),
            tempdir.path().join("state/manifest.json"),
        ...
        let mut guard = acquire_monitor_run_guard_with_lease_ttl(&config, lease_ttl)
            .expect("monitor guard should lock");
        let state_store = guard
            .state_store(&config.manifest_path)
            .expect("guard should own state writer")
            .clone();

        thread::sleep(lease_ttl.saturating_mul(2));
        let takeover = AssetStateStore::open_writer(&config.manifest_path, "writer-b", lease_ttl);
        assert!(
            matches!(takeover, Err(AssetStateStoreError::WriterLeaseHeld { .. })),
            "heartbeat lease was not held: {takeover:?}"
        );
        ...
        let deadline = Instant::now() + takeover_deadline;
        loop {
            match AssetStateStore::open_writer(&config.manifest_path, "writer-c", lease_ttl) {
                Ok(writer) => {
                    writer
                        .release_writer_lease()
    ...
    #[cfg(unix)]
    #[test]
    fn run_monitor_preserves_completed_migration_pair_and_processes_only_ordinary_records() {
        let (tempdir, writer, _) =
            db_loaded_lifecycle_pair_at_phase(LegacyUploadMigrationPhase::Complete);
        let manifest_path = tempdir.path().join("manifest.json");
        let download_root = tempdir.path().join("download");
        let heic_output_dir = tempdir.path().join("heic");
        fs::create_dir_all(&download_root).expect("download root should exist");

        let mut ordinary = AssetRecord::new("ordinary", download_root.join("ordinary.DNG"));
        ordinary.state = State::Failed;
        ordinary.failures.push(FailureRecord::new_with_kind(
            "heic_verify",
            "terminal ordinary failure",
            FailureKind::HeicVisualContent,
        ));
        writer
            .persist_record_trusted(&ordinary)
            .expect("ordinary record should persist beside the sealed pair");
        writer
            .export_json()
            .expect("pre-scan checkpoint should contain the exact pair and ordinary record");
        let before = writer.load().expect("pre-scan database should load");
        let sealed_before = ["asset-a", "asset-b"].map(|asset_id| {
            serde_json::to_vec(before.get(asset_id).expect("sealed record should exist"))
                .expect("sealed record should encode")
        });
        writer
            .release_writer_lease()
            .expect("fixture writer should release before the monitor starts");
        drop(writer);

        let mut config = MonitorConfig::new(download_root, &manifest_path, heic_output_dir);
        config.full_lifecycle = true;
        config.rolling_lifecycle = true;
        config.auto_delete = true;
        config.jobs = 2;
        config.max_lifecycle_per_scan = 1;
        config.upload_session_path = Some(tempdir.path().join("must-not-read-upload-session.json"));
        let delete_session_path = tempdir.path().join("delete-session.json");
        write_test_read_session(&delete_session_path);
        config.delete_session_path = Some(delete_session_path);
        let mut guard = acquire_monitor_run_guard(&config).expect("monitor guard should lock");

        let summary = run_monitor_once(&config, &mut guard)
            .expect("completed migration records must not block restored service");

        assert_eq!(summary.uploads_attempted, 0);
        assert_eq!(summary.uploads_completed, 0);
        assert_eq!(summary.originals_deleted, 0);
        assert_eq!(summary.failures, 0);
        assert_eq!(
            summary.state_counts,
            BTreeMap::from([("needs_review".to_string(), 1)])
        );
        ...
        let after = guard
            .state_store(&manifest_path)
            .expect("monitor should retain its writer")
            .load()
            .expect("post-scan database should load");
        assert_eq!(after.get("ordinary").unwrap().state, State::NeedsReview);
        let (ordinary_after, sealed_asset_ids) = classify_legacy_upload_migration_records(&after)
            .unwrap()
            .into_parts();
        assert_eq!(
            sealed_asset_ids,
        ...
        assert_eq!(
            pending_lifecycle_count_for_config(&ordinary_after, &config),
            0
        );
        assert_eq!(
            LifecycleAdmissionBudget::for_scan_with_auto_delete(&ordinary_after, 1, true)
                .remaining_slots(),
            1
        );
        let active = active_lifecycle_asset_ids_for_config(&config, &ordinary_after);
        assert!(active.is_empty());
        assert_eq!(rolling_lifecycle_worker_count(&config, active.len()), 0);
        assert!(
            rolling_lifecycle_worker_asset_ids(
                &ordinary_after,
                &config,
        ...
        assert!(conversion_requests(&ordinary_after, &config, Some(&active)).is_empty());
        assert!(delete_lifecycle_asset_ids(&ordinary_after, &config, &active).is_empty());
        assert_eq!(
            startup_delete_audit(&ordinary_after),
            StartupDeleteAudit::default()
        );
        let metrics = VerifiedMetrics::from_manifest(&ordinary_after);
        assert_eq!(metrics.state_counts, summary.state_counts);
        assert_eq!(metrics.terminal_records, summary.terminal_records);
        assert_eq!(metrics.pending_records, summary.pending_records);
        let checkpoint = Manifest::load(&manifest_path).expect("post-scan checkpoint should load");
        for (index, asset_id) in ["asset-a", "asset-b"].into_iter().enumerate() {
            assert_eq!(
                serde_json::to_vec(after.get(asset_id).expect("sealed DB record should remain"))
                    .unwrap(),
                sealed_before[index]
            );
            assert_eq!(
                serde_json::to_vec(
                    checkpoint
                        .get(asset_id)
                        .expect("sealed checkpoint record should remain"),
                )
                .unwrap(),
                sealed_before[index]
            );
        }
        assert!(!config.upload_session_path.as_ref().unwrap().exists());
    }

    #[cfg(unix)]
    #[test]
    fn run_monitor_rejects_incomplete_migration_pair_without_state_or_checkpoint_writes() {
        let (tempdir, writer, before) =
            db_loaded_lifecycle_pair_at_phase(LegacyUploadMigrationPhase::Reset);
        let manifest_path = tempdir.path().join("manifest.json");
        let checkpoint_before = fs::read(&manifest_path).expect("checkpoint should exist");
        writer
            .release_writer_lease()
            .expect("fixture writer should release before the monitor starts");
        drop(writer);
        let download_root = tempdir.path().join("download");
        fs::create_dir_all(&download_root).expect("download root should exist");
        let config = MonitorConfig::new(download_root, &manifest_path, tempdir.path().join("heic"));
        let mut guard = acquire_monitor_run_guard(&config).expect("monitor guard should lock");

        let error = run_monitor_once(&config, &mut guard)
            .expect_err("incomplete migration journal must block ordinary monitor work");

        assert!(matches!(
            error,
            MonitorError::LegacyUploadMigration(LegacyUploadMigrationError::IncompleteJournal {
                phase: LegacyUploadMigrationPhase::Reset,
            },)
        ));
        assert_eq!(
            guard.state_store(&manifest_path).unwrap().load().unwrap(),
            before
        );
    ...
    #[cfg(unix)]
    #[test]
    fn run_monitor_rejects_tampered_migration_journal_without_asset_or_checkpoint_writes() {
        let (tempdir, writer, completed) =
            db_loaded_lifecycle_pair_at_phase(LegacyUploadMigrationPhase::Complete);
        let manifest_path = tempdir.path().join("manifest.json");
        let checkpoint_before = fs::read(&manifest_path).expect("checkpoint should exist");
        writer
            .release_writer_lease()
            .expect("fixture writer should release before database tampering");
        drop(writer);
        let database_path = AssetStateStore::db_path_for_manifest(&manifest_path);
        let mut tampered = completed.get("asset-a").unwrap().clone();
        tampered
            .proofs
            .get_mut(crate::legacy_upload_migration::LEGACY_UPLOAD_MIGRATION_PROOF_NAME)
            .unwrap()["entries"][0]["witness_sha256"] = json!("f".repeat(64));
        let tampered_json = serde_json::to_string(&tampered).unwrap();
        let connection = rusqlite::Connection::open(&database_path).unwrap();
        connection
            .execute(
                "UPDATE assets SET record_json = ?1 WHERE asset_id = ?2",
                rusqlite::params![tampered_json, "asset-a"],
            )
            .unwrap();
        let asset_rows_before = connection
        ...
        let error = run_monitor_once(&config, &mut guard)
            .expect_err("tampered migration journal must block ordinary monitor work");

        assert!(matches!(
            error,
            MonitorError::StateStore(AssetStateStoreError::Manifest(
                ManifestError::ReservedInternalProofCapabilityMismatch,
            ))
        ));
        let connection = rusqlite::Connection::open(&database_path).unwrap();
        let asset_rows_after = connection
            .prepare("SELECT asset_id, record_json FROM assets ORDER BY asset_id")
            .unwrap()
            .query_map([], |row| {
                Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?))
            ...
            .collect::<Result<Vec<_>, _>>()
            .unwrap();
        assert_eq!(asset_rows_after, asset_rows_before);
        assert_eq!(fs::read(&manifest_path).unwrap(), checkpoint_before);
    }
