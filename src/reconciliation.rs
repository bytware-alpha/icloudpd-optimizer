...
type RemoteRecordIdentity = (CloudKitDatabaseScope, String, Option<String>, String);
...
pub enum OriginalAssetResolutionDisposition {
    ...
    Coexistence {
        ...
        replacement_proof: OriginalAssetReplacementProof,
    },
    NoDateCandidate,
    NoRawResource,
    RawSizeMismatch,
...
impl From<CloudKitReplacementResourceProof> for OriginalAssetReplacementProof {
    fn from(proof: CloudKitReplacementResourceProof) -> Self {
        Self {
            ...
            owner_record_name: proof.owner_record_name,
        ...
        for target in &batch.targets {
            ...
            let (new_state, original_asset_proof, disposition) = match &resolution.disposition {
                ...
                CloudKitOriginalAssetResolveDisposition::Coexistence {
                    ...
                    insert_remote_record_identity(
                        &mut seen_remote_records,
                        &batch.destination,
                        &original_proof.record_name,
                        &replacement_proof.record_name,
                    )?;
                    summary.no_action = summary.no_action.saturating_add(1);
                    (
                        State::NoAction,
                        None,
                        OriginalAssetResolutionDisposition::Coexistence {
                            original_proof: original_proof.clone(),
                            replacement_proof: replacement_proof.clone().into(),
                        },
                    )
                }
...
fn validate_coexistence_proofs(
    asset_id: &str,
    original_proof: &OriginalAssetProof,
    ...
    validate_exact_original_proof(asset_id, original_proof, target, destination)?;
    validate_replacement_proof(asset_id, replacement_proof, target, destination)
}

fn insert_remote_record_identity(
    seen_remote_records: &mut BTreeSet<RemoteRecordIdentity>,
    destination: &CloudKitLibraryDestination,
    original_record_name: &str,
    replacement_record_name: &str,
) -> Result<(), OriginalAssetResolutionError> {
    let original_identity = remote_record_identity(destination, original_record_name);
    if !seen_remote_records.insert(original_identity) {
        return Err(OriginalAssetResolutionError::DuplicateOriginalRecord {
            record_name: original_record_name.to_string(),
        });
    }
    if replacement_record_name != original_record_name
        && !seen_remote_records.insert(remote_record_identity(destination, replacement_record_name))
    {
        return Err(OriginalAssetResolutionError::DuplicateOriginalRecord {
            record_name: replacement_record_name.to_string(),
        });
    }
    Ok(())
}
...
fn resolution_evidence(
    ...
    match disposition {
        ...
        CloudKitOriginalAssetResolveDisposition::Coexistence { .. } => {
            Some(ResolutionEvidence::Coexistence)
        }
        CloudKitOriginalAssetResolveDisposition::NoDateCandidate => {
...
fn evidence_is_coherent(
    ...
    match evidence {
        ...
        ResolutionEvidence::Coexistence => {
            observations.date_candidates > 0
                && observations.raw_resources > 0
                && observations.raw_size_matches > 0
                && observations.raw_hash_matches == 1
...
fn insert_existing_remote_identity(
    ...
    let record_name = identity.3.clone();
...
fn remote_record_identity(
    ...
    (
        ...
        destination.owner_record_name.clone(),
...
) -> Result<Vec<RemoteRecordIdentity>, OriginalAssetResolutionError> {
    ...
    let (evidence, identity) = match &proof.disposition {
        OriginalAssetResolutionDisposition::ExactOriginal { proof: original } => {
            ...
            if top_level_original != Some(original)
                ...
                || original.owner_record_name != proof.destination.owner_record_name
        ...
        OriginalAssetResolutionDisposition::ReplacementPresent { proof: replacement } => {
            if !is_safe_remote_identity(&replacement.record_name)
                ...
                || replacement.owner_record_name != proof.destination.owner_record_name
            {
                return Err(malformed_existing_resolution(record));
            }
            (
        ...
        OriginalAssetResolutionDisposition::Coexistence {
            ...
            if !is_safe_remote_identity(&original_proof.record_name)
                || !is_safe_remote_identity(&original_proof.record_change_tag)
                || original_proof.record_type != "CPLAsset"
                || !is_safe_single_filename(&original_proof.filename)
                || original_proof.filename != proof.source.filename
                || original_proof.size_bytes != proof.source.nas_size_bytes
                || original_proof.matched_raw_sha256 != proof.source.nas_sha256
                || original_proof.database_scope != proof.destination.database_scope
                || original_proof.zone_name != proof.destination.zone_name
                || original_proof.owner_record_name != proof.destination.owner_record_name
                || !is_safe_remote_identity(&replacement_proof.record_name)
                || !is_safe_remote_identity(&replacement_proof.record_change_tag)
                || replacement_proof.record_type != "CPLAsset"
                || !is_safe_remote_identity(&replacement_proof.resource_field)
                || replacement_proof.size_bytes == 0
                || !is_sha256(&replacement_proof.matched_heic_sha256)
                || replacement_proof.database_scope != proof.destination.database_scope
                || replacement_proof.zone_name != proof.destination.zone_name
                || replacement_proof.owner_record_name != proof.destination.owner_record_name
            {
                return Err(malformed_existing_resolution(record));
            }
            (
                ResolutionEvidence::Coexistence,
                if original_proof.record_name == replacement_proof.record_name {
                    vec![remote_record_identity(
                        &proof.destination,
                        &original_proof.record_name,
                    )]
                    ...
                    vec![
                        remote_record_identity(&proof.destination, &original_proof.record_name),
                        remote_record_identity(&proof.destination, &replacement_proof.record_name),
                    ]
                },
            )
        }
        OriginalAssetResolutionDisposition::NoDateCandidate => {
            (ResolutionEvidence::NoDateCandidate, Vec::new())
        }
        OriginalAssetResolutionDisposition::NoRawResource => {
            (ResolutionEvidence::NoRawResource, Vec::new())
        }
        OriginalAssetResolutionDisposition::RawSizeMismatch => {
            (ResolutionEvidence::RawSizeMismatch, Vec::new())
        }
        OriginalAssetResolutionDisposition::RawHashMismatch => {
            (ResolutionEvidence::RawHashMismatch, Vec::new())
        }
        OriginalAssetResolutionDisposition::Ambiguous => {
            (ResolutionEvidence::Ambiguous, Vec::new())
        }
    };
