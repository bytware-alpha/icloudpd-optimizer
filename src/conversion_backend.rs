const MACOS_REQUIRED_TOOLS: [&str; 5] = ["sips", "heif-info", "magick", "exiftool", "cp"];
const LINUX_REQUIRED_TOOLS: [&str; 4] = ["heif-enc", "heif-info", "magick", "exiftool"];
...
pub fn backend_report_for_target(target: TargetPlatform) -> ConversionBackendReport {
    match target.os {
        ...
        "linux" => ConversionBackendReport {
            name: "linux-native",
            workflow_convert_supported: true,
            reason: "workflow convert is supported by the Linux native embedded-preview/heif-enc backend",
        },
