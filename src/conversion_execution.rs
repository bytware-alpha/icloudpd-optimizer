...
fn execute_measured_conversion_for_target(
    ...
    let conversion_result = (|| {
        ...
        // Conversion tools commonly create files with the process umask (often
        // 0644).  Seal the output before hashing or recording any proof so the
        // file that subsequent immutable-image checks observe is owner-only.
        // This is deliberately fail-closed: a failed chmod, fsync, identity
        // check, or path revalidation aborts the conversion and cleanup removes
        // the untrusted output.
        seal_conversion_output(&request.output_path)?;
...
/// Seal a newly generated conversion output before it becomes proof-bearing.
///
/// The descriptor is opened without following symlinks, checked for a regular
/// file owned by the current user with a single link, changed to exact 0600,
/// synchronised, and then revalidated through both the descriptor and pathname.
/// Any failure is surfaced as the existing redacted output-unreadable failure
/// kind; callers remove the generated output on error.
fn seal_conversion_output(path: &Path) -> Result<(), ConversionExecutionError> {
    #[cfg(unix)]
    {
        ...
        let file =
            options
                .open(path)
                .map_err(|source| ConversionExecutionError::OutputUnreadable {
                    path: path.to_path_buf(),
                    source,
                })?;
        let before =
        ...
        validate_conversion_output_identity(&before).map_err(|source| {
            ConversionExecutionError::OutputUnreadable {
                path: path.to_path_buf(),
                source,
            }
        ...
        validate_sealed_conversion_output_metadata(&sealed).map_err(|source| {
            ConversionExecutionError::OutputUnreadable {
                path: path.to_path_buf(),
                source,
            }
        ...
        if !same_conversion_output_identity(&sealed, &path_metadata) {
            return Err(ConversionExecutionError::OutputUnreadable {
                path: path.to_path_buf(),
                source: io::Error::new(
                    io::ErrorKind::InvalidData,
                    "conversion output changed during sealing",
                ),
            });
        }
        Ok(())
    ...
    {
        let _ = path;
        Err(ConversionExecutionError::OutputUnreadable {
            path: path.to_path_buf(),
            source: io::Error::new(
                io::ErrorKind::Unsupported,
                "conversion output sealing requires a Unix filesystem",
            ),
        })
    }
}
...
#[cfg(unix)]
fn validate_conversion_output_identity(metadata: &fs::Metadata) -> io::Result<()> {
    if !metadata.file_type().is_file() {
    ...
    if metadata.uid() != current_euid {
        return Err(io::Error::new(
            ...
            "conversion output owner does not match the current user",
        ));
    }
    if metadata.nlink() != 1 {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "conversion output has multiple hard links",
        ));
    }
    Ok(())
}
...
#[cfg(unix)]
fn validate_sealed_conversion_output_metadata(metadata: &fs::Metadata) -> io::Result<()> {
    validate_conversion_output_identity(metadata)?;
    if metadata.permissions().mode() & 0o7777 != 0o600 {
        return Err(io::Error::new(
            io::ErrorKind::PermissionDenied,
            "conversion output is not sealed owner-only",
        ));
    }
    Ok(())
}
...
#[cfg(unix)]
fn same_conversion_output_identity(left: &fs::Metadata, right: &fs::Metadata) -> bool {
...
mod tests {
    ...
    use crate::adjusted_source::adjusted_source_proof_digest;
    ...
    use crate::proof::NasRawProof;
    #[cfg(all(unix, not(target_os = "linux")))]
    use crate::workflow::ConversionSourceBinding;
    use crate::workflow::{
        OriginalAssetProof, discover_raw_asset, record_adjusted_source_proof, record_nas_proof,
    ...
    fn linux_conversion_runs_full_chain_and_records_chain_tool_name() {
        ...
        assert_eq!(
            fs::metadata(&output_path)
                .expect("heic output metadata should be readable")
                .permissions()
                .mode()
                & 0o7777,
            0o600,
            "successful conversion output must be sealed owner-only before proof recording"
        );
    ...
    #[cfg(unix)]
    #[test]
    fn conversion_output_sealing_fails_closed_for_non_regular_output() {
        let tempdir = tempfile::tempdir().expect("tempdir should be created");
        let output_path = tempdir.path().join("IMG_0001.heic");
        fs::create_dir(&output_path).expect("output directory should be created");
        let before_mode = fs::metadata(&output_path)
        ...
        let error =
            seal_conversion_output(&output_path).expect_err("sealing a directory must fail closed");
        assert!(matches!(
            error,
            ConversionExecutionError::OutputUnreadable { path, .. } if path == output_path
        ));
        assert_eq!(
            fs::metadata(&output_path)
                ...
                & 0o7777,
            before_mode,
            "failed sealing must not mutate a non-regular output"
        );
    }
...
"#,
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            assert!(
                !command_log.contains("exiftool-metadata\n"),
                "post-child source identity failure must abort before metadata copy"
            );
        }
        #[cfg(not(target_os = "linux"))]
        let updated =
...
"#,
        ...
        assert!(
            ...
            "unexpected batch timeout failure: {observed_error}"
        );
...
"#,
        ...
        assert!(
            ...
            "unexpected completed-chunk timeout failure: {observed_error}"
        );
        for asset_id in ["asset-1", "asset-2"] {
            assert_eq!(
                manifest.get(asset_id).expect("asset should exist").state,
...
"#,
    ...
    impl PathGuard {
        ...
        fn preserve() -> Self {
            let lock = crate::PROCESS_PATH_TEST_LOCK
                .lock()
                .unwrap_or_else(std::sync::PoisonError::into_inner);
