...
use crate::legacy_upload_migration::{
    LegacyUploadMigrationManifestRegistryAuthority, LegacyUploadMigrationRegistry,
    LegacyUploadMigrationRegistryWriteMode, LegacyUploadMigrationStateStoreWriteAuthority,
    validate_legacy_upload_migration_record, validate_legacy_upload_migration_registry,
    validate_legacy_upload_migration_registry_binding,
};
use crate::manifest::{
    AssetRecord, Manifest, ManifestError, record_has_reserved_internal_proof,
    sanitize_untrusted_recipe_claims,
};
#[cfg(unix)]
...
#[cfg(unix)]
const JSON_CHECKPOINT_READ_BUFFER_BYTES: usize = 64 * 1024;
...
std::thread_local! {
    ...
    static JSON_CHECKPOINT_READ_HOOK: RefCell<Option<Box<dyn FnOnce()>>> = const { RefCell::new(None) };
}
...
#[cfg(test)]
fn set_json_checkpoint_read_hook_for_current_thread(hook: impl FnOnce() + 'static) {
    JSON_CHECKPOINT_READ_HOOK.with(|slot| *slot.borrow_mut() = Some(Box::new(hook)));
}

#[cfg(test)]
fn run_json_checkpoint_read_hook() {
    JSON_CHECKPOINT_READ_HOOK.with(|slot| {
        if let Some(hook) = slot.borrow_mut().take() {
            hook();
        }
    });
...
static IMMUTABLE_SNAPSHOT_SEQUENCE: AtomicU64 = AtomicU64::new(0);

struct ImmutableSnapshotDirectory {
    path: PathBuf,
}

impl Drop for ImmutableSnapshotDirectory {
    fn drop(&mut self) {
        let _ = fs::remove_dir_all(&self.path);
    }
}

/// A point-in-time database copy suitable for immutable child-process reads.
///
/// The snapshot is isolated from the live WAL, so concurrent monitor workers can
/// continue committing distinct assets while a private upload child validates its
/// exact record witness.
pub(crate) struct AssetStateStoreImmutableSnapshot {
    #[allow(dead_code)]
    temporary_directory: ImmutableSnapshotDirectory,
    manifest_path: PathBuf,
}

impl AssetStateStoreImmutableSnapshot {
...
pub(crate) struct StateStoreManifestRecordAuthority {
    asset_id: String,
    record_sha256: String,
}

impl StateStoreManifestRecordAuthority {
    fn for_record(record: &AssetRecord) -> Result<Self, ManifestError> {
        if record_has_reserved_internal_proof(record)
            && validate_legacy_upload_migration_record(record).is_err()
        {
            return Err(ManifestError::ReservedInternalProofCapabilityMismatch);
        }
        Ok(Self {
            asset_id: record.asset_id.clone(),
            record_sha256: state_store_asset_record_sha256(record)?,
        })
    }

    pub(crate) fn authorizes(&self, record: &AssetRecord) -> bool {
        self.asset_id == record.asset_id
            && state_store_asset_record_sha256(record)
                .is_ok_and(|digest| digest == self.record_sha256)
    }
}

pub(crate) struct StateStoreManifestExportAuthority {
    destination: PathBuf,
    manifest_sha256: String,
}

impl StateStoreManifestExportAuthority {
    fn for_manifest(manifest: &Manifest, destination: &Path) -> Result<Self, ManifestError> {
        ...
        Ok(Self {
            ...
            manifest_sha256: state_store_manifest_sha256(manifest)?,
        })
    }

    pub(crate) fn authorizes(&self, manifest: &Manifest, destination: &Path) -> bool {
        self.destination == destination
            && state_store_manifest_sha256(manifest)
...
impl AssetStateStore {
    ...
    pub fn migrate_schema_only(
        ...
        #[cfg(unix)]
        {
            ...
            transaction.commit()?;
            checkpoint_owned_wal(&connection);
    ...
    /// Materialize a stable state database copy for a child process that must
    /// use an immutable read. The live writer is fenced only for the short
    /// witness/copy window; subsequent worker commits cannot invalidate the
    /// child's immutable snapshot.
    pub(crate) fn create_immutable_read_snapshot(
        &self,
    ) -> Result<AssetStateStoreImmutableSnapshot, AssetStateStoreError> {
        let _mutation_guard = self.mutation_guard();
        let token = self.writer_token()?;
        let initial_witness = capture_immutable_read_witness(&self.db_path)?;
        let connection = self.connect_writer()?;
        self.validate_writer_lease_read_only(&connection, token)?;
        let pre_copy_witness = capture_immutable_read_witness(&self.db_path)?;
        if pre_copy_witness != initial_witness {
            return Err(AssetStateStoreError::ImmutableReadSnapshotChanged);
        }

        let temporary_directory = create_immutable_snapshot_directory().map_err(|source| {
            AssetStateStoreError::CreateDirectory {
                path: self.db_path.clone(),
                source,
            }
        })?;
        let manifest_path = temporary_directory.path.join("manifest.json");
        let snapshot_db_path = AssetStateStore::db_path_for_manifest(&manifest_path);
        fs::copy(&self.db_path, &snapshot_db_path).map_err(|source| {
            AssetStateStoreError::ImmutableSnapshotCopy {
                path: snapshot_db_path.clone(),
                source,
            }
        })?;
        let snapshot_file = fs::File::open(&snapshot_db_path)
            .map_err(|source| AssetStateStoreError::ImmutableReadDatabaseIo { source })?;
        snapshot_file
            .sync_all()
            .map_err(|source| AssetStateStoreError::ImmutableReadDatabaseIo { source })?;
        let post_copy_witness = capture_immutable_read_witness(&self.db_path)?;
        if post_copy_witness != pre_copy_witness {
            return Err(AssetStateStoreError::ImmutableReadSnapshotChanged);
        }
        Ok(AssetStateStoreImmutableSnapshot {
            temporary_directory,
            manifest_path,
        })
    }
    ...
    pub(crate) fn load_for_legacy_upload_apply(
        &self,
        manifest_path: &Path,
    ) -> Result<Manifest, AssetStateStoreError> {
        if self.writer.is_none()
            || !manifest_path_identity_matches(&self.manifest_path, manifest_path)
        {
            return Err(AssetStateStoreError::WriterLeaseRequired);
        }
        self.renew_writer_lease()?;
        self.load()
    }

    pub(crate) fn revalidate_legacy_upload_apply_state(
        &self,
        manifest_path: &Path,
        expected: &Manifest,
    ) -> Result<(), AssetStateStoreError> {
        ...
        let transaction = connection.transaction_with_behavior(TransactionBehavior::Immediate)?;
        self.validate_writer_lease(&transaction, token)?;
        let actual = load_database_manifest_from_transaction(&transaction)?;
        if actual != *expected {
            return Err(AssetStateStoreError::LegacyUploadApplyStateChanged);
        }
        transaction.commit()?;
        checkpoint_owned_wal(&connection);
        Ok(())
    ...
    pub fn persist_record(&self, record: &AssetRecord) -> Result<Duration, AssetStateStoreError> {
        ...
        self.persist_record_with_owned_lease(&sanitized)
    }
    ...
    pub fn persist_records_atomic<'a>(
        ...
        reject_reserved_internal_proofs(records.iter().copied())?;
        let sanitized = records
            ...
            .collect::<Vec<_>>();
        self.persist_records_atomic_with_owned_lease(sanitized.iter())
    }
    ...
    pub fn persist_records_exact_cas_atomic<'a>(
        ...
        if updates.iter().any(|update| {
            record_has_reserved_internal_proof(update.expected)
                || record_has_reserved_internal_proof(update.updated)
        }) {
            return Err(AssetStateStoreError::ReservedInternalProofRequiresAuthority);
        }
    ...
    pub(crate) fn persist_records_exact_cas_atomic_trusted<'a>(
        ...
        self.persist_records_exact_cas_atomic_with_owned_lease(updates, None)
    }

    pub(crate) fn persist_records_exact_cas_atomic_with_legacy_upload_migration_authority<'a>(
        &self,
        authority: &LegacyUploadMigrationStateStoreWriteAuthority,
        updates: impl IntoIterator<Item = AssetRecordExactCasUpdate<'a>>,
    ) -> Result<Duration, AssetStateStoreError> {
        self.persist_records_exact_cas_atomic_with_owned_lease(updates, Some(authority))
    }
    ...
    pub fn persist_manifest_records(
        ...
        reject_reserved_internal_proofs(manifest.records().values())?;
        let mut sanitized = Manifest::new();
        for record in manifest.records().values() {
            sanitized.upsert(record.clone());
        }
        self.persist_manifest_records_with_owned_lease(&sanitized)
    }
    ...
    fn initialize_writer(&self, transaction: &Transaction<'_>) -> Result<(), AssetStateStoreError> {
        ...
        ensure_legacy_upload_migration_registry_table(transaction)?;
        Ok(())
    }
    ...
    fn validate_writer_lease_read_only(
        &self,
        connection: &Connection,
        token: &WriterLeaseToken,
    ) -> Result<(), AssetStateStoreError> {
        let (current_owner_id, current_epoch, expires_at_unix_ms): (String, i64, i64) = connection
            .query_row(
                "SELECT owner_id, epoch, expires_at_unix_ms FROM writer_lease WHERE singleton = 1",
                [],
                |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)),
            )?;
        let current_epoch = u64::try_from(current_epoch).map_err(|_| {
            AssetStateStoreError::InvalidWriterLeaseEpoch {
                epoch: current_epoch,
            }
        })?;
        if current_owner_id != token.owner_id
    ...
    fn load_or_import_with_owned_lease(&self) -> Result<Manifest, AssetStateStoreError> {
        ...
        if !import_metadata.imported_once {
            ...
            if let Some(registry) = snapshot.manifest.legacy_upload_migration_registry() {
                insert_or_replay_legacy_upload_migration_registry(&transaction, registry)?;
            }
    ...
    fn persist_record_with_owned_lease(
        ...
        let transaction = connection.transaction_with_behavior(TransactionBehavior::Immediate)?;
        self.validate_writer_lease(&transaction, token)?;
        reject_persisted_reserved_internal_proof(&transaction, &record.asset_id)?;
        ...
        self.validate_writer_lease(&transaction, token)?;
        {
            ...
            for (record, requested_json) in &prepared {
                let persisted = statement
                    ...
                    .optional()?;
                reject_reserved_internal_proof_json(
                    &record.asset_id,
                    persisted
                        .as_ref()
                        .map(|(_, record_json)| record_json.as_str()),
                )?;
    ...
    fn persist_records_exact_cas_atomic_with_owned_lease<'a>(
        &self,
        updates: impl IntoIterator<Item = AssetRecordExactCasUpdate<'a>>,
        legacy_upload_migration_authority: Option<&LegacyUploadMigrationStateStoreWriteAuthority>,
    ) -> Result<Duration, AssetStateStoreError> {
        let _mutation_guard = self.mutation_guard();
        let token = self.writer_token()?;
        let started = Instant::now();
        ...
        let transaction = connection.transaction_with_behavior(TransactionBehavior::Immediate)?;
        self.validate_writer_lease(&transaction, token)?;
        if let Some(authority) = legacy_upload_migration_authority {
            apply_legacy_upload_migration_registry_authority(&transaction, authority)?;
        }
        {
            let mut statement = transaction
    ...
    fn export_json_with_owned_lease(&self) -> Result<Manifest, AssetStateStoreError> {
        self.export_json_with_owned_lease_using(|manifest, path| {
            let authority = StateStoreManifestExportAuthority::for_manifest(manifest, path)?;
            manifest.save_atomic_with_state_store_export_authority(&authority, path)
        })
    }
        ...
        F: FnOnce(&Manifest, &Path) -> Result<(), ManifestError>,
    {
        ...
        self.validate_writer_lease(&transaction, token)?;
        let manifest = load_database_manifest_from_transaction(&transaction)?;
        if let Some(checkpoint) = load_json_checkpoint_no_follow(&self.manifest_path)?
            && checkpoint.legacy_upload_migration_registry()
                != manifest.legacy_upload_migration_registry()
            && checkpoint.legacy_upload_migration_registry().is_some()
        {
            return Err(AssetStateStoreError::JsonCheckpointRegistryMismatch);
        }
    ...
    fn load_database_manifest(&self) -> Result<Manifest, AssetStateStoreError> {
        ...
        let mut manifest = load_database_manifest_from_statement(&mut statement)?;
        drop(statement);
        install_database_legacy_upload_migration_registry(&connection, &mut manifest)?;
        validate_database_checkpoint_registry_binding(&self.manifest_path, &manifest)?;
        drop(connection);
...
fn reject_reserved_internal_proof(record: &AssetRecord) -> Result<(), AssetStateStoreError> {
    if record_has_reserved_internal_proof(record) {
...
fn reject_persisted_reserved_internal_proof(
    transaction: &Transaction<'_>,
    asset_id: &str,
) -> Result<(), AssetStateStoreError> {
    let record_json = transaction
        ...
        .optional()?;
    reject_reserved_internal_proof_json(asset_id, record_json.as_deref())
}

fn reject_reserved_internal_proof_json(
    asset_id: &str,
    ...
    let record: AssetRecord = crate::strict_json::from_str(record_json).map_err(|source| {
...
fn create_v2_schema(transaction: &Transaction<'_>) -> Result<(), AssetStateStoreError> {
    transaction.execute_batch(
        "CREATE TABLE assets (
         ...
         CREATE TABLE legacy_upload_migration_registry (
           singleton INTEGER PRIMARY KEY CHECK (singleton = 1),
...
fn ensure_legacy_upload_migration_registry_table(
    transaction: &Transaction<'_>,
) -> Result<(), AssetStateStoreError> {
    transaction.execute_batch(
        "CREATE TABLE IF NOT EXISTS legacy_upload_migration_registry (
           singleton INTEGER PRIMARY KEY CHECK (singleton = 1),
           schema_version INTEGER NOT NULL,
           registry_sha256 TEXT NOT NULL,
           registry_json TEXT NOT NULL
...
fn manifest_path_identity_matches(left: &Path, right: &Path) -> bool {
    normalized_manifest_path(left) == normalized_manifest_path(right)
}
...
fn load_database_manifest_from_transaction(
    ...
    let mut manifest = load_database_manifest_from_statement(&mut statement)?;
    drop(statement);
    install_database_legacy_upload_migration_registry(transaction, &mut manifest)?;
    Ok(manifest)
}
...
fn create_immutable_snapshot_directory() -> Result<ImmutableSnapshotDirectory, io::Error> {
    ...
    let timestamp = SystemTime::now()
        ...
        .as_nanos();
    for _ in 0..100 {
        let sequence = IMMUTABLE_SNAPSHOT_SEQUENCE.fetch_add(1, Ordering::Relaxed);
        let path = parent.join(format!(
            "icloudpd-state-snapshot-{}-{timestamp}-{sequence}",
            std::process::id()
        ));
        match fs::create_dir(&path) {
            Ok(()) => return Ok(ImmutableSnapshotDirectory { path }),
    ...
    Err(io::Error::new(
        ...
        "immutable snapshot directory name allocation exhausted",
    ))
}
...
#[cfg(unix)]
fn load_json_checkpoint_manifest_from_reader(
    reader: impl Read,
) -> Result<Option<Manifest>, ManifestError> {
    let reader = BufReader::with_capacity(JSON_CHECKPOINT_READ_BUFFER_BYTES, reader);
    match Manifest::load_from_reader(reader) {
        Ok(manifest) => Ok(Some(manifest)),
        Err(ManifestError::Json(_)) => Ok(None),
        Err(error) => Err(error),
    }
}
...
fn load_json_checkpoint_no_follow(path: &Path) -> Result<Option<Manifest>, AssetStateStoreError> {
    ...
    run_json_checkpoint_read_hook();
    let manifest = match load_json_checkpoint_manifest_from_reader(&mut file) {
...
/// Keep writer-owned commits out of the immutable-read rejection path. A busy
/// checkpoint is intentionally ignored: the committed mutation remains durable,
/// and a later immutable snapshot operation will fail closed while the WAL is live.
fn checkpoint_owned_wal(connection: &Connection) {
...
fn load_database_manifest_from_statement(
    ...
    while let Some(row) = rows.next()? {
        ...
        let authority = StateStoreManifestRecordAuthority::for_record(&record)?;
        manifest.upsert_state_store_record(&authority, record)?;
    }
    Ok(manifest)
}

fn legacy_upload_migration_registry_table_exists(
    connection: &Connection,
) -> Result<bool, AssetStateStoreError> {
    Ok(connection.query_row(
        "SELECT EXISTS(
           SELECT 1 FROM sqlite_master
           WHERE type = 'table' AND name = 'legacy_upload_migration_registry'
         )",
        [],
        |row| row.get::<_, i64>(0),
    )? == 1)
...
fn load_database_legacy_upload_migration_registry(
    connection: &Connection,
) -> Result<Option<LegacyUploadMigrationRegistry>, AssetStateStoreError> {
    if !legacy_upload_migration_registry_table_exists(connection)? {
        return Ok(None);
    }
    let persisted = connection
        .query_row(
            "SELECT schema_version, registry_sha256, registry_json
             FROM legacy_upload_migration_registry WHERE singleton = 1",
            [],
            |row| {
                Ok((
                    row.get::<_, i64>(0)?,
        ...
        .optional()?;
    let Some((schema_version, registry_sha256, registry_json)) = persisted else {
        return Ok(None);
    };
    let registry: LegacyUploadMigrationRegistry = crate::strict_json::from_str(&registry_json)
        .map_err(AssetStateStoreError::DecodeLegacyUploadMigrationRegistry)?;
    let canonical_json = serde_json::to_string(&registry)
        .map_err(AssetStateStoreError::DecodeLegacyUploadMigrationRegistry)?;
    if schema_version != i64::try_from(registry.schema_version).unwrap_or(i64::MAX)
        || registry_sha256 != registry.registry_sha256
        || canonical_json != registry_json
        || validate_legacy_upload_migration_registry(&registry).is_err()
    {
        return Err(AssetStateStoreError::LegacyUploadMigrationRegistryMismatch);
    }
    Ok(Some(registry))
}

fn install_database_legacy_upload_migration_registry(
    connection: &Connection,
    manifest: &mut Manifest,
) -> Result<(), AssetStateStoreError> {
    let Some(registry) = load_database_legacy_upload_migration_registry(connection)? else {
        return Ok(());
    };
...
fn validate_database_checkpoint_registry_binding(
    checkpoint_path: &Path,
    database_manifest: &Manifest,
) -> Result<(), AssetStateStoreError> {
    let Some(checkpoint) = load_json_checkpoint_no_follow(checkpoint_path)? else {
        return Ok(());
    };
    if checkpoint.legacy_upload_migration_registry().is_some()
        && checkpoint.legacy_upload_migration_registry()
            != database_manifest.legacy_upload_migration_registry()
    {
        return Err(AssetStateStoreError::JsonCheckpointRegistryMismatch);
    }
    Ok(())
...
fn encoded_legacy_upload_migration_registry(
...
fn insert_or_replay_legacy_upload_migration_registry(
    transaction: &Transaction<'_>,
    registry: &LegacyUploadMigrationRegistry,
) -> Result<(), AssetStateStoreError> {
    let registry_json = encoded_legacy_upload_migration_registry(registry)?;
    let persisted = load_database_legacy_upload_migration_registry(transaction)?;
    match persisted {
        Some(persisted) if persisted == *registry => Ok(()),
        Some(_) => Err(AssetStateStoreError::LegacyUploadMigrationRegistryMismatch),
        None => {
            let changed = transaction.execute(
                "INSERT INTO legacy_upload_migration_registry (
                   singleton, schema_version, registry_sha256, registry_json
                 ) VALUES (1, ?1, ?2, ?3)",
                params![
                    i64::try_from(registry.schema_version).unwrap_or(i64::MAX),
...
fn apply_legacy_upload_migration_registry_authority(
    transaction: &Transaction<'_>,
    authority: &LegacyUploadMigrationStateStoreWriteAuthority,
) -> Result<(), AssetStateStoreError> {
    match authority.mode() {
        LegacyUploadMigrationRegistryWriteMode::InsertOnceOrExactReplay => {
            insert_or_replay_legacy_upload_migration_registry(transaction, authority.registry())
        }
        LegacyUploadMigrationRegistryWriteMode::VerifyExact => {
            match load_database_legacy_upload_migration_registry(transaction)? {
                Some(persisted) if persisted == *authority.registry() => Ok(()),
                _ => Err(AssetStateStoreError::LegacyUploadMigrationRegistryMismatch),
            }
        }
    }
}
...
fn state_store_asset_record_sha256(record: &AssetRecord) -> Result<String, ManifestError> {
    Ok(format!("{:x}", Sha256::digest(serde_json::to_vec(record)?)))
}

fn state_store_manifest_sha256(manifest: &Manifest) -> Result<String, ManifestError> {
    Ok(format!(
        "{:x}",
...
pub enum AssetStateStoreError {
    ...
    #[error("sealed legacy upload migration registry does not match authoritative state")]
    LegacyUploadMigrationRegistryMismatch,
    #[error("JSON checkpoint sealed registry does not match authoritative state")]
    JsonCheckpointRegistryMismatch,
    #[error("sealed legacy upload migration registry JSON is invalid")]
    DecodeLegacyUploadMigrationRegistry(serde_json::Error),
    ...
    #[error("state changed during legacy upload evidence validation")]
    LegacyUploadApplyStateChanged,
    ...
    #[error("failed to copy immutable state database snapshot to {path}: {source}")]
    ImmutableSnapshotCopy { path: PathBuf, source: io::Error },
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn large_json_checkpoint_load_uses_buffered_reads_and_preserves_manifest() {
        let tempdir = tempfile::tempdir().expect("tempdir");
        let manifest_path = tempdir.path().join("manifest.json");
        ...
        for index in 0..4_096 {
            let mut record = AssetRecord::new(
                ...
                format!("/nas/asset-{index:05}.dng"),
            );
            record.proofs.insert(
                "checkpoint-padding".to_string(),
                serde_json::json!({"value": padding}),
            );
            manifest.upsert(record);
        }
        manifest
            .save_atomic(&manifest_path)
            .expect("save large checkpoint");

        let loaded = load_json_checkpoint_no_follow(&manifest_path)
            .expect("load large checkpoint")
            .expect("large checkpoint should be valid");
        assert_eq!(loaded, manifest);

        let bytes = fs::read(&manifest_path).expect("read checkpoint bytes");
        let mut reader = CountingReader::new(bytes.clone());
        let buffered = load_json_checkpoint_manifest_from_reader(&mut reader)
            .expect("load buffered checkpoint")
            .expect("buffered checkpoint should be valid");
        assert_eq!(buffered, manifest);
        let maximum_buffer_fills = bytes.len().div_ceil(JSON_CHECKPOINT_READ_BUFFER_BYTES) + 1;
        assert!(
            reader.read_calls <= maximum_buffer_fills,
            "buffered checkpoint parser used {} underlying reads for {} bytes (expected at most {})",
            reader.read_calls,
            bytes.len(),
            maximum_buffer_fills
        );
    }
    ...
    #[cfg(unix)]
    #[test]
    fn json_checkpoint_status_rejects_checkpoint_replacement_during_read() {
        let (_tempdir, manifest_path, reader) = checkpoint_test_store();
        let replacement_path = manifest_path.with_file_name("replacement.json");
        let held_file_alias = manifest_path.with_file_name("held-checkpoint-alias.json");
        let mut replacement = Manifest::new();
        replacement.upsert(AssetRecord::new("asset", "/nas/asset.dng"));
        replacement
            .save_atomic(&replacement_path)
            .expect("save replacement checkpoint");
        let checkpoint_path = manifest_path.clone();
        let alias_path = held_file_alias;
        set_json_checkpoint_read_hook_for_current_thread(move || {
            fs::hard_link(&checkpoint_path, &alias_path).expect("alias held checkpoint");
            fs::rename(&replacement_path, &checkpoint_path).expect("replace checkpoint");
        });

        assert!(matches!(
            reader.json_checkpoint_status(),
            Err(AssetStateStoreError::JsonCheckpointChanged { .. })
        ));
    }
    ...
    #[test]
    fn writer_snapshot_isolated_from_later_commits_and_rejects_pending_external_wal() {
        let tempdir = tempfile::tempdir().expect("tempdir");
        let manifest_path = tempdir.path().join("manifest.json");
        let writer = AssetStateStore::open_writer(
            &manifest_path,
        ...
        writer
            ...
            .expect("empty manifest should import");
        writer
            .persist_record_trusted(&AssetRecord::new("before", "/nas/before.dng"))
            .expect("initial record should persist");
        let snapshot = writer
            .create_immutable_read_snapshot()
            .expect("immutable copy should be created");
        let snapshot_root = snapshot
            .manifest_path()
            .parent()
            .expect("snapshot should have a parent")
            .to_path_buf();
        let snapshot_store = AssetStateStore::open_immutable_read_only(snapshot.manifest_path())
            .expect("immutable copy should open");
        assert!(
            snapshot_store
                .load()
        ...
        writer
            .persist_record_trusted(&AssetRecord::new("after", "/nas/after.dng"))
            .expect("later record should persist");
        let snapshot_manifest = snapshot_store
            .load()
            .expect("snapshot should remain readable");
        assert!(snapshot_manifest.get("before").is_ok());
        assert!(snapshot_manifest.get("after").is_err());

        let connection = Connection::open(AssetStateStore::db_path_for_manifest(&manifest_path))
            .expect("external connection should open");
        connection
            .pragma_update(None, "wal_autocheckpoint", 0)
            .expect("automatic checkpointing should disable");
        let external = AssetRecord::new("external", "/nas/external.dng");
        let external_json = encode_record(&external).expect("external record should encode");
        connection
            .execute(
                "INSERT INTO assets (asset_id, state, updated_at, record_json) VALUES (?1, ?2, ?3, ?4)",
                params![
            ...
            .expect("external mutation should commit to WAL");
        assert!(matches!(
            writer.create_immutable_read_snapshot(),
            Err(AssetStateStoreError::ReadOnlyWalPending)
        ));
    ...
    #[test]
    fn sealed_read_and_export_capabilities_reject_record_manifest_and_path_substitution() {
        let record_a = AssetRecord::new("asset-a", "/nas/asset-a.dng");
        let record_b = AssetRecord::new("asset-b", "/nas/asset-b.dng");
        let record_authority =
            StateStoreManifestRecordAuthority::for_record(&record_a).expect("record authority");
        let mut loaded = Manifest::new();
        assert!(matches!(
            loaded.upsert_state_store_record(&record_authority, record_b.clone()),
            Err(ManifestError::ReservedInternalProofCapabilityMismatch)
        ));
        assert!(loaded.records().is_empty());
        loaded
            .upsert_state_store_record(&record_authority, record_a.clone())
            .expect("exact record should be authorized");

        let temp = tempfile::tempdir().expect("tempdir");
        let destination = temp.path().join("manifest.json");
        let wrong_destination = temp.path().join("substituted.json");
        let export_authority =
            StateStoreManifestExportAuthority::for_manifest(&loaded, &destination)
                .expect("export authority");
        assert!(matches!(
            loaded.save_atomic_with_state_store_export_authority(
                &export_authority,
                &wrong_destination,
            ),
            Err(ManifestError::ReservedInternalProofCapabilityMismatch)
        ...
        let mut substituted = loaded.clone();
        substituted.upsert_trusted(record_b);
        assert!(matches!(
            substituted
                .save_atomic_with_state_store_export_authority(&export_authority, &destination),
            Err(ManifestError::ReservedInternalProofCapabilityMismatch)
        ));
        assert!(!destination.exists());
        ...
        loaded
            .save_atomic_with_state_store_export_authority(&export_authority, &destination)
            .expect("exact checkpoint should be authorized");
        assert_eq!(Manifest::load(&destination).expect("round trip"), loaded);
    }
}
