...
impl VerifiedUploadSource {
    ...
    pub(crate) fn held_file(&self) -> Result<File, UploadError> {
        let mut file = self
            .file
            .try_clone()
    ...
    pub(crate) fn sealed_path(&self) -> &Path {
        &self.path
    }
...
impl CloudKitLibraryDestination {
    ...
    pub(crate) fn zone_id_payload(&self) -> Value {
        let mut zone_id = Map::from_iter([("zoneName".to_string(), json!(self.zone_name))]);
        if let Some(owner_record_name) = &self.owner_record_name {
            zone_id.insert("ownerRecordName".to_string(), json!(owner_record_name));
        }
        Value::Object(zone_id)
    }
}
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CloudKitActiveAssetReadRequest {
    pub record_name: String,
    pub record_change_tag: String,
    pub database_scope: CloudKitDatabaseScope,
    pub zone_name: String,
...
pub enum CloudKitActiveAssetRemoteState {
    Active,
    ActiveUnmarked,
}
...
#[derive(Clone, Copy, Debug, Deserialize, Eq, Ord, PartialEq, PartialOrd, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum CloudKitActiveAssetLookupMode {
    FullFields,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct CloudKitActiveAssetValidation {
    pub remote_state: CloudKitActiveAssetRemoteState,
    pub lookup_mode: CloudKitActiveAssetLookupMode,
}

#[derive(Clone, Copy, Debug, Deserialize, Eq, Ord, PartialEq, PartialOrd, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum CloudKitUploadedHeicInitialState {
    Active,
    ActiveUnmarked,
    AlreadyDeleted,
}
...
#[derive(Clone, Copy, Debug, Deserialize, Eq, Ord, PartialEq, PartialOrd, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum CloudKitUploadedHeicInitialStateLookupMode {
    FilteredMarker,
    FullFields,
}
...
pub struct CloudKitUploadedHeicAsset {
    ...
    pub owner_record_name_sha256: String,
    pub initial_remote_state: CloudKitUploadedHeicInitialState,
    pub initial_state_lookup_mode: CloudKitUploadedHeicInitialStateLookupMode,
...
pub enum CloudKitOriginalAssetResolveDisposition {
    ...
    /// The uniquely resolved CloudKit asset carries both the exact original RAW resource and
    /// the exact HEIC replacement resource.  Both proofs are retained because neither resource
    /// alone is sufficient to describe the coexistence state.
    Coexistence {
        original_proof: OriginalAssetProof,
...
pub fn run_icloud_upload(
    ...
    run_icloud_upload_with_verified_source(
...
pub(crate) fn run_icloud_upload_with_verified_source(
    session_path: &Path,
    heic_source: &VerifiedUploadSource,
    destination: &CloudKitLibraryDestination,
) -> Result<IcloudUploadOutcome, UploadError> {
    let session = load_upload_session(session_path)?;
...
impl<T: PhotosUploadTransport> PhotosUploadClient<T> {
    ...
    pub fn upload_verified_source_to_library(
        ...
        let (single_file, streamed_heic_sha256, streamed_size_bytes) = self
            .transport
            .post_signed_upload(session, &upload_url, source.held_file()?, heic_size)?;
...
impl<T: CloudKitDeleteTransport> CloudKitDeleteClient<T> {
    ...
    pub fn delete_originals_batch(
        ...
        validate_session_destination(session, &cloudkit_delete_batch_destination(request)?)?;
        let response = self
            .transport
        ...
        F: FnOnce(&Value) -> Result<(), E>,
    {
        ...
        let destination = cloudkit_delete_batch_destination(request)
            .map_err(CloudKitDeleteBatchSendError::InvalidRequest)?;
        validate_session_destination(session, &destination)
            .map_err(CloudKitDeleteBatchSendError::InvalidRequest)?;
    ...
    pub fn lookup_delete_states(
        ...
        let destination = CloudKitLibraryDestination {
            ...
            owner_record_name: first.owner_record_name.clone(),
        };
        validate_session_destination(session, &destination)?;
    ...
    pub fn validate_active_asset_identity(
        &mut self,
        session: &CloudKitDeleteSession,
        request: &CloudKitActiveAssetReadRequest,
    ) -> Result<CloudKitActiveAssetValidation, UploadError> {
        validate_active_asset_read_request(request)?;
        let destination = CloudKitLibraryDestination {
            database_scope: request.database_scope,
            zone_name: request.zone_name.clone(),
            owner_record_name: request.owner_record_name.clone(),
        ...
        let response = self.transport.post_records_lookup(
            session,
            cloudkit_records_lookup_all_fields_payload(
                &[request.record_name.as_str()],
                &destination,
            ),
        )?;
        validate_active_asset_lookup_response(response, request)
    }
}
...
impl<T: CloudKitDeleteTransport + CloudKitOriginalAssetReadTransport> CloudKitDeleteClient<T> {
    pub fn resolve_uploaded_heic_asset(
        ...
        self.resolve_uploaded_heic_asset_with_mode(
            session,
            request,
            UploadedHeicInitialStateMode::RequireActive,
        )
    }

    pub fn inspect_uploaded_heic_asset_initial_state(
        &mut self,
        session: &CloudKitDeleteSession,
        request: &CloudKitUploadedHeicResolveRequest,
    ) -> Result<CloudKitUploadedHeicAsset, UploadError> {
        self.resolve_uploaded_heic_asset_with_mode(
            session,
            request,
            UploadedHeicInitialStateMode::ObserveExact,
        )
    }

    pub fn inspect_uploaded_heic_asset_initial_state_full_fields(
        &mut self,
        session: &CloudKitDeleteSession,
        request: &CloudKitUploadedHeicResolveRequest,
    ) -> Result<CloudKitUploadedHeicAsset, UploadError> {
        self.resolve_uploaded_heic_asset_with_mode(
            session,
            request,
            UploadedHeicInitialStateMode::ObserveExactFullFields,
        )
    }

    fn resolve_uploaded_heic_asset_with_mode(
        &mut self,
        session: &CloudKitDeleteSession,
        request: &CloudKitUploadedHeicResolveRequest,
        mode: UploadedHeicInitialStateMode,
    ) -> Result<CloudKitUploadedHeicAsset, UploadError> {
        ...
        let asset_payload = match mode {
            ...
            UploadedHeicInitialStateMode::RequireActive
            | UploadedHeicInitialStateMode::ObserveExact => cloudkit_records_lookup_payload(
                &[request.uploaded_asset_id.as_str()],
                &["masterRef", "isDeleted"],
                &destination,
            ),
        };
        let asset_response = self.transport.post_records_lookup(session, asset_payload)?;
        let asset = parse_uploaded_heic_asset_lookup_response(asset_response, request, mode)?;
        ...
        Ok(CloudKitUploadedHeicAsset {
            ...
            owner_record_name_sha256: format!(
                "{:x}",
                Sha256::digest(asset.owner_record_name.as_bytes())
            ),
            initial_remote_state: asset.initial_remote_state,
            initial_state_lookup_mode: mode.lookup_mode(),
...
impl<T: CloudKitUploadedHeicReadTransport> CloudKitUploadedHeicReadClient<T> {
    pub fn resolve_uploaded_heic_asset(
        ...
        resolve_uploaded_heic_asset_with_transport(
            &mut self.transport,
            session,
            request,
            UploadedHeicInitialStateMode::RequireActive,
...
#[derive(Clone, Copy)]
enum UploadedHeicInitialStateMode {
    RequireActive,
    ObserveExact,
    ObserveExactFullFields,
}

impl UploadedHeicInitialStateMode {
    fn lookup_mode(self) -> CloudKitUploadedHeicInitialStateLookupMode {
        match self {
            Self::RequireActive | Self::ObserveExact => {
                CloudKitUploadedHeicInitialStateLookupMode::FilteredMarker
            }
            Self::ObserveExactFullFields => CloudKitUploadedHeicInitialStateLookupMode::FullFields,
        }
    }
}
...
fn resolve_uploaded_heic_asset_with_transport<T: CloudKitUploadedHeicReadTransport>(
    ...
    let asset_response = transport.post_records_lookup(session, asset_payload)?;
    let asset = parse_uploaded_heic_asset_lookup_response(asset_response, request, mode)?;
...
fn build_batch_resolve_outcome(
    ...
    let resolutions = request
        ...
        .map(|(target_position, target)| {
            ...
            let unique_raw_match = raw_matches.and_then(|matches| {
                (matches.len() == 1).then(|| matches.values().next().expect("length is one"))
            });
            let unique_replacement_match = replacement_matches.and_then(|matches| {
                (matches.len() == 1).then(|| matches.values().next().expect("length is one"))
            });
            ...
            } else if let (Some(raw_candidate), Some(replacement_candidate)) =
                (unique_raw_match, unique_replacement_match)
            {
                ...
                CloudKitOriginalAssetResolveDisposition::Coexistence {
                    original_proof: OriginalAssetProof {
                        record_name: raw_candidate.asset.record_name.clone(),
                        record_change_tag: raw_candidate.asset.record_change_tag.clone(),
                    ...
                    replacement_proof: CloudKitReplacementResourceProof {
                        record_name: replacement_candidate.asset.record_name.clone(),
                        record_change_tag: replacement_candidate.asset.record_change_tag.clone(),
                        record_type: "CPLAsset".to_string(),
                        database_scope: destination.database_scope,
                        zone_name: destination.zone_name.clone(),
                        owner_record_name: destination.owner_record_name.clone(),
                        resource_field: replacement_candidate.resource_field.clone(),
                        size_bytes: replacement.size_bytes,
                        matched_heic_sha256: replacement.sha256.clone(),
                    },
                }
            } else if let Some(candidate) = unique_raw_match {
                CloudKitOriginalAssetResolveDisposition::ExactOriginal {
                    proof: OriginalAssetProof {
            ...
            } else if let Some(candidate) = unique_replacement_match {
...
fn photosupload_owner_record_name(destination: &CloudKitLibraryDestination) -> Option<&str> {
...
fn create_upload_url_payload(size_bytes: u64, destination: &CloudKitLibraryDestination) -> Value {
    ...
    if let Some(owner_record_name) = photosupload_owner_record_name(destination) {
        payload["ownerRecordName"] = json!(owner_record_name);
    }
    payload
}
...
fn cloudkit_delete_batch_destination(
    request: &CloudKitDeleteBatchRequest,
) -> Result<CloudKitLibraryDestination, UploadError> {
    request
        .requests
        .first()
...
fn cloudkit_delete_batch_payload(request: &CloudKitDeleteBatchRequest) -> Value {
    let destination = cloudkit_delete_batch_destination(request)
        .expect("delete batch payload requires a validated non-empty request");
...
fn cloudkit_records_lookup_all_fields_payload(
    record_names: &[&str],
    destination: &CloudKitLibraryDestination,
) -> Value {
    let records: Vec<Value> = record_names
...
fn parse_cloudkit_delete_state_lookup_response(
    ...
    for request in &request.requests {
        ...
        if !cloudkit_delete_record_zone_matches(record, request) {
            return Err(UploadError::InvalidCloudKitDeleteLookupResponse(
                "records/lookup response zone differs from the requested destination",
            ));
        }
        ...
        if !is_deleted {
            if record_change_tag != request.record_change_tag {
                return Err(UploadError::InvalidCloudKitDeleteLookupResponse(
                    "records/lookup found an un-deleted record with a changed recordChangeTag",
                ));
            }
...
fn parse_cloudkit_delete_record(
    ...
    if !cloudkit_delete_record_zone_matches(record, request) {
        return Err(UploadError::InvalidCloudKitDeleteResponse(
            "records/modify response zone differs from the requested destination",
        ));
    }
    let record_name = record.get("recordName").and_then(Value::as_str).ok_or(
...
fn cloudkit_delete_record_zone_matches(record: &Value, request: &CloudKitDeleteRequest) -> bool {
    let Some(zone) = record.get("zoneID") else {
        return request.database_scope == CloudKitDatabaseScope::Private
            && request.zone_name == PRIMARY_SYNC_ZONE;
    };
    let Some(zone) = zone.as_object() else {
        return false;
    };
    zone.get("zoneName").and_then(Value::as_str) == Some(request.zone_name.as_str())
        && zone.get("ownerRecordName").and_then(Value::as_str)
            == request.owner_record_name.as_deref()
}
...
fn parse_original_asset_query_response(
    ...
    for record in records {
        ...
        match record_type {
            "CPLAsset" => assets.push(parse_cloudkit_asset_record(record, destination)?),
            "CPLMaster" => {
                validate_original_asset_zone_identity(
                    record.get("zoneID"),
                    destination,
                    "CPLMaster zone differs from the requested destination",
                )?;
...
fn parse_uploaded_heic_asset_lookup_response(
    ...
    let initial_remote_state = match mode {
        UploadedHeicInitialStateMode::RequireActive => {
            match field_value(fields, "isDeleted") {
                Some(value) if value.as_bool() == Some(false) || value.as_i64() == Some(0) => {}
                Some(value) if cloudkit_truthy(value) => {
                    return Err(UploadError::InvalidCloudKitUploadedHeicResponse(
                ...
                _ => {
                    return Err(UploadError::InvalidCloudKitUploadedHeicResponse(
                        "uploaded HEIC active delete state is malformed",
                    ));
                }
            }
            CloudKitUploadedHeicInitialState::Active
        }
        UploadedHeicInitialStateMode::ObserveExact => match field_value(fields, "isDeleted") {
            Some(value) if value.as_bool() == Some(false) || value.as_i64() == Some(0) => {
                CloudKitUploadedHeicInitialState::Active
            }
            Some(value) if value.as_bool() == Some(true) || value.as_i64() == Some(1) => {
                CloudKitUploadedHeicInitialState::AlreadyDeleted
            }
            _ => {
                return Err(UploadError::InvalidCloudKitUploadedHeicResponse(
                    "uploaded HEIC initial delete state is malformed",
                ));
            }
        },
        UploadedHeicInitialStateMode::ObserveExactFullFields => {
            if record.get("deleted").and_then(Value::as_bool) != Some(false) {
                return Err(UploadError::InvalidCloudKitUploadedHeicResponse(
                    "uploaded HEIC full-fields lookup top-level delete state is malformed",
                ));
            }
            match fields.get("isDeleted") {
                None => CloudKitUploadedHeicInitialState::ActiveUnmarked,
                Some(_) => match field_value(fields, "isDeleted") {
                    Some(value) if value.as_bool() == Some(false) || value.as_i64() == Some(0) => {
                        CloudKitUploadedHeicInitialState::Active
                    }
                    Some(value) if value.as_bool() == Some(true) || value.as_i64() == Some(1) => {
                    ...
                    _ => {
                        return Err(UploadError::InvalidCloudKitUploadedHeicResponse(
                            "uploaded HEIC full-fields marker state is malformed",
                        ));
                    }
                },
            }
    ...
    let (master_record_name, owner_record_name) = parse_uploaded_heic_master_ref(fields, request)?;
    validate_uploaded_heic_record_destination(record, request, &owner_record_name)?;
...
fn validate_active_asset_lookup_response(
    value: Value,
    request: &CloudKitActiveAssetReadRequest,
) -> Result<CloudKitActiveAssetValidation, UploadError> {
    let records = cloudkit_lookup_records(&value)?;
    if records.len() != 1 {
        return Err(UploadError::InvalidCloudKitUploadedHeicResponse(
            "active asset lookup must return exactly one record",
        ));
    }
    let record = &records[0];
    reject_cloudkit_record_error(record, "records/lookup")?;
    require_record_type(record, "CPLAsset")?;
    if required_record_string(record, "recordName")? != request.record_name
        || required_record_string(record, "recordChangeTag")? != request.record_change_tag
    {
        return Err(UploadError::InvalidCloudKitUploadedHeicResponse(
            "active asset identity changed",
        ));
    }
    let destination_request = CloudKitDeleteRequest {
        record_name: request.record_name.clone(),
        record_change_tag: request.record_change_tag.clone(),
        database_scope: request.database_scope,
        zone_name: request.zone_name.clone(),
        owner_record_name: request.owner_record_name.clone(),
    };
    if !cloudkit_delete_record_zone_matches(record, &destination_request) {
        return Err(UploadError::InvalidCloudKitUploadedHeicResponse(
            "active asset destination changed",
        ));
    }
    if record.get("deleted").and_then(Value::as_bool) != Some(false) {
        return Err(UploadError::InvalidCloudKitUploadedHeicResponse(
            "active asset full-fields lookup top-level delete state is malformed",
        ));
    }
    let fields = record_fields(record)?;
    let remote_state = match fields.get("isDeleted") {
        None => CloudKitActiveAssetRemoteState::ActiveUnmarked,
        Some(_) => match field_value(fields, "isDeleted") {
            Some(value) if value.as_bool() == Some(false) || value.as_i64() == Some(0) => {
            ...
            _ => {
                return Err(UploadError::InvalidCloudKitUploadedHeicResponse(
                    "active asset full-fields marker state is malformed",
                ));
            }
        },
    };
    Ok(CloudKitActiveAssetValidation {
        remote_state,
...
fn validate_uploaded_heic_record_destination(
    ...
    validate_uploaded_heic_record_zone_identity(
        record.get("zoneID"),
        request,
        owner_record_name,
...
fn parse_uploaded_heic_master_ref(
    ...
    let owner_record_name = uploaded_heic_reference_owner_record_name(
        master_ref.get("zoneID"),
        request,
        "uploaded HEIC masterRef zone differs from the requested destination",
    )?;
    let record_name = master_ref
        .get("recordName")
        .and_then(Value::as_str)
    ...
    Ok((record_name, owner_record_name))
}

fn validate_uploaded_heic_record_zone_identity(
    zone: Option<&Value>,
    request: &CloudKitUploadedHeicResolveRequest,
    owner_record_name: &str,
    error: &'static str,
    ...
    match zone {
        None => Ok(()),
        Some(Value::Object(zone)) if zone.is_empty() => Ok(()),
        _ => validate_uploaded_heic_zone_identity(zone, request, owner_record_name, error),
    }
}

fn uploaded_heic_reference_owner_record_name(
    zone: Option<&Value>,
    request: &CloudKitUploadedHeicResolveRequest,
    error: &'static str,
    ...
    let owner_record_name = zone
        .get("ownerRecordName")
        .and_then(Value::as_str)
        .filter(|owner| !owner.trim().is_empty())
        .ok_or(UploadError::InvalidCloudKitUploadedHeicResponse(error))?;
    if let Some(expected) = request.owner_record_name.as_deref() {
        if owner_record_name != expected {
            return Err(UploadError::InvalidCloudKitUploadedHeicResponse(error));
        }
    } else if request.database_scope != CloudKitDatabaseScope::Private
        || request.zone_name != PRIMARY_SYNC_ZONE
    {
        return Err(UploadError::InvalidCloudKitUploadedHeicResponse(error));
    }
    Ok(owner_record_name.to_string())
}
...
fn validate_uploaded_heic_zone_identity(
    ...
    let zone = zone
        ...
        .ok_or(UploadError::InvalidCloudKitUploadedHeicResponse(error))?;
    if zone.get("zoneName").and_then(Value::as_str) != Some(request.zone_name.as_str())
        || zone.get("ownerRecordName").and_then(Value::as_str) != Some(owner_record_name)
    {
        return Err(UploadError::InvalidCloudKitUploadedHeicResponse(error));
    }
    Ok(())
...
fn parse_cloudkit_asset_record(
    record: &Value,
    destination: &CloudKitLibraryDestination,
) -> Result<CloudKitAssetRecord, UploadError> {
    validate_original_asset_zone_identity(
        record.get("zoneID"),
        destination,
        "CPLAsset zone differs from the requested destination",
    )?;
    let record_name = required_record_string(record, "recordName")?;
    let record_change_tag = required_record_string(record, "recordChangeTag")?;
    ...
    let fields = record_fields(record)?;
    let master_ref = field_value(fields, "masterRef")
        .and_then(Value::as_object)
        .ok_or(UploadError::InvalidCloudKitOriginalAssetResponse(
            "CPLAsset masterRef is malformed",
        ))?;
    validate_original_asset_zone_identity(
        master_ref.get("zoneID"),
        destination,
        "CPLAsset masterRef zone differs from the requested destination",
    )?;
    let master_record_name = master_ref.get("recordName").and_then(Value::as_str).ok_or(
        UploadError::InvalidCloudKitOriginalAssetResponse("CPLAsset missing masterRef"),
    )?;
...
fn validate_original_asset_zone_identity(
    zone: Option<&Value>,
    destination: &CloudKitLibraryDestination,
    error: &'static str,
) -> Result<(), UploadError> {
    let zone = zone
        .and_then(Value::as_object)
        .ok_or(UploadError::InvalidCloudKitOriginalAssetResponse(error))?;
    if zone.get("zoneName").and_then(Value::as_str) != Some(destination.zone_name.as_str())
        || zone.get("ownerRecordName").and_then(Value::as_str)
            != destination.owner_record_name.as_deref()
    {
        return Err(UploadError::InvalidCloudKitOriginalAssetResponse(error));
    }
    Ok(())
}
...
pub fn build_upload_proof(
    ...
    Ok(UploadProof {
        ...
        owner_record_name: upload.response.owner_record_name.clone(),
...
impl RawCloudKitDeleteSession {
    fn validate(self) -> Result<CloudKitDeleteSession, UploadError> {
        ...
        let zone_id = self.zone_id.unwrap_or_default();
        let zone_name = resolve_session_zone_component(
            self.zone_name,
            zone_id.zone_name,
            "CloudKit zone name",
        )?
        .unwrap_or_else(primary_sync_zone_name);
        let owner_record_name = resolve_session_zone_component(
            self.owner_record_name,
            zone_id.owner_record_name,
...
fn resolve_session_zone_component(
    direct: Option<String>,
    nested: Option<String>,
    field: &'static str,
) -> Result<Option<String>, UploadError> {
    match (direct, nested) {
        (Some(direct), Some(nested)) if direct != nested => Err(UploadError::InvalidSession(
            format!("conflicting {field} values"),
        )),
...
pub(crate) fn validate_library_destination(
    ...
    if let Some(owner_record_name) = &destination.owner_record_name {
        if owner_record_name.trim().is_empty() {
            return Err(UploadError::InvalidSession(
                "CloudKit zone owner record name cannot be empty".to_string(),
            ));
        }
        reject_control_chars(owner_record_name, "CloudKit zone owner record name")?;
        if owner_record_name.trim() != owner_record_name {
            return Err(UploadError::InvalidSession(
                "CloudKit zone owner record name must not include leading or trailing whitespace"
                    .to_string(),
            ));
        }
    }
    if destination.database_scope == CloudKitDatabaseScope::Shared
        && destination.owner_record_name.is_none()
    {
        return Err(UploadError::InvalidSession(
            "shared CloudKit library zone requires ownerRecordName".to_string(),
        ));
    }
    if destination.database_scope == CloudKitDatabaseScope::Shared
...
fn validate_session_destination(
    session: &CloudKitDeleteSession,
    destination: &CloudKitLibraryDestination,
) -> Result<(), UploadError> {
    validate_library_destination(destination)?;
    if session.database_scope != destination.database_scope || session.zone != *destination {
        return Err(UploadError::InvalidSession(
            "CloudKit request destination must match the authenticated session destination"
                .to_string(),
        ));
    }
    Ok(())
...
fn validate_cloudkit_delete_batch_request(
    ...
    let first_destination = request.requests.first().map(|delete_request| {
        (
            ...
            delete_request.owner_record_name.as_deref(),
        )
    });
...
fn validate_active_asset_read_request(
    request: &CloudKitActiveAssetReadRequest,
) -> Result<(), UploadError> {
    if request.record_name.trim().is_empty() || request.record_change_tag.trim().is_empty() {
        return Err(UploadError::InvalidCloudKitUploadedHeicRequest(
            "active asset identity is required",
        ));
    }
    reject_cloudkit_identity_chars(&request.record_name, "active asset record name")?;
    reject_cloudkit_identity_chars(&request.record_change_tag, "active asset record change tag")?;
    validate_library_destination(&CloudKitLibraryDestination {
        database_scope: request.database_scope,
        zone_name: request.zone_name.clone(),
        owner_record_name: request.owner_record_name.clone(),
...
mod tests {
    ...
    #[test]
    fn delete_lookup_never_authorizes_resend_after_live_change_tag_drift() {
        let request = CloudKitDeleteBatchRequest {
            requests: vec![CloudKitDeleteRequest {
                record_name: "retired-replacement".to_string(),
                record_change_tag: "sealed-old-tag".to_string(),
                database_scope: CloudKitDatabaseScope::Private,
                zone_name: PRIMARY_SYNC_ZONE.to_string(),
                owner_record_name: None,
            }],
        };
        let error = parse_cloudkit_delete_state_lookup_response(
            json!({
                "records": [{
                    "recordName": "retired-replacement",
                    "recordType": "CPLAsset",
                    "recordChangeTag": "new-unapproved-tag",
                    "fields": {"isDeleted": {"value": false}}
                }]
            }),
            &request,
        )
        .expect_err("an un-deleted record with drifted identity must not authorize a resend");
        assert!(matches!(
            error,
