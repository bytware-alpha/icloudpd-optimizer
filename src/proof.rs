...
/// Filesystem metadata used to detect RAW path drift without rehashing file bytes.
///
/// This is captured from the same path that passed NAS proof hashing and post-hash
/// stability checks, then compared just before CloudKit deletion.
///
/// ```no_run
/// # use icloudpd_optimizer::proof::RawFileFingerprint;
/// let fingerprint = RawFileFingerprint::capture("/nas/photos/IMG_0001.dng")?;
/// assert!(fingerprint.len > 0);
/// # Ok::<(), Box<dyn std::error::Error>>(())
/// ```
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct RawFileFingerprint {
    pub canonical_path: PathBuf,
    pub len: u64,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct NasRawProofWithFingerprint {
    pub proof: NasRawProof,
    pub raw_fingerprint: RawFileFingerprint,
}
...
pub fn prove_nas_raw(
    ...
    Ok(prove_nas_raw_with_after_hash_hook(nas_root, raw_path, min_age_days, now, || {})?.proof)
}

pub(crate) fn prove_nas_raw_with_min_age_seconds_and_fingerprint(
    nas_root: impl AsRef<Path>,
    raw_path: impl AsRef<Path>,
    min_age_seconds: u64,
    now: SystemTime,
) -> Result<NasRawProofWithFingerprint, ProofError> {
...
fn prove_nas_raw_with_min_age_seconds_after_hash_hook(
    ...
    let fingerprint_before = RawFileFingerprint::from_metadata(&canonical_raw, &metadata_before)?;
    ...
    let modified_at = metadata_before
        .modified()
        .map_err(|source| ProofError::ReadMetadata {
            path: canonical_raw.clone(),
            source,
    ...
    let fingerprint_after = RawFileFingerprint::from_metadata(&canonical_raw, &metadata_after)?;
    ...
    let final_path_fingerprint =
        RawFileFingerprint::from_metadata(&final_canonical_raw, &final_path_metadata)?;
    ...
    Ok(NasRawProofWithFingerprint {
        ...
        raw_fingerprint: final_path_fingerprint,
    })
}
...
impl RawFileFingerprint {
    /// Captures the canonical path and metadata fingerprint for a live RAW path.
    pub fn capture(path: impl AsRef<Path>) -> Result<Self, ProofError> {
        let requested = path.as_ref();
        let canonical_path =
            fs::canonicalize(requested).map_err(|source| ProofError::CanonicalizeRaw {
                path: requested.to_path_buf(),
                source,
            })?;
        let metadata =
            fs::metadata(&canonical_path).map_err(|source| ProofError::ReadMetadata {
        ...
        Self::from_metadata(&canonical_path, &metadata)
    }
    ...
    fn from_metadata(path: &Path, metadata: &Metadata) -> Result<Self, ProofError> {
        Ok(Self {
            ...
            #[cfg(unix)]
            modified_unix_nanos: metadata.mtime_nsec(),
            #[cfg(not(unix))]
