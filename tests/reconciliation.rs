...
use icloudpd_optimizer::reconciliation::{
    OriginalAssetResolutionBatch, OriginalAssetResolutionDisposition, OriginalAssetResolutionProof,
};
...
fn coexistence_resolution() -> CloudKitOriginalAssetResolution {
    CloudKitOriginalAssetResolution {
        observations: CloudKitOriginalAssetResolveObservations {
            date_candidates: 1,
            raw_resources: 1,
        ...
        disposition: CloudKitOriginalAssetResolveDisposition::Coexistence {
            original_proof: OriginalAssetProof {
                record_name: "CPLAsset-coexistence".to_string(),
                record_change_tag: "coexistence-tag".to_string(),
                record_type: "CPLAsset".to_string(),
                database_scope: Default::default(),
                zone_name: "PrimarySync".to_string(),
                owner_record_name: None,
            ...
            replacement_proof: CloudKitReplacementResourceProof {
                ...
                zone_name: "PrimarySync".to_string(),
                owner_record_name: None,
                resource_field: "resOriginalAltRes".to_string(),
                size_bytes: 24,
                matched_heic_sha256: HEIC_SHA256.to_string(),
            },
        },
...
#[test]
fn coexistence_resolution_records_both_proofs_and_never_enables_deletion() {
    let mut manifest = Manifest::new();
    manifest.upsert(failed_resolver_record("asset-coexistence"));

    let result = manifest
        .apply_original_asset_resolution_batch(batch("asset-coexistence", coexistence_resolution()))
        .expect("validated coexistence resolution should apply");

    assert_eq!(result.summary.no_action, 1);
    let record = manifest.get("asset-coexistence").unwrap();
    assert_eq!(record.state, State::NoAction);
    assert!(!record.proofs.contains_key("original_asset"));
    let proof: OriginalAssetResolutionProof =
        serde_json::from_value(record.proofs["original_asset_resolution"].clone()).unwrap();
    let OriginalAssetResolutionDisposition::Coexistence {
        original_proof,
        replacement_proof,
    } = proof.disposition
    else {
        panic!("coexistence proof must preserve both remote proofs");
    };
    assert_eq!(original_proof.record_name, "CPLAsset-coexistence");
    assert_eq!(replacement_proof.resource_field, "resOriginalAltRes");
    assert!(mark_delete_eligible(&mut manifest, "asset-coexistence").is_err());
}

#[test]
fn coexistence_resolution_rejects_invalid_original_proof_binding() {
    let mut manifest = Manifest::new();
    manifest.upsert(failed_resolver_record("asset-coexistence-invalid"));
    let mut resolution = coexistence_resolution();
    if let CloudKitOriginalAssetResolveDisposition::Coexistence { original_proof, .. } =
        &mut resolution.disposition
    {
        original_proof.matched_raw_sha256 = "c".repeat(64);
    }

    let error = manifest
        .apply_original_asset_resolution_batch(batch("asset-coexistence-invalid", resolution))
        .expect_err("coexistence proof must bind the exact RAW target");
    assert!(matches!(
        error,
        icloudpd_optimizer::reconciliation::OriginalAssetResolutionError::InvalidResolutionProof { .. }
    ));
}
...
fn remote_record_identity_is_scoped_to_database_and_zone() {
    ...
    proof
        .owner_record_name
        .clone_from(&destination.owner_record_name);
