...
use icloudpd_optimizer::manifest::{
    AssetRecord, FailureKind, FailureRecord, Manifest, ManifestError, State,
};
use icloudpd_optimizer::proof::{NasRawProof, ProofError, prove_nas_raw};
use icloudpd_optimizer::upload::{
    CloudKitDeleteOutcome, CloudKitUploadedHeicAsset, CloudKitUploadedHeicInitialState,
};
...
fn uploaded_heic_asset() -> CloudKitUploadedHeicAsset {
    CloudKitUploadedHeicAsset {
        ...
        owner_record_name_sha256: "a".repeat(64),
        initial_remote_state: CloudKitUploadedHeicInitialState::Active,
        initial_state_lookup_mode:
            icloudpd_optimizer::upload::CloudKitUploadedHeicInitialStateLookupMode::FilteredMarker,
...
#[test]
fn delete_eligibility_rejects_shared_owner_proof_mismatch_without_mutation() {
    let (_tempdir, mut manifest, _) = real_upload_verified_manifest();
    let mut record = manifest.get("asset-1").expect("asset should exist").clone();
    for proof_key in ["original_asset", "upload"] {
        proof_mut(&mut record, proof_key)["database_scope"] = json!("shared");
        proof_mut(&mut record, proof_key)["zone_name"] = json!("SharedSync-family");
    }
    let original_owner = "_secret-original-owner";
    let uploaded_owner = "_secret-uploaded-owner";
    proof_mut(&mut record, "original_asset")["owner_record_name"] = json!(original_owner);
    proof_mut(&mut record, "upload")["owner_record_name"] = json!(uploaded_owner);
    manifest = replace_trusted_record(manifest, record);
    let before = manifest.get("asset-1").expect("asset should exist").clone();

    let error = mark_delete_eligible(&mut manifest, "asset-1")
        .expect_err("delete eligibility must bind the shared zone owner");

    assert!(matches!(
        error,
        WorkflowError::OwnerIdentityMismatch {
            proof_key: "upload",
            field: "owner_record_name",
            status: "mismatch",
        }
    ));
    let rendered = error.to_string();
    assert!(!rendered.contains(original_owner));
    assert!(!rendered.contains(uploaded_owner));
    assert!(rendered.contains("mismatch"));
    let persisted_log = serde_json::to_string(&FailureRecord::new("delete_eligibility", rendered))
        .expect("failure log should serialize");
    assert!(!persisted_log.contains(original_owner));
    assert!(!persisted_log.contains(uploaded_owner));
    assert_eq!(
        manifest.get("asset-1").expect("asset should exist"),
        &before
    );
