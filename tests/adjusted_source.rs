...
fn shared_session(owner: Option<&str>) -> CloudKitDeleteSession {
    ...
    session.zone = CloudKitLibraryDestination {
        ...
        owner_record_name: owner.map(ToString::to_string),
    };
    session
}
...
fn shared_request(
    ...
    let mut request = resolve_request(output_path);
    request.original_asset.database_scope = CloudKitDatabaseScope::Shared;
    request.original_asset.zone_name = "SharedSync-ABCDEF123456".to_string();
    request.original_asset.owner_record_name = owner.map(ToString::to_string);
    request
}

fn set_record_destination(record: &mut Value, zone_name: &str, owner: Option<&str>) {
...
#[test]
fn rejects_shared_adjusted_source_destination_without_owner_before_transport() {
    let bytes = nonblank_jpeg(4, 3);
    let mut record = direct_asset_record(&bytes, 4, 3);
    set_record_destination(&mut record, "SharedSync-ABCDEF123456", None);
    let mut transport = FakeTransport {
        lookups: VecDeque::from([json!({"records": [record]})]),
    ...
    let directory = tempdir().expect("test directory");
    let request = shared_request(safe_path(&directory, "adjusted.jpg"), None);

    let error = CloudKitAdjustedSourceResolver::new(&mut transport)
        .resolve(&shared_session(None), &request)
        .expect_err("shared adjusted-source lookup without owner must fail closed");

    assert!(matches!(error, AdjustedSourceError::InvalidRequest(_)));
    assert!(transport.lookup_payloads.is_empty());
    assert_eq!(transport.download_calls, 0);
}

#[test]
fn rejects_mismatched_shared_adjusted_source_owner_before_transport() {
    let mut transport = FakeTransport::default();
    let directory = tempdir().expect("test directory");
    let request = shared_request(safe_path(&directory, "adjusted.jpg"), Some("_wrong-owner"));

    let error = CloudKitAdjustedSourceResolver::new(&mut transport)
        .resolve(&shared_session(Some("_shared-owner")), &request)
        .expect_err("mismatched adjusted-source owner must fail closed");
...
#[test]
fn resolves_adjusted_source_with_exact_shared_owner() {
    let bytes = nonblank_jpeg(4, 3);
    let mut record = direct_asset_record(&bytes, 4, 3);
    set_record_destination(
        &mut record,
    ...
    let directory = tempdir().expect("test directory");
    let request = shared_request(safe_path(&directory, "adjusted.jpg"), Some("_shared-owner"));
    ...
    assert_eq!(
        transport.lookup_payloads[0]["zoneID"],
        json!({
