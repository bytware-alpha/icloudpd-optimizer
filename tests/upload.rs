...
use icloudpd_optimizer::upload::{
    ...
    CloudKitOriginalAssetResolveTarget, CloudKitResourceDownload, CloudKitUploadedHeicInitialState,
...
fn shared_delete_session_json() -> String {
    serde_json::json!({
        ...
        "zone_id": {
            "zoneName": "SharedSync-ABCDEF123456",
...
fn set_cloudkit_pair_destination(records: &mut Value, zone_name: &str, owner: Option<&str>) {
    let mut zone = json!({"zoneName": zone_name});
    if let Some(owner) = owner {
        zone["ownerRecordName"] = json!(owner);
    }
    records[0]["zoneID"] = zone.clone();
    records[0]["fields"]["masterRef"]["value"]["zoneID"] = zone.clone();
    records[1]["zoneID"] = zone;
}
...
#[test]
fn load_cloudkit_delete_session_rejects_shared_library_without_zone_owner() {
    ...
    let error = CloudKitDeleteSession::from_json(&body.to_string())
        .expect_err("shared library sessions must bind the zone owner");

    assert!(matches!(error, UploadError::InvalidSession(_)));
}

#[test]
fn cloudkit_destination_serde_preserves_private_legacy_and_shared_owner_identity() {
    let private: CloudKitLibraryDestination = serde_json::from_value(json!({
        "database_scope": "private",
        "zone_name": "PrimarySync"
    }))
    .expect("legacy private destination should deserialize");
    assert_eq!(private, CloudKitLibraryDestination::primary_sync());

    let shared = CloudKitLibraryDestination {
        database_scope: CloudKitDatabaseScope::Shared,
        zone_name: "SharedSync-ABCDEF123456".to_string(),
        owner_record_name: Some("_shared-owner".to_string()),
    };
    assert_eq!(
        serde_json::to_value(shared).expect("shared destination should serialize"),
        json!({
            "database_scope": "shared",
            "zone_name": "SharedSync-ABCDEF123456",
            "owner_record_name": "_shared-owner"
        })
    );
}
...
#[test]
fn cloudkit_delete_client_binds_shared_owner_and_rejects_missing_owner_before_transport() {
    let session = CloudKitDeleteSession::from_json(&shared_delete_session_json())
        .expect("shared session should load");
    let request = CloudKitDeleteRequest {
    ...
    let mut transport = FakeCloudKitDeleteTransport::modify_response(json!({
        "records": [{
            "recordName": "CPLAsset-original-123",
    ...
    CloudKitDeleteClient::new(&mut transport)
        .delete_original(&session, &request)
        .expect("matching shared owner should be sent");
    assert_eq!(
        transport.payloads[0]["zoneID"],
        json!({
            "zoneName": "SharedSync-ABCDEF123456",
            "ownerRecordName": "_shared-owner"
        })
    ...
    let mut missing_owner = request.clone();
    missing_owner.owner_record_name = None;
    let mut transport = FakeCloudKitDeleteTransport::success();
    let error = CloudKitDeleteClient::new(&mut transport)
        .delete_original(&session, &missing_owner)
        .expect_err("shared delete without an owner must fail closed");
    assert!(matches!(error, UploadError::InvalidSession(_)));
    assert!(transport.payloads.is_empty());

    let mut wrong_owner = missing_owner;
    wrong_owner.owner_record_name = Some("_wrong-owner".to_string());
    let mut transport = FakeCloudKitDeleteTransport::success();
    let error = CloudKitDeleteClient::new(&mut transport)
        .delete_original(&session, &wrong_owner)
        .expect_err("delete destination must match the authenticated session owner");
    assert!(matches!(error, UploadError::InvalidSession(_)));
    assert!(transport.payloads.is_empty());
    ...
    let error = CloudKitDeleteClient::new(&mut transport)
        .delete_original(&session, &request)
        .expect_err("delete response owner must match the request destination");
    assert!(matches!(
        error,
    ...
    for response in [
        ...
        json!({
            "records": [{
                "recordName": "CPLAsset-original-123",
                "recordChangeTag": "change-tag-2",
                "zoneID": {"zoneName": "SharedSync-ABCDEF123456"},
                "fields": {"isDeleted": {"value": 1}}
            }]
        }),
    ] {
        let mut transport = FakeCloudKitDeleteTransport::modify_response(response);
        let error = CloudKitDeleteClient::new(&mut transport)
            .delete_original(&session, &request)
            .expect_err("shared delete response must include its exact zone owner");
        assert!(matches!(
            error,
            UploadError::InvalidCloudKitDeleteResponse(_)
        ));
...
#[test]
fn cloudkit_shared_delete_state_lookup_requires_exact_response_zone_owner() {
    let session = CloudKitDeleteSession::from_json(&shared_delete_session_json())
        .expect("shared session should load");
    let delete = CloudKitDeleteRequest {
        record_name: "CPLAsset-original-123".to_string(),
        record_change_tag: "change-tag-1".to_string(),
        database_scope: CloudKitDatabaseScope::Shared,
        zone_name: "SharedSync-ABCDEF123456".to_string(),
    ...
    for record in cases {
        let mut transport = FakeCloudKitDeleteTransport::lookup_responses_with_downloads(
            vec![json!({"records": [record]})],
            vec![],
        );
        let error = CloudKitDeleteClient::new(&mut transport)
            .lookup_delete_states(&session, &request)
            .expect_err("shared reconciliation lookup must bind the exact zone owner");
        assert!(matches!(
            error,
...
#[test]
fn cloudkit_private_primary_sync_delete_responses_remain_compatible_without_zone_id() {
    let session = CloudKitDeleteSession::from_json(&valid_delete_session_json())
        .expect("session should load");
    let delete = primary_delete_request("CPLAsset-original-123", "change-tag-1");
    let mut modify_transport = FakeCloudKitDeleteTransport::success();
    CloudKitDeleteClient::new(&mut modify_transport)
        .delete_original(&session, &delete)
        .expect("private PrimarySync modify response may omit zoneID");

    let request = CloudKitDeleteBatchRequest {
        requests: vec![delete],
    };
    let mut lookup_transport = FakeCloudKitDeleteTransport::lookup_responses_with_downloads(
        vec![json!({
            "records": [{
                "recordName": "CPLAsset-original-123",
                "recordChangeTag": "change-tag-2",
    ...
    CloudKitDeleteClient::new(&mut lookup_transport)
        .lookup_delete_states(&session, &request)
        .expect("private PrimarySync lookup response may omit zoneID");
}

#[test]
fn cloudkit_private_owner_bound_delete_responses_may_omit_zone_id() {
    let mut session_json: Value =
        serde_json::from_str(&valid_delete_session_json()).expect("session JSON should parse");
    session_json["owner_record_name"] = json!("opaque-private-owner");
    let session = CloudKitDeleteSession::from_json(&session_json.to_string())
        .expect("owner-bound private session should load");
    let mut delete = primary_delete_request("CPLAsset-original-123", "change-tag-1");
    delete.owner_record_name = Some("opaque-private-owner".to_string());

    let mut modify_transport = FakeCloudKitDeleteTransport::success();
    CloudKitDeleteClient::new(&mut modify_transport)
        .delete_original(&session, &delete)
        .expect("owner-bound private PrimarySync modify response may omit zoneID");
    assert_eq!(
        modify_transport.payloads[0]["zoneID"],
        json!({
            "zoneName": "PrimarySync",
            "ownerRecordName": "opaque-private-owner"
        })
    ...
    CloudKitDeleteClient::new(&mut lookup_transport)
        .lookup_delete_states(&session, &request)
        .expect("owner-bound private PrimarySync lookup response may omit zoneID");
}
...
#[test]
fn uploaded_heic_read_client_recovers_exact_already_deleted_initial_state_distinctly() {
    let session =
        CloudKitDeleteSession::from_json(&valid_delete_session_json()).expect("session loads");
    let bytes = b"already-deleted-uploaded-heic";
    let mut deleted_asset =
        cloudkit_uploaded_heic_asset("CPLAsset-uploaded-heic-123", "CPLMaster-heic-123", "tag-1");
    deleted_asset["records"][0]["fields"]["isDeleted"]["value"] = json!(1);
    let master = cloudkit_uploaded_heic_master(
        "CPLMaster-heic-123",
        bytes.len() as u64,
        "https://p140-icloud-content.icloud.com/uploaded-heic",
    );

    let recovered = CloudKitUploadedHeicReadClient::new(ReadOnlyUploadedHeicTransport {
        lookup_responses: vec![deleted_asset, master],
        bytes: bytes.to_vec(),
    })
    .inspect_uploaded_heic_asset_initial_state(
        &session,
        &primary_uploaded_heic_resolve_request(
            "CPLAsset-uploaded-heic-123",
            sha256_hex(bytes),
    ...
    .expect("the explicit migration recovery inspector may seal an exact tombstone");

    assert_eq!(
        recovered.initial_remote_state,
        CloudKitUploadedHeicInitialState::AlreadyDeleted
    );
}

#[test]
fn uploaded_heic_recovery_inspector_rejects_missing_or_malformed_tombstone_state() {
    let session =
        CloudKitDeleteSession::from_json(&valid_delete_session_json()).expect("session loads");
    let bytes = b"already-deleted-uploaded-heic";
    let request = primary_uploaded_heic_resolve_request(
        "CPLAsset-uploaded-heic-123",
        sha256_hex(bytes),
        bytes.len() as u64,
    );
    ...
    missing["records"][0]["fields"]
        .as_object_mut()
        .expect("asset fields")
        .remove("isDeleted");
    let mut malformed = exact;
    malformed["records"][0]["fields"]["isDeleted"]["value"] = json!(2);

    for asset in [missing, malformed] {
        let mut transport = ObservingUploadedHeicReadTransport {
            lookup_responses: vec![asset],
            lookup_calls: 0,
            download_calls: 0,
        };
        assert!(
            CloudKitUploadedHeicReadClient::new(&mut transport)
                .inspect_uploaded_heic_asset_initial_state(&session, &request)
                .is_err()
        );
...
#[test]
fn filtered_uploaded_heic_resolver_rejects_omitted_marker() {
    let session =
        CloudKitDeleteSession::from_json(&valid_delete_session_json()).expect("session loads");
    let bytes = b"filtered-marker-must-be-explicit";
    let mut asset =
        cloudkit_uploaded_heic_asset("CPLAsset-uploaded-heic-123", "CPLMaster-heic-123", "tag-1");
    asset["records"][0]["fields"]
        .as_object_mut()
        .expect("asset fields")
        .remove("isDeleted");
    let master = cloudkit_uploaded_heic_master(
...
#[test]
fn full_fields_uploaded_heic_inspector_seals_active_unmarked_request_mode() {
    let session =
        CloudKitDeleteSession::from_json(&valid_delete_session_json()).expect("session loads");
    let bytes = b"active-unmarked-uploaded-heic";
    let mut asset =
        cloudkit_uploaded_heic_asset("CPLAsset-uploaded-heic-123", "CPLMaster-heic-123", "tag-1");
    asset["records"][0]["deleted"] = json!(false);
    asset["records"][0]["fields"]
        .as_object_mut()
        .expect("asset fields")
        .remove("isDeleted");
    ...
    let resolved = CloudKitDeleteClient::new(&mut transport)
        ...
        .expect("an unfiltered exact lookup may seal an unset marker as active-unmarked");
    ...
    assert_eq!(
        resolved.initial_state_lookup_mode,
        CloudKitUploadedHeicInitialStateLookupMode::FullFields
    );
    assert!(transport.lookup_payloads[0].get("desiredKeys").is_none());
}

#[test]
fn full_fields_uploaded_heic_inspector_requires_exact_top_level_and_marker_state() {
    let session =
        CloudKitDeleteSession::from_json(&valid_delete_session_json()).expect("session loads");
    let bytes = b"active-unmarked-uploaded-heic";
    let request = primary_uploaded_heic_resolve_request(
    ...
    let mut exact =
        cloudkit_uploaded_heic_asset("CPLAsset-uploaded-heic-123", "CPLMaster-heic-123", "tag-1");
    exact["records"][0]["deleted"] = json!(false);
    exact["records"][0]["fields"]
        .as_object_mut()
        .expect("asset fields")
        .remove("isDeleted");
    let mut missing_top_level = exact.clone();
    missing_top_level["records"][0]
        .as_object_mut()
        .expect("asset record")
        .remove("deleted");
    let mut deleted_top_level = exact.clone();
    deleted_top_level["records"][0]["deleted"] = json!(true);
    let mut malformed_top_level = exact.clone();
    malformed_top_level["records"][0]["deleted"] = json!(0);
    let mut malformed_marker = exact;
    malformed_marker["records"][0]["fields"]["isDeleted"] = json!({"value": null});
    ...
    for asset in [
        ...
        assert!(
            CloudKitUploadedHeicReadClient::new(&mut transport)
                .inspect_uploaded_heic_asset_initial_state_full_fields(&session, &request)
                .is_err()
        );
        assert_eq!(transport.lookup_calls, 1);
        assert_eq!(transport.download_calls, 0);
...
#[test]
fn full_fields_uploaded_heic_inspector_preserves_explicit_marker_states() {
    let session =
        CloudKitDeleteSession::from_json(&valid_delete_session_json()).expect("session loads");
    let bytes = b"full-fields-uploaded-heic";
    let request = primary_uploaded_heic_resolve_request(
        "CPLAsset-uploaded-heic-123",
        sha256_hex(bytes),
        bytes.len() as u64,
    ...
    for (marker, expected) in [
        (json!(false), CloudKitUploadedHeicInitialState::Active),
        (json!(0), CloudKitUploadedHeicInitialState::Active),
        (
            json!(true),
            CloudKitUploadedHeicInitialState::AlreadyDeleted,
        ),
        (json!(1), CloudKitUploadedHeicInitialState::AlreadyDeleted),
    ] {
        let mut asset = cloudkit_uploaded_heic_asset(
            "CPLAsset-uploaded-heic-123",
            "CPLMaster-heic-123",
            "tag-1",
        );
        asset["records"][0]["deleted"] = json!(false);
        asset["records"][0]["fields"]["isDeleted"]["value"] = marker;
        let master = cloudkit_uploaded_heic_master(
            "CPLMaster-heic-123",
            bytes.len() as u64,
            "https://p140-icloud-content.icloud.com/uploaded-heic",
        ...
        let resolved = CloudKitUploadedHeicReadClient::new(ReadOnlyUploadedHeicTransport {
            lookup_responses: vec![asset, master],
            bytes: bytes.to_vec(),
        })
        .inspect_uploaded_heic_asset_initial_state_full_fields(&session, &request)
        .expect("full-fields inspection must preserve explicit exact markers");
        assert_eq!(resolved.initial_remote_state, expected);
        assert_eq!(
            resolved.initial_state_lookup_mode,
            CloudKitUploadedHeicInitialStateLookupMode::FullFields
        );
...
#[test]
fn active_asset_identity_revalidation_uses_full_fields_and_accepts_explicit_active_unmarked() {
    let session =
        CloudKitDeleteSession::from_json(&valid_delete_session_json()).expect("session loads");
    let request = CloudKitActiveAssetReadRequest {
        record_name: "CPLAsset-original-123".to_string(),
    ...
    let active_unmarked = json!({"records": [{
        "recordName": "CPLAsset-original-123",
        "recordType": "CPLAsset",
        "recordChangeTag": "original-tag",
        "deleted": false,
        "fields": {}
    }]});
    let read_only = CloudKitUploadedHeicReadClient::new(ReadOnlyUploadedHeicTransport {
        lookup_responses: vec![active_unmarked.clone()],
        bytes: vec![],
    })
    .validate_active_asset_identity(&session, &request)
    .expect("exact active original identity must validate");
    assert_eq!(
        read_only.remote_state,
        CloudKitActiveAssetRemoteState::ActiveUnmarked
    );
    assert_eq!(
        read_only.lookup_mode,
        CloudKitActiveAssetLookupMode::FullFields
    );

    let mut transport =
        FakeCloudKitDeleteTransport::lookup_responses_with_downloads(vec![active_unmarked], vec![]);
    let delete_capable = CloudKitDeleteClient::new(&mut transport)
        .validate_active_asset_identity(&session, &request)
        .expect("delete-capable client must use the same exact read-only identity validation");
    assert_eq!(delete_capable, read_only);
    assert_eq!(transport.lookup_payloads.len(), 1);
    assert!(transport.lookup_payloads[0].get("desiredKeys").is_none());
    assert!(transport.payloads.is_empty());

    for marker in [json!(false), json!(0)] {
        let active = json!({"records": [{
            "recordName": "CPLAsset-original-123",
            "recordType": "CPLAsset",
            "recordChangeTag": "original-tag",
            "deleted": false,
            "fields": {"isDeleted": {"value": marker}}
        }]});
        let validation = CloudKitUploadedHeicReadClient::new(ReadOnlyUploadedHeicTransport {
            lookup_responses: vec![active],
            bytes: vec![],
        })
        .validate_active_asset_identity(&session, &request)
        .expect("explicit false or zero must remain an exact active state");
        assert_eq!(
...
#[test]
fn active_asset_identity_revalidation_rejects_deleted_malformed_error_or_drift() {
    let session =
        CloudKitDeleteSession::from_json(&valid_delete_session_json()).expect("session loads");
    let request = CloudKitActiveAssetReadRequest {
        record_name: "CPLAsset-original-123".to_string(),
    ...
    let mut top_level_deleted = active.clone();
    top_level_deleted["records"][0]["deleted"] = json!(true);
    let mut missing_top_level = active.clone();
    missing_top_level["records"][0]
        .as_object_mut()
        .unwrap()
        .remove("deleted");
    let mut malformed_top_level = active.clone();
    malformed_top_level["records"][0]["deleted"] = json!(0);
    let mut deleted_marker = active.clone();
    deleted_marker["records"][0]["fields"]["isDeleted"]["value"] = json!(1);
    let mut malformed_marker = active.clone();
    malformed_marker["records"][0]["fields"]["isDeleted"]["value"] = json!(null);
    let mut changed = active.clone();
    changed["records"][0]["recordChangeTag"] = json!("changed-tag");
    let mut record_error = active.clone();
    record_error["records"][0]["serverErrorCode"] = json!("UNKNOWN_ITEM");
    let mut wrong_destination = active;
    wrong_destination["records"][0]["zoneID"] =
        json!({"zoneName": "Wrong", "ownerRecordName": null});
    ...
    for response in [
        ...
        assert!(
            CloudKitUploadedHeicReadClient::new(ReadOnlyUploadedHeicTransport {
                lookup_responses: vec![response.clone()],
                bytes: vec![],
            })
            .validate_active_asset_identity(&session, &request)
            .is_err()
        ...
        assert!(
            CloudKitDeleteClient::new(
                FakeCloudKitDeleteTransport::lookup_responses_with_downloads(
                    vec![response],
                    vec![],
                ),
            )
...
#[test]
fn uploaded_heic_read_client_accepts_omitted_record_zones_with_exact_master_ref_zone() {
    let session =
        CloudKitDeleteSession::from_json(&valid_delete_session_json()).expect("session loads");
    let bytes = b"read-only-uploaded-heic";
    let mut asset =
    ...
    asset["records"][0]
        ...
        .expect("asset record")
        .remove("zoneID");
    let mut master = cloudkit_uploaded_heic_master(
        "CPLMaster-heic-123",
        bytes.len() as u64,
        "https://p140-icloud-content.icloud.com/uploaded-heic",
    );
    ...
    .expect("request-scoped records may omit record-level zones");
...
#[test]
fn uploaded_heic_read_client_binds_private_primary_sync_owner_enriched_by_master_ref() {
    let session =
        CloudKitDeleteSession::from_json(&valid_delete_session_json()).expect("session loads");
    let bytes = b"read-only-uploaded-heic";
    let mut asset =
    ...
    asset["records"][0]
        ...
        .expect("asset record")
        .remove("zoneID");
    asset["records"][0]["fields"]["masterRef"]["value"]["zoneID"]["ownerRecordName"] =
        json!("opaque-private-owner");
    let mut master = cloudkit_uploaded_heic_master(
        "CPLMaster-heic-123",
        bytes.len() as u64,
        "https://p140-icloud-content.icloud.com/uploaded-heic",
    ...
    .expect("private PrimarySync may enrich an omitted request owner from an exact masterRef");
    assert_eq!(
        resolved.owner_record_name_sha256,
        sha256_hex(b"opaque-private-owner")
    );
}

#[test]
fn uploaded_heic_read_client_rejects_incomplete_or_inconsistent_private_owner_witness() {
    let session =
        CloudKitDeleteSession::from_json(&valid_delete_session_json()).expect("session loads");
    let bytes = b"read-only-uploaded-heic";
    let asset_id = "CPLAsset-uploaded-heic-123";
    let master_id = "CPLMaster-heic-123";
    let asset = cloudkit_uploaded_heic_asset(asset_id, master_id, "tag-1");
    let master = cloudkit_uploaded_heic_master(
        master_id,
        bytes.len() as u64,
        "https://p140-icloud-content.icloud.com/uploaded-heic",
    ...
    let mut missing_owner = asset.clone();
    missing_owner["records"][0]["fields"]["masterRef"]["value"]["zoneID"]
        .as_object_mut()
        .expect("masterRef zone")
        .remove("ownerRecordName");
    let mut blank_owner = asset.clone();
    blank_owner["records"][0]["fields"]["masterRef"]["value"]["zoneID"]["ownerRecordName"] =
        json!(" ");
    let mut wrong_asset_zone = asset.clone();
    wrong_asset_zone["records"][0]["zoneID"] = json!({
        "zoneName": "PrimarySync",
        "ownerRecordName": "different-owner"
    });
    let mut wrong_master_zone = master.clone();
    wrong_master_zone["records"][0]["zoneID"] = json!({
        "zoneName": "PrimarySync",
        "ownerRecordName": "different-owner"
    });
    ...
    for responses in [
        vec![missing_owner],
        vec![blank_owner],
        vec![wrong_asset_zone],
        vec![asset.clone(), wrong_master_zone],
    ] {
        let mut transport = ObservingUploadedHeicReadTransport {
        ...
        let error = CloudKitUploadedHeicReadClient::new(&mut transport)
            ...
            .expect_err("private owner witness must remain complete and internally exact");
        assert!(matches!(
            error,
            UploadError::InvalidCloudKitUploadedHeicResponse(_)
        ));
...
#[test]
fn uploaded_heic_read_client_accepts_empty_record_zones_with_exact_master_ref_zone() {
    let session =
        CloudKitDeleteSession::from_json(&valid_delete_session_json()).expect("session loads");
    let bytes = b"read-only-uploaded-heic";
    let mut asset =
        cloudkit_uploaded_heic_asset("CPLAsset-uploaded-heic-123", "CPLMaster-heic-123", "tag-1");
    asset["records"][0]["zoneID"] = json!({});
    let mut master = cloudkit_uploaded_heic_master(
        "CPLMaster-heic-123",
        bytes.len() as u64,
        "https://p140-icloud-content.icloud.com/uploaded-heic",
    );
    master["records"][0]["zoneID"] = json!({});
    ...
    .expect("request-scoped records may return empty record-level zones");
...
#[test]
fn uploaded_heic_read_client_binds_shared_owner_in_requests_and_responses() {
    let session = CloudKitDeleteSession::from_json(&shared_delete_session_json())
        .expect("shared session loads");
    let bytes = b"shared-uploaded-heic";
    let asset_id = "CPLAsset-uploaded-heic-123";
    let master_id = "CPLMaster-heic-123";
    let mut asset = cloudkit_uploaded_heic_asset(asset_id, master_id, "tag-1");
    asset["records"][0]["zoneID"]["zoneName"] = json!("SharedSync-ABCDEF123456");
    asset["records"][0]["zoneID"]["ownerRecordName"] = json!("_shared-owner");
    asset["records"][0]["fields"]["masterRef"]["value"]["zoneID"]["zoneName"] =
    ...
    master["records"][0]["zoneID"]["zoneName"] = json!("SharedSync-ABCDEF123456");
    master["records"][0]["zoneID"]["ownerRecordName"] = json!("_shared-owner");
    let request = CloudKitUploadedHeicResolveRequest {
        uploaded_asset_id: asset_id.to_string(),
        expected_heic_sha256: sha256_hex(bytes),
    ...
    let mut transport = FakeCloudKitDeleteTransport::lookup_responses_with_downloads(
        vec![asset.clone(), master],
        vec![bytes.to_vec()],
    );

    CloudKitDeleteClient::new(&mut transport)
        .resolve_uploaded_heic_asset(&session, &request)
        .expect("matching shared owner must resolve");
    assert!(
        transport
            .lookup_payloads
            .iter()
            .all(|payload| { payload["zoneID"]["ownerRecordName"] == json!("_shared-owner") })
    );

    asset["records"][0]["zoneID"]["ownerRecordName"] = json!("_wrong-owner");
    let mut transport = ObservingUploadedHeicReadTransport {
        lookup_responses: vec![asset],
        lookup_calls: 0,
        download_calls: 0,
    ...
    let error = CloudKitUploadedHeicReadClient::new(&mut transport)
        .resolve_uploaded_heic_asset(&session, &request)
        .expect_err("wrong shared owner must fail closed");
    assert!(matches!(
        error,
        UploadError::InvalidCloudKitUploadedHeicResponse(_)
    ));
...
#[test]
fn uploaded_heic_read_client_rejects_nonempty_partial_or_wrong_record_zones() {
    let session = CloudKitDeleteSession::from_json(&shared_delete_session_json())
        .expect("shared session loads");
    let bytes = b"shared-uploaded-heic";
    let asset_id = "CPLAsset-uploaded-heic-123";
    ...
    let mut exact_asset = cloudkit_uploaded_heic_asset(asset_id, master_id, "tag-1");
    exact_asset["records"][0]["zoneID"] = json!({
        "zoneName": "SharedSync-ABCDEF123456",
        "ownerRecordName": "_shared-owner"
    });
    exact_asset["records"][0]["fields"]["masterRef"]["value"]["zoneID"] = json!({
        "zoneName": "SharedSync-ABCDEF123456",
        "ownerRecordName": "_shared-owner"
    });
    let mut exact_master = cloudkit_uploaded_heic_master(
        master_id,
        bytes.len() as u64,
        "https://p140-icloud-content.icloud.com/uploaded-heic",
    );
    exact_master["records"][0]["zoneID"] = json!({
        "zoneName": "SharedSync-ABCDEF123456",
        "ownerRecordName": "_shared-owner"
    });

    let invalid_record_zones = [
        json!({"zoneName": "SharedSync-ABCDEF123456"}),
        json!({"ownerRecordName": "_shared-owner"}),
        json!({
            "zoneName": "SharedSync-WRONG",
            "ownerRecordName": "_shared-owner"
        }),
        json!({
            "zoneName": "SharedSync-ABCDEF123456",
    ...
    for invalid_zone in invalid_record_zones {
        for invalid_asset_record in [true, false] {
            let mut asset = exact_asset.clone();
            let mut master = exact_master.clone();
            if invalid_asset_record {
                asset["records"][0]["zoneID"] = invalid_zone.clone();
            } else {
                master["records"][0]["zoneID"] = invalid_zone.clone();
            }
            let mut transport = ObservingUploadedHeicReadTransport {
                lookup_responses: vec![asset, master],
                lookup_calls: 0,
            ...
            let error = CloudKitUploadedHeicReadClient::new(&mut transport)
                .resolve_uploaded_heic_asset(&session, &request)
                .expect_err("nonempty partial or mismatched record zone must fail closed");
            ...
            assert_eq!(
                transport.lookup_calls,
                if invalid_asset_record { 1 } else { 2 }
            );
            assert_eq!(transport.download_calls, 0);
        }
    }
...
#[test]
fn uploaded_heic_read_client_requires_complete_exact_master_ref_zone() {
    let session = CloudKitDeleteSession::from_json(&shared_delete_session_json())
        .expect("shared session loads");
    let bytes = b"shared-uploaded-heic";
    let asset_id = "CPLAsset-uploaded-heic-123";
    ...
    let invalid_reference_zones = [
        None,
        Some(json!({})),
        Some(json!({"zoneName": "SharedSync-ABCDEF123456"})),
        Some(json!({"ownerRecordName": "_shared-owner"})),
        Some(json!({
            "zoneName": "SharedSync-WRONG",
            "ownerRecordName": "_shared-owner"
        })),
    ...
    for invalid_zone in invalid_reference_zones {
        let mut asset = exact_asset.clone();
        let master_ref = asset["records"][0]["fields"]["masterRef"]["value"]
            .as_object_mut()
            .expect("masterRef value");
        match invalid_zone {
            Some(zone) => {
                master_ref.insert("zoneID".to_string(), zone);
            }
            None => {
                master_ref.remove("zoneID");
            }
        }
        let mut transport = ObservingUploadedHeicReadTransport {
            lookup_responses: vec![asset, master.clone()],
            lookup_calls: 0,
            download_calls: 0,
        };
        ...
        let error = CloudKitUploadedHeicReadClient::new(&mut transport)
            .resolve_uploaded_heic_asset(&session, &request)
            .expect_err("masterRef zone must remain complete and exact");
...
fn cloudkit_original_asset_resolver_records_shared_library_destination() {
    ...
    let mut records =
        cloudkit_raw_pair("CPLAsset-original-123", "CPLMaster-raw-123", "change-tag-1");
    set_cloudkit_pair_destination(
        &mut records,
        "SharedSync-ABCDEF123456",
        Some("_shared-owner"),
    );
    let mut transport = FakeCloudKitDeleteTransport::query_responses_with_downloads(
    ...
    assert_eq!(proof.owner_record_name.as_deref(), Some("_shared-owner"));
    assert_eq!(
        transport.query_payloads[0]["zoneID"],
        json!({
            "zoneName": "SharedSync-ABCDEF123456",
...
#[test]
fn cloudkit_original_asset_resolvers_reject_unbound_shared_query_records_before_download() {
    let session =
        CloudKitDeleteSession::from_json(&shared_delete_session_json()).expect("session loads");
    let correct_pair = || {
        let mut records =
    ...
    for (record_index, path) in [
        (0, vec!["zoneID", "ownerRecordName"]),
        (1, vec!["zoneID", "ownerRecordName"]),
    ] {
        let mut missing = correct_pair();
        missing[record_index][path[0]]
            .as_object_mut()
            .expect("zone object")
            .remove(path[1]);
        cases.push(missing);

        let mut wrong = correct_pair();
        wrong[record_index][path[0]][path[1]] = json!("_wrong-owner");
        cases.push(wrong);
    }
    let mut missing_ref = correct_pair();
    missing_ref[0]["fields"]["masterRef"]["value"]["zoneID"]
        .as_object_mut()
        .expect("reference zone object")
        .remove("ownerRecordName");
    cases.push(missing_ref);
    let mut wrong_ref = correct_pair();
    wrong_ref[0]["fields"]["masterRef"]["value"]["zoneID"]["ownerRecordName"] =
        json!("_wrong-owner");
    cases.push(wrong_ref);

    for records in cases {
        let mut single_transport = FakeCloudKitDeleteTransport::query_responses_with_downloads(
            vec![json!({"records": records.clone()})],
            vec![b"raw-bytes".to_vec()],
        );
        let error = CloudKitDeleteClient::new(&mut single_transport)
            .resolve_original_asset(&session, &original_asset_resolve_request())
            .expect_err("single resolver must reject an unbound shared record graph");
        assert!(matches!(
            error,
            UploadError::InvalidCloudKitOriginalAssetResponse(_)
        ));
        assert!(single_transport.downloaded_urls.is_empty());

        let mut batch_transport = FakeCloudKitDeleteTransport::query_responses_with_downloads(
            vec![json!({"records": records})],
            vec![b"raw-bytes".to_vec()],
        );
        let error = CloudKitDeleteClient::new(&mut batch_transport)
            ...
            .expect_err("batch resolver must reject an unbound shared record graph");
        assert!(matches!(
            error,
            UploadError::InvalidCloudKitOriginalAssetResponse(_)
        ));
        assert!(batch_transport.downloaded_urls.is_empty());
    }
}

#[test]
fn cloudkit_original_asset_batch_resolver_propagates_exact_shared_owner() {
    let session =
        CloudKitDeleteSession::from_json(&shared_delete_session_json()).expect("session loads");
    let mut records =
        cloudkit_raw_pair("CPLAsset-original-123", "CPLMaster-raw-123", "change-tag-1");
    ...
    let proofs = CloudKitDeleteClient::new(&mut transport)
        ...
        .expect("exact shared record graph should resolve");

    assert_eq!(
        proofs["asset-1"].owner_record_name.as_deref(),
        Some("_shared-owner")
    );
}
...
#[test]
fn cloudkit_original_asset_batch_reconciliation_reports_unique_raw_and_replacement_coexistence() {
    let session = CloudKitDeleteSession::from_json(&valid_delete_session_json())
        .expect("session should load");
    let replacement = b"replacement-heic";
    let mut records = cloudkit_raw_pair_with(
    ...
    let CloudKitOriginalAssetResolveDisposition::Coexistence {
        original_proof,
        replacement_proof,
    } = &outcome.resolutions["asset-1"].disposition
    else {
        panic!("unique raw and replacement candidates should coexist");
    };
    assert_eq!(original_proof.record_name, "CPLAsset-both");
    assert_eq!(original_proof.size_bytes, 9);
    assert_eq!(replacement_proof.record_name, "CPLAsset-both");
    assert_eq!(replacement_proof.size_bytes, replacement.len() as u64);
    assert_eq!(replacement_proof.resource_field, "resOriginalAltRes");
    assert_eq!(
        outcome.resolutions["asset-1"].observations.raw_hash_matches,
        1
    );
    assert_eq!(
        outcome.resolutions["asset-1"]
...
#[test]
fn cloudkit_original_asset_batch_resolver_duplicate_replacement_candidates_stay_ambiguous() {
    let session = CloudKitDeleteSession::from_json(&valid_delete_session_json())
        .expect("session should load");
    let replacement = b"replacement-heic";
    let mut records = cloudkit_raw_pair_with(
        "CPLAsset-duplicate-replacement",
        "CPLMaster-duplicate-replacement",
        "tag-duplicate-replacement",
        9,
        1_800_000_000_000,
    );
    for (prefix, suffix) in [("resOriginalAlt", "one"), ("resSidecar", "two")] {
        records[1]["fields"][format!("{prefix}Res")] = json!({"value": {
            "size": replacement.len(),
            "downloadURL": format!("https://p140-icloud-content.icloud.com/replacement-{suffix}")
        }});
        records[1]["fields"][format!("{prefix}FileType")] = json!({"value": "public.heif"});
        records[1]["fields"][format!("{prefix}Fingerprint")] =
            json!({"value": format!("replacement-fingerprint-{suffix}")});
    }
    let mut target = batch_resolve_target("asset-1", "IMG_0001.dng", b"other-raw");
    target.replacement_candidate = Some(CloudKitLocalReplacementCandidate {
        sha256: sha256_hex(replacement),
    ...
    let outcome = CloudKitDeleteClient::new(&mut transport)
        .resolve_original_assets_batch_outcome(&session, &batch_resolve_request(vec![target]))
        .expect("duplicate replacement candidates should remain an audit result");
...
fn photos_upload_client_posts_requested_shared_library_zone() {
    ...
    assert_eq!(
        transport.service_calls[0].1["ownerRecordName"],
        json!("_shared-owner")
    );
    assert_eq!(
        transport.service_calls[1].1["zoneName"],
    ...
    assert_eq!(
        transport.service_calls[1].1["ownerRecordName"],
        json!("_shared-owner")
    );
    assert_eq!(
    ...
    assert_eq!(
        outcome.response.owner_record_name.as_deref(),
        Some("_shared-owner")
    );
}

#[test]
fn photos_upload_client_omits_private_primary_owner_from_service_payloads_but_preserves_outcome() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let heic_path = tempdir.path().join("IMG_0001.heic");
    fs::write(&heic_path, b"heic-bytes").expect("heic should be written");
    let session = UploadSession::from_json(&valid_session_json()).expect("session should load");
    ...
    let destination = CloudKitLibraryDestination {
        database_scope: CloudKitDatabaseScope::Private,
        zone_name: "PrimarySync".to_string(),
        owner_record_name: Some("_private-owner".to_string()),
    };

    let outcome = PhotosUploadClient::new(&mut transport)
        .with_status_poll_delay(std::time::Duration::ZERO)
        .upload_heic_to_library(&session, &heic_path, &destination)
        .expect("private PrimarySync upload should succeed");

    for (endpoint, payload) in &transport.service_calls[..2] {
        assert!(
            matches!(
                endpoint,
                PhotosUploadEndpoint::CreateUploadUrl | PhotosUploadEndpoint::PutAsset
            ),
            "unexpected service endpoint in upload setup: {endpoint:?}"
        );
        assert_eq!(payload["zoneName"], json!("PrimarySync"));
        assert!(
            payload.get("ownerRecordName").is_none(),
            "private PrimarySync service payload must omit ownerRecordName: {payload}"
        );
    }
    assert_eq!(
        outcome.response.database_scope,
    ...
    assert_eq!(outcome.response.zone_name, "PrimarySync");
    assert_eq!(
        outcome.response.owner_record_name.as_deref(),
        Some("_private-owner")
    );
