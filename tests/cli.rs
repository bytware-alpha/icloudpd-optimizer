...
fn upload_verified_reverify_target_set_sha256(asset_ids: &[&str]) -> String {
    let mut asset_ids = asset_ids.to_vec();
...
fn assert_upload_verified_reverify_gating_failure(args: &[&str], sentinels: &[&str]) {
    let output = binary().args(args).output().expect("command should run");
    assert_upload_verified_reverify_gating_output(output, sentinels);
}

fn assert_upload_verified_reverify_gating_output(output: std::process::Output, sentinels: &[&str]) {
    assert!(!output.status.success());
    assert!(output.stdout.is_empty());
    let stderr = String::from_utf8(output.stderr).expect("stderr should be utf8");
    assert_eq!(
        stderr,
        "upload-verified-reverify failed: class=gating target=batch\n"
    );
...
fn quarantine_mirror_only_evidence_entry(asset_id: &str) -> Value {
    json!({
        "asset_id": asset_id,
...
fn quarantine_evidence(entries: Vec<Value>) -> Value {
    Value::Array(entries)
}
...
fn replace_failed_assets_quarantine_evidence(
    fixture: &mut FailedAssetsQuarantineFixture,
    evidence: impl AsRef<[u8]>,
) {
    fs::write(&fixture.evidence_path, evidence).expect("fixture evidence should save");
...
fn workflow_original_asset_verified_records_shared_library_destination() {
    ...
    assert_eq!(
        record.proofs["original_asset"]["owner_record_name"],
        "_shared-owner"
    );
}
...
#[test]
fn monitor_upload_verified_reverify_stderr_redacts_setup_inputs() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let config = tempdir.path().join("SENTINEL_REVERIFY_CONFIG_PATH.json");
    let asset_id = "SENTINEL_REVERIFY_ASSET_ID";
    let target_set_sha256 = upload_verified_reverify_target_set_sha256(&[asset_id]);

    let output = binary()
        .args([
            "monitor",
            "upload-verified-reverify",
            "--config",
    ...
    assert!(output.stdout.is_empty());
    let stderr = String::from_utf8(output.stderr).expect("stderr should be utf8");
    assert!(stderr.contains("class=configuration target=batch"));
    for sentinel in ["SENTINEL_REVERIFY_CONFIG_PATH", asset_id] {
        assert!(!stderr.contains(sentinel), "stderr leaked {sentinel}");
    }
}

#[test]
fn monitor_upload_verified_reverify_redacts_invalid_target_count_before_clap_renders_it() {
    let asset_id = "SENTINEL_REVERIFY_INVALID_COUNT_ASSET";
    let target_set_sha256 = upload_verified_reverify_target_set_sha256(&[asset_id]);
    let invalid_count = "SENTINEL_TARGET_COUNT_TOKEN";
    assert_upload_verified_reverify_gating_failure(
        &[
            "monitor",
            "upload-verified-reverify",
            "--config",
            "/private/SENTINEL_INVALID_COUNT_CONFIG.json",
            "--asset-id",
            asset_id,
            "--expected-target-count",
            invalid_count,
        ...
        &[invalid_count, asset_id, "SENTINEL_INVALID_COUNT_CONFIG"],
    );
}

#[test]
fn monitor_upload_verified_reverify_redacts_overflow_target_count_before_clap_renders_it() {
    let asset_id = "SENTINEL_REVERIFY_OVERFLOW_COUNT_ASSET";
    let target_set_sha256 = upload_verified_reverify_target_set_sha256(&[asset_id]);
    let overflow_count = "18446744073709551616";
    assert_upload_verified_reverify_gating_failure(
        &[
            "monitor",
            "upload-verified-reverify",
            "--config",
            "/private/SENTINEL_OVERFLOW_COUNT_CONFIG.json",
            "--asset-id",
            asset_id,
            "--expected-target-count",
            overflow_count,
        ...
        &[overflow_count, asset_id, "SENTINEL_OVERFLOW_COUNT_CONFIG"],
    );
}

#[test]
fn monitor_upload_verified_reverify_redacts_missing_target_count_option() {
    let asset_id = "SENTINEL_REVERIFY_MISSING_COUNT_ASSET";
    let target_set_sha256 = upload_verified_reverify_target_set_sha256(&[asset_id]);
    assert_upload_verified_reverify_gating_failure(
        &[
            "monitor",
            "upload-verified-reverify",
            "--config",
            "/private/SENTINEL_MISSING_COUNT_CONFIG.json",
            "--asset-id",
            asset_id,
            "--expected-target-set-sha256",
            &target_set_sha256,
        ],
        &[asset_id, "SENTINEL_MISSING_COUNT_CONFIG"],
    );
}

#[test]
fn monitor_upload_verified_reverify_redacts_target_count_flag_without_value() {
    let asset_id = "SENTINEL_REVERIFY_EMPTY_COUNT_ASSET";
    let target_set_sha256 = upload_verified_reverify_target_set_sha256(&[asset_id]);
    assert_upload_verified_reverify_gating_failure(
        &[
            "monitor",
            "upload-verified-reverify",
            "--config",
            "/private/SENTINEL_EMPTY_COUNT_CONFIG.json",
            "--asset-id",
            asset_id,
            "--expected-target-count",
            "--expected-target-set-sha256",
        ...
        &[asset_id, "SENTINEL_EMPTY_COUNT_CONFIG"],
    );
}

#[test]
fn monitor_upload_verified_reverify_redacts_repeated_target_counts() {
    let asset_id = "SENTINEL_REVERIFY_REPEATED_COUNT_ASSET";
    let target_set_sha256 = upload_verified_reverify_target_set_sha256(&[asset_id]);
    let first_count = "SENTINEL_FIRST_TARGET_COUNT";
    let second_count = "SENTINEL_SECOND_TARGET_COUNT";
    assert_upload_verified_reverify_gating_failure(
        &[
            "monitor",
            "upload-verified-reverify",
            "--config",
            "/private/SENTINEL_REPEATED_COUNT_CONFIG.json",
            "--asset-id",
            asset_id,
            "--expected-target-count",
            first_count,
        ...
        &[
            ...
            second_count,
            asset_id,
            "SENTINEL_REPEATED_COUNT_CONFIG",
        ],
    );
}

#[test]
fn monitor_upload_verified_reverify_redacts_long_option_shaped_target_count() {
    let asset_id = "SENTINEL_REVERIFY_LONG_OPTION_COUNT_ASSET";
    let target_set_sha256 = upload_verified_reverify_target_set_sha256(&[asset_id]);
    let option_shaped_count = "--SENTINEL_SECRET_TARGET_COUNT";
    assert_upload_verified_reverify_gating_failure(
        &[
            "monitor",
            "upload-verified-reverify",
            "--config",
            "/private/SENTINEL_LONG_OPTION_COUNT_CONFIG.json",
            "--asset-id",
            asset_id,
            "--expected-target-count",
            option_shaped_count,
        ...
        &[
            option_shaped_count,
            asset_id,
            "SENTINEL_LONG_OPTION_COUNT_CONFIG",
        ],
    );
}

#[test]
fn monitor_upload_verified_reverify_redacts_short_option_shaped_target_count() {
    let asset_id = "SENTINEL_REVERIFY_SHORT_OPTION_COUNT_ASSET";
    let target_set_sha256 = upload_verified_reverify_target_set_sha256(&[asset_id]);
    let option_shaped_count = "-SENTINEL_SECRET_TARGET_COUNT";
    assert_upload_verified_reverify_gating_failure(
        &[
            ...
            "upload-verified-reverify",
            "--config",
            "/private/SENTINEL_SHORT_OPTION_COUNT_CONFIG.json",
            "--asset-id",
            asset_id,
            "--expected-target-count",
            option_shaped_count,
        ...
        &[
            option_shaped_count,
            asset_id,
            "SENTINEL_SHORT_OPTION_COUNT_CONFIG",
        ],
    );
}

#[test]
fn monitor_upload_verified_reverify_redacts_option_terminator_after_target_count() {
    let asset_id = "SENTINEL_REVERIFY_TERMINATOR_COUNT_ASSET";
    let target_set_sha256 = upload_verified_reverify_target_set_sha256(&[asset_id]);
    let trailing_token = "--SENTINEL_AFTER_OPTION_TERMINATOR";
    assert_upload_verified_reverify_gating_failure(
        &[
            "monitor",
            "upload-verified-reverify",
            "--config",
            "/private/SENTINEL_TERMINATOR_COUNT_CONFIG.json",
            "--asset-id",
            asset_id,
            "--expected-target-count",
            "--",
        ...
        &[trailing_token, asset_id, "SENTINEL_TERMINATOR_COUNT_CONFIG"],
    );
}

#[test]
fn monitor_upload_verified_reverify_redacts_equals_option_shaped_target_count() {
    let asset_id = "SENTINEL_REVERIFY_EQUALS_OPTION_COUNT_ASSET";
    let target_set_sha256 = upload_verified_reverify_target_set_sha256(&[asset_id]);
    let option_shaped_count = "--SENTINEL_EQUALS_TARGET_COUNT";
    let count_argument = format!("--expected-target-count={option_shaped_count}");
    assert_upload_verified_reverify_gating_failure(
        &[
            "monitor",
            "upload-verified-reverify",
            "--config",
            "/private/SENTINEL_EQUALS_OPTION_COUNT_CONFIG.json",
            "--asset-id",
            asset_id,
            &count_argument,
            "--expected-target-set-sha256",
        ...
        &[
            option_shaped_count,
            asset_id,
            "SENTINEL_EQUALS_OPTION_COUNT_CONFIG",
        ],
    );
}

#[test]
fn monitor_upload_verified_reverify_redacts_unrelated_unknown_argument() {
    let asset_id = "ordinary-asset";
    let target_set_sha256 = upload_verified_reverify_target_set_sha256(&[asset_id]);
    let unknown_argument = "--SENTINEL_UNRELATED_UNKNOWN_ARGUMENT";
    let output = binary()
        .args([
            "monitor",
            "upload-verified-reverify",
    ...
    assert_upload_verified_reverify_gating_output(output, &[unknown_argument]);
}

#[test]
fn monitor_upload_verified_reverify_redacts_every_clap_parse_failure_class() {
    let asset_id = "SENTINEL_PARSE_MATRIX_ASSET";
    let target_set_sha256 = upload_verified_reverify_target_set_sha256(&[asset_id]);
    let base = || {
        vec![
            "monitor".to_string(),
            "upload-verified-reverify".to_string(),
            "--config".to_string(),
            "/private/SENTINEL_PARSE_MATRIX_CONFIG.json".to_string(),
            "--asset-id".to_string(),
            asset_id.to_string(),
            "--expected-target-count".to_string(),
            "1".to_string(),
            "--expected-target-set-sha256".to_string(),
            target_set_sha256.clone(),
        ]
    };
    let mut malformed_apply = base();
    malformed_apply.push("--apply=SENTINEL_APPLY_TOKEN".to_string());
    let mut positional = base();
    positional.push("SENTINEL_REVERIFY_POSITIONAL_TOKEN".to_string());
    let cases = [
        ("malformed apply", malformed_apply, "SENTINEL_APPLY_TOKEN"),
        (
            "missing config path",
            vec![
                "monitor".to_string(),
        ...
        (
            ...
            vec![
                ...
                "upload-verified-reverify".to_string(),
                "--config".to_string(),
                "/private/SENTINEL_MISSING_COUNT_PATH.json".to_string(),
                "--asset-id".to_string(),
                asset_id.to_string(),
                "--expected-target-set-sha256".to_string(),
                target_set_sha256.clone(),
            ],
            "SENTINEL_MISSING_COUNT_PATH",
        ),
        (
            "malformed count",
            vec![
                ...
                "upload-verified-reverify".to_string(),
                "--config".to_string(),
                "/private/SENTINEL_MALFORMED_COUNT_PATH.json".to_string(),
                "--asset-id".to_string(),
                asset_id.to_string(),
                "--expected-target-count".to_string(),
                "SENTINEL_MALFORMED_COUNT".to_string(),
                "--expected-target-set-sha256".to_string(),
                target_set_sha256.clone(),
            ],
            "SENTINEL_MALFORMED_COUNT",
        ),
        (
            "malformed hash",
            vec![
                ...
                "upload-verified-reverify".to_string(),
                "--config".to_string(),
                "/private/SENTINEL_MALFORMED_HASH_PATH.json".to_string(),
                "--asset-id".to_string(),
                asset_id.to_string(),
                "--expected-target-count".to_string(),
                "1".to_string(),
                "--expected-target-set-sha256".to_string(),
                "SENTINEL_MALFORMED_HASH".to_string(),
            ],
            "SENTINEL_MALFORMED_HASH",
        ),
        (
            "missing hash option",
            vec![
                ...
                "upload-verified-reverify".to_string(),
                "--config".to_string(),
                "/private/SENTINEL_MISSING_HASH_OPTION_PATH.json".to_string(),
                "--asset-id".to_string(),
                asset_id.to_string(),
                "--expected-target-count".to_string(),
                "1".to_string(),
            ],
            "SENTINEL_MISSING_HASH_OPTION_PATH",
        ),
        (
            "missing hash value",
            vec![
                ...
                "upload-verified-reverify".to_string(),
                "--config".to_string(),
                "/private/SENTINEL_MISSING_HASH_PATH.json".to_string(),
                "--asset-id".to_string(),
                asset_id.to_string(),
                "--expected-target-count".to_string(),
                "1".to_string(),
            ...
            "SENTINEL_MISSING_HASH_PATH",
        ),
        (
            "positional token",
            positional,
            "SENTINEL_REVERIFY_POSITIONAL_TOKEN",
        ),
    ];

    for (case, args, sentinel) in cases {
        let output = binary()
            .args(&args)
            .output()
            .unwrap_or_else(|error| panic!("{case} command should run: {error}"));
        assert_upload_verified_reverify_gating_output(output, &[sentinel, asset_id]);
    }
}

#[test]
fn non_reverify_unknown_argument_retains_normal_clap_diagnostic() {
    let unknown_argument = "--SENTINEL_NON_REVERIFY_UNKNOWN_ARGUMENT";
    let output = binary()
        .args(["monitor", "stats", unknown_argument])
        .output()
        .expect("command should run");
...
fn monitor_failed_assets_quarantine_dry_run_reports_verified_immutable_target_set() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let evidence = quarantine_evidence(vec![
...
#[test]
fn monitor_failed_assets_quarantine_accepts_literal_raw_array_evidence_shape() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let mut fixture = write_failed_assets_quarantine_fixture(
        ...
        &[
            ("asset-alpha", State::Failed),
            ("asset-beta", State::Failed),
            ("asset-gamma", State::Failed),
        ],
        json!({"ignored": true}),
    );
    replace_failed_assets_quarantine_evidence(
        &mut fixture,
        include_str!("fixtures/failed-assets-quarantine-evidence.json"),
    );

    let report = failed_assets_quarantine_dry_run(&fixture, "3", "2");

    assert_eq!(report["counts"]["failed_assets"], 3);
    assert_eq!(report["counts"]["with_upload_or_delete_side_effects"], 2);
    assert_eq!(report["counts"]["clean_of_recorded_remote_side_effects"], 1);
    assert_eq!(report["counts"]["side_effect_assets"], 2);
}

#[test]
fn monitor_failed_assets_quarantine_accepts_mirror_only_side_effect_evidence() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let fixture = write_failed_assets_quarantine_fixture(
        &tempdir,
        &[("asset-alpha", State::Failed)],
        quarantine_evidence(vec![quarantine_mirror_only_evidence_entry("asset-alpha")]),
    );

    let report = failed_assets_quarantine_dry_run(&fixture, "1", "1");

    assert_eq!(report["verified"], true);
    assert_eq!(report["counts"]["side_effect_assets"], 1);
}

#[test]
fn monitor_failed_assets_quarantine_rejects_stale_failed_cohorts_without_record_updates() {
    let evidence = quarantine_evidence(vec![
        quarantine_evidence_entry("asset-alpha", 1),
        quarantine_evidence_entry("asset-clean", 0),
    ]);
    for records in [
        vec![
            ("asset-alpha", State::Failed),
            ("asset-clean", State::Failed),
            ("asset-extra", State::Failed),
        ],
        vec![("asset-alpha", State::Failed)],
        vec![
            ("asset-alpha", State::Failed),
            ("asset-clean", State::Converted),
        ],
    ] {
        let tempdir = tempfile::tempdir().expect("tempdir should be created");
        let fixture = write_failed_assets_quarantine_fixture(&tempdir, &records, evidence.clone());
        ...
        let durable_manifest = state_store.load().expect("state should load");
        for (asset_id, state) in records {
            assert_eq!(
            ...
            assert_eq!(
                durable_manifest
                    .get(asset_id)
                    .expect("durable asset should exist")
                    .state,
                state
            );
        }
...
#[test]
fn monitor_failed_assets_quarantine_target_set_changes_with_exact_record_bytes() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let fixture = write_failed_assets_quarantine_fixture(
        &tempdir,
        &[("asset-alpha", State::Failed)],
        quarantine_evidence(vec![quarantine_evidence_entry("asset-alpha", 1)]),
    );
    let first_report = failed_assets_quarantine_dry_run(&fixture, "1", "1");
    let state_store = AssetStateStore::open_writer(
        &fixture.manifest_path,
        "target-set-change-test",
        Duration::from_secs(1),
    )
    .expect("state store should open");
    let mut changed_record = state_store
        .load()
    ...
    changed_record
        .proofs
        .insert("record_byte_change".to_string(), json!({"changed": true}));
    changed_record.updated_at = "9999999999.000000000Z".to_string();
    state_store
        .persist_record(&changed_record)
        .expect("changed record should persist");
    state_store.export_json().expect("state should export");
    drop(state_store);

    let second_report = failed_assets_quarantine_dry_run(&fixture, "1", "1");
...
fn monitor_failed_assets_quarantine_rejects_same_size_failed_cohort_swap_atomically() {
    ...
    let fixture = write_failed_assets_quarantine_fixture(
        ...
        quarantine_evidence(vec![
            quarantine_evidence_entry("asset-alpha", 1),
            quarantine_evidence_entry("asset-beta", 0),
        ]),
    );
...
fn monitor_failed_assets_quarantine_rejects_mismatched_or_invalid_evidence_hashes() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let evidence = quarantine_evidence(vec![quarantine_evidence_entry("asset-alpha", 1)]);
    let fixture = write_failed_assets_quarantine_fixture(
        &tempdir,
        &[("asset-alpha", State::Failed)],
        evidence,
...
#[test]
fn monitor_failed_assets_quarantine_rejects_expected_count_mismatches() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let evidence = quarantine_evidence(vec![
        quarantine_evidence_entry("asset-alpha", 1),
    ...
    binary()
        .args(failed_assets_quarantine_args(&count_fixture, "2", "1"))
        .assert()
        .failure()
        .stderr(predicate::str::contains(
...
fn monitor_failed_assets_quarantine_rejects_duplicate_or_empty_asset_ids() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    for (evidence, expected_failed_asset_count, expected_side_effect_asset_count) in [
        (
            quarantine_evidence(vec![
                quarantine_evidence_entry("asset-alpha", 1),
                quarantine_evidence_entry("asset-alpha", 1),
        ...
        (
            quarantine_evidence(vec![quarantine_evidence_entry("", 1)]),
            "1",
            "1",
        ),
    ] {
...
#[test]
fn monitor_failed_assets_quarantine_rejects_empty_array_and_zero_side_effect_targets() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let empty_evidence = quarantine_evidence(vec![]);
    ...
    binary()
        ...
        .failure()
        .stderr(predicate::str::contains(
            "failed-asset set must be nonempty",
        ));

    let zero_side_effect_evidence =
        quarantine_evidence(vec![quarantine_evidence_entry("asset-alpha", 0)]);
...
fn monitor_failed_assets_quarantine_target_set_is_deterministic_across_evidence_order() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let first_evidence = quarantine_evidence(vec![
        quarantine_evidence_entry("asset-alpha", 1),
        quarantine_evidence_entry("asset-beta", 2),
    ]);
    let mut fixture = write_failed_assets_quarantine_fixture(
    ...
    let reversed_evidence = quarantine_evidence(vec![
        quarantine_evidence_entry("asset-beta", 2),
        quarantine_evidence_entry("asset-alpha", 1),
    ]);
    replace_failed_assets_quarantine_evidence(
...
fn monitor_failed_assets_quarantine_applies_atomically_preserves_history_and_rejects_second_apply()
{
    ...
    for (asset_id, uploads) in [("asset-alpha", 3), ("asset-beta", 1)] {
        ...
        assert_eq!(record.proofs["failure_quarantine"]["mirror_successes"], 0);
        assert!(
...
#[test]
fn service_hard_stop_blocks_monitor_before_it_acquires_processing_state() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let config_path = tempdir.path().join("monitor.json");
    let download_root = tempdir.path().join("download");
    let heic_dir = tempdir.path().join("heic");
    let manifest_path = tempdir.path().join("manifest.json");
    let hard_stop_path = tempdir.path().join("service.hard-stop");
    ...
    binary()
        ...
        .assert()
        .success();
    fs::write(&hard_stop_path, "production processing disabled\n")
        .expect("hard stop should be written");
    ...
    binary()
        .args([
            ...
            hard_stop_path
                .to_str()
                .expect("hard stop path should be utf8"),
        ])
        .assert()
        .failure()
        .stderr(predicate::str::contains(
            "production processing is hard-stopped",
        ));

    assert!(hard_stop_path.exists());
    assert!(
        !manifest_path.with_extension("state.sqlite3").exists(),
        "the hard stop must reject execution before monitor processing state is acquired"
    );
}

#[test]
fn service_install_rejects_debug_binary_before_mutating_service_paths() {
    let tempdir = tempfile::tempdir().expect("tempdir should be created");
    let config_path = tempdir.path().join("monitor.json");
    let download_root = tempdir.path().join("download");
    let heic_dir = tempdir.path().join("heic");
    ...
    binary()
        ...
        .failure()
        .stderr(predicate::str::contains(
            "service admission rejected: installed sealed Service authority is required",
        ));

    let hard_stop_path = plist_path.with_extension("hard-stop");
    assert!(
        !plist_path.exists(),
        "admission must reject before writing the launchd plist"
    );
    assert!(
        !hard_stop_path.exists(),
        "admission must reject before engaging the durable processing hard-stop"
    );
    assert!(
        !stdout_path.exists() && !stderr_path.exists(),
        "admission must reject before creating service log directories or files"
    );
    assert!(
...
#[test]
fn production_macos_signing_keychain_binding_is_explicit_and_redacted() {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let script = fs::read_to_string(repo_root.join("packaging/macos/build-app.sh"))
        .expect("macOS app build script should be readable");
    let policy =
        fs::read_to_string(repo_root.join("policies/authorization-policy-production.json"))
            .expect("production authority should be readable");

    assert!(script.contains("account_record=\"$(id -P 2>/dev/null || true)\""));
    assert!(script.contains("effective_user=\"$(id -un 2>/dev/null || true)\""));
    assert!(script.contains("NF != 10"));
    assert!(script.contains("$3 !~ /^[0-9]+$/"));
    assert!(script.contains("$9 !~ /^\\//"));
    assert!(script.contains("NR == 1"));
    assert!(script.contains("NR != 1 { invalid = 1 }"));
    assert!(script.contains("print $9"));
    assert!(script.contains("trusted_user\" != \"$effective_user"));
    assert!(script.contains("trusted_uid\" != \"$effective_uid"));
    assert!(!script.contains("dscl . -search /Users UniqueID"));
    assert!(!script.contains("NFSHomeDirectory"));
    assert!(
        script.contains("expected_keychain=\"$trusted_home/Library/Keychains/login.keychain-db\"")
    );
    assert!(!script.contains("security default-keychain"));
    assert!(script.contains("[ -L \"$keychain_component\" ]"));
    assert!(script.contains("stat -f '%u'"));
    assert!(script.contains("stat -f '%Lp'"));
    assert!(script.contains("stat -f '%HT'"));
    assert!(script.contains(
        "[ \"$keychain_component\" != \"$expected_keychain\" ] \\\n      && xattr -p com.apple.quarantine"
    ));
    assert!(script.contains("realpath \"$expected_keychain\" 2>/dev/null || true"));
    assert!(
        script.contains(
            "identity_listing=\"$(security find-identity -v -p codesigning \"$expected_keychain\" 2>/dev/null)\""
        )
    );
    assert!(script.contains("requested_identity_sha1="));
    assert!(script.contains("label == expected || toupper(hash) == requested"));
    assert!(script.contains("--sign \"$leaf_sha1\" \"$signing_probe\""));
    assert!(script.contains("--sign \"${leaf_sha1:-$sign_identity}\""));
    assert!(script.contains("codesign_designated_requirement_equals"));
    assert!(script.contains("codesign -d -r- \"$signed_path\""));
    assert!(script.contains("[ \"$actual_requirement\" != \"$expected_requirement\" ]"));
    assert!(script.contains("-r=\"$helper_requirement\" \"$resources_path/icloudpd-optimizer\""));
    assert!(script.contains("-r=\"$host_requirement\" \"$macos_path/ICloudPDOptimizerApp\""));
    assert!(script.contains("-r=\"$host_requirement\" \"$app_path\""));
    assert!(script.contains(
        "\"-R=${helper_requirement#designated => }\" \"$resources_path/icloudpd-optimizer\""
    ));
    assert_eq!(
        script
            .matches("\"-R=${host_requirement#designated => }\"")
            .count(),
        2,
        "each host signature must be evaluated as one -R requirement expression"
    );
    assert!(!script.contains("--strict -R \"$helper_requirement\""));
    assert!(!script.contains("--strict -R \"$host_requirement\""));
    assert_eq!(
        script
            .matches("codesign_designated_requirement_equals \"$helper_requirement\"")
            .count(),
        1,
        "the embedded helper must have its exact designated requirement extracted"
    );
    assert_eq!(
        script
            .matches("codesign_designated_requirement_equals \"$host_requirement\"")
            .count(),
        2,
        "each role bundle's host executable and enclosing app must have exact designated requirements"
    );
    assert!(script.contains(
        "--identifier com.icloudpd-optimizer.helper --keychain \"$keychain_path\" --sign \"${leaf_sha1:-$sign_identity}\" -r=\"$helper_requirement\" \"$resources_path/icloudpd-optimizer\""
    ));
    assert!(script.contains(
        "--identifier \"$id\" --keychain \"$keychain_path\" --sign \"${leaf_sha1:-$sign_identity}\" -r=\"$host_requirement\" \"$macos_path/ICloudPDOptimizerApp\""
    ));
    assert!(script.contains(
        "--identifier \"$id\" --keychain \"$keychain_path\" --sign \"${leaf_sha1:-$sign_identity}\" -r=\"$host_requirement\" \"$app_path\""
    ));
    assert!(script.contains(
        "--identifier com.icloudpd-optimizer.helper --sign - \"$resources_path/icloudpd-optimizer\""
    ));
    assert!(script.contains("--identifier \"$id\" --sign - \"$macos_path/ICloudPDOptimizerApp\""));
    assert!(script.contains("--identifier \"$id\" --sign - \"$app_path\""));
    assert!(script.contains("codesign_redacted"));
    assert!(script.contains("codesign_verify_redacted"));
    assert!(
        !script.contains("authorization-provenance.json\" <<JSON\n{\"schema_version\":1,\"leaf")
    );
    assert!(policy.contains("certificate leaf[subject.OU]"));
    let provenance = script
        .split("authorization-provenance.json\" <<JSON\n")
        .nth(1)
        .and_then(|section| section.split("\nJSON").next())
        .expect("packaging script should write authorization provenance");
    for secret_or_machine_specific_marker in [
        "leaf_sha1",
        "leaf fingerprint",
        "certificate leaf =",
        "root_sha1",
        ...
        assert!(
            !policy
                .to_ascii_lowercase()
                .contains(&secret_or_machine_specific_marker.to_ascii_lowercase()),
            "policy must not contain {secret_or_machine_specific_marker:?}"
        );
        assert!(
            !provenance
                .to_ascii_lowercase()
                .contains(&secret_or_machine_specific_marker.to_ascii_lowercase()),
            "provenance must not contain {secret_or_machine_specific_marker:?}"
        );
    }
    assert!(policy.contains("\"authority_kind\": \"apple_development_team\""));
    assert!(policy.contains("\"team_id\""));
}
...
fn macos_app_packaging_surface_is_documented() {
    ...
    assert!(script.contains("NSLocalNetworkUsageDescription"));
    assert!(!script.contains("NSBonjourServices"));
    assert!(!script.contains("NSAppTransportSecurity"));
    ...
    assert!(app_source.contains("loadDashboardMonitorConfig(at: configPath)"));
    ...
    let proxy_launch_args = swift_function_body(
        &app_source,
        "private func shouldProxyLaunchArguments(_ args: [String]) -> Bool",
    );
    assert!(proxy_launch_args.contains("guard !args.isEmpty else"));
    for excluded_mode in [
        "!isPrimeAccessArgs(args)",
        ...
        assert!(
            proxy_launch_args.contains(excluded_mode),
            "proxy launch arguments must exclude {excluded_mode}"
        );
    }
...
#[test]
fn macos_smb_credential_action_is_service_targeted_and_gated() {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let app_source =
        fs::read_to_string(repo_root.join("packaging/macos/ICloudPDOptimizerApp.swift"))
            .expect("macOS app source should be readable");
    let cli_source =
        fs::read_to_string(repo_root.join("src/cli.rs")).expect("CLI source should be readable");

    assert!(app_source.contains("Label(\"Set Up SMB Credential\""));
    assert!(app_source.contains("SMBCredentialSetupSheet"));
    assert!(app_source.contains("SecureField(\"Enter NAS/SMB password\""));
    assert!(app_source.contains("This is the NAS/SMB password, not your Mac login password."));
    assert!(app_source.contains("Authorize NAS Folder"));
    assert!(app_source.contains("NAS folder access and SMB login are separate permissions."));
    assert!(app_source.contains("Approve the macOS Keychain dialog if shown"));
    assert!(app_source.contains("Verify the exact signed helper headlessly"));
    assert!(
        app_source
            .contains(".interactiveDismissDisabled(model.smbCredentialAuthorizationInFlight)")
    );
    assert!(!app_source.contains("NSButton(title: \"Set SMB Credential\""));
    assert!(!app_source.contains("NSSecureTextField(frame: .zero)"));
    for status in [
        "authorized",
        "not_found",
        "ambiguous",
        ...
        assert!(
            app_source.contains(&format!("case \"{status}\"")),
            "setup sheet must map the redacted {status} result"
        );
    }
    assert!(app_source.contains(
        "Credentials are ready for unattended access; this does not enable the monitor service."
    ));
    assert!(app_source.contains("This is not an SMB password rejection."));
    assert!(app_source.contains(
        "The signed helper, sealed configuration, or existing dedicated credential access control did not match."
    ));
    assert!(app_source.contains("smb_credential_setup_headless_proof_started"));
    assert!(app_source.contains("smb_credential_setup_finished"));
    assert!(app_source.contains("case checkingExistingCredential"));
    assert!(app_source.contains(
        "@Published private(set) var smbCredentialSetupPhase: SMBCredentialSetupPhase = .checkingExistingCredential"
    ));
    assert!(app_source.contains("A password field will appear only if the fixed result proves the dedicated record is not_found."));
    assert!(
        app_source.contains(
            "Label(\"Re-check exact credential\", systemImage: \"arrow.clockwise.circle\")"
        )
    );
    let credential_preflight = swift_function_body(&app_source, "func presentSMBCredentialSetup()");
    assert!(
        credential_preflight
            .contains("beginSMBCredentialProof(event: \"smb_credential_setup_preflight_started\")")
    );
    let credential_recheck = swift_function_body(&app_source, "func recheckSMBCredential()");
    assert!(credential_recheck.contains(
        "beginSMBCredentialProof(event: \"smb_credential_setup_recheck_proof_started\")"
    ));
    assert!(app_source.contains("iCloudPD Optimizer Service.app"));
    let credential_proof = swift_function_body(
        &app_source,
        "private func runInstalledServiceSMBCredentialAction(",
    );
    let credential_setup = swift_function_body(
        &app_source,
        "private func startInstalledServiceSMBCredentialSetup(",
    );
    for action in [credential_proof, credential_setup] {
        assert!(
            action.contains("installedServiceHelperExecutable(serviceBundle)"),
            "SMB credential actions must directly launch the installed helper"
        );
        assert!(
            !action.contains("Contents/MacOS/ICloudPDOptimizerApp"),
            "SMB credential actions must never launch the Service host wrapper"
        );
        assert!(
            !action.contains("result.stderr"),
            "SMB credential actions must not display or log raw helper stderr"
        );
    }
    assert!(
        credential_proof.contains("SMBCredentialActionResult.parse(result)"),
        "credential proof results must use the strict redacted parser"
    );
    assert!(app_source.contains("supplied_credential_validation"));
    assert!(app_source.contains("stored_credential_validation"));
    assert!(app_source.contains("let keys = Set(payload.keys)"));
    assert!(app_source.contains("keys == Set([\"status\"])"));
    assert!(app_source.contains("keys == Set([\"status\", \"category\", \"stage\", \"reason\"])"));
    assert!(app_source.contains("Redacted diagnostic:"));
    let credential_result_parser = swift_function_body(
        &app_source,
        "static func parse(_ result: ProcessResult) -> Self",
    );
    assert!(!credential_result_parser.contains("stderr"));
    assert!(credential_result_parser.contains("SMBCredentialValidationDiagnostic("));
    assert!(credential_result_parser.contains("return .integrityMismatch"));
    let installed_helper = swift_function_body(
        &app_source,
        "private func installedServiceHelperExecutable(_ serviceBundle: URL) -> URL",
    );
    assert!(installed_helper.contains("Contents/Resources/icloudpd-optimizer"));
    let dashboard_setup = swift_function_body(
        &app_source,
        "func setSMBCredentialFromDashboard(_ password: inout WipeablePasswordBuffer)",
    );
    assert!(dashboard_setup.contains("smbCredentialSetupResult == \"not_found\""));
    assert!(dashboard_setup.contains("smbCredentialSetupPhase = .validatingConnection"));
    assert!(dashboard_setup.contains("storedOutcome.status == \"authorized\""));
    assert!(dashboard_setup.contains("smbCredentialSetupPhase = .verifyingSignedHelper"));
    assert!(dashboard_setup.contains("proveSMBCredential()"));
    assert_swift_code_order(
        dashboard_setup,
        "guard storedOutcome.status",
        "smbCredentialSetupPhase = .verifyingSignedHelper",
    );
    let credential_process = swift_function_body(
        &app_source,
        "init(executableURL: URL, arguments: [String], password: inout WipeablePasswordBuffer) throws",
    );
    assert!(credential_process.contains("bundledHelperProcess("));
    assert!(credential_process.contains("mode: .dashboard"));
    assert!(credential_process.contains("stdin.fileHandleForWriting.close()"));
    assert!(!credential_process.contains("standardInput: password"));
    assert!(app_source.contains("!candidate.contains(13)"));
    assert!(app_source.contains("SMBMountRootResolver.resolve("));
    assert!(app_source.contains("observation.fileSystemType == \"smbfs\""));
    assert!(app_source.contains("observe(selected.mountPoint) == selected"));
    assert!(!app_source.contains("plan.readRoots.count == 1"));
    assert!(app_source.contains("\"server_rejected\""));
    assert!(cli_source.contains("SetSmbCredential(SmbCredentialArgs)"));
    assert!(!cli_source.contains("MonitorCommand::SetSmbCredential"));
}

#[test]
fn macos_signed_dashboard_smb_credential_provisioning_is_stdin_only_and_fail_closed() {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let app_source =
        fs::read_to_string(repo_root.join("packaging/macos/ICloudPDOptimizerApp.swift"))
            .expect("macOS app source should be readable");
    ...
    assert!(app_source.contains(
        "let signedDashboardSMBCredentialProvisioningArgument = \"--signed-dashboard-set-smb-credential\""
    ));
    let provisioning_args = swift_function_body(
        &app_source,
        "private func isSignedDashboardSMBCredentialProvisioningArgs(_ args: [String]) -> Bool",
    );
    assert!(
        provisioning_args.contains("args == [signedDashboardSMBCredentialProvisioningArgument]")
    );
    let proxy_args = swift_function_body(
        &app_source,
        "private func shouldProxyLaunchArguments(_ args: [String]) -> Bool",
    );
    assert!(proxy_args.contains("!isSignedDashboardSMBCredentialProvisioningArgs(args)"));

    let provisioning = swift_function_body(
        &app_source,
        "func runSignedDashboardSMBCredentialProvisioningAndExit() -> Never",
    );
    assert!(provisioning.contains("resolveEmbeddedDashboardConfigPath()?.path"));
    assert!(!provisioning.contains("resolveDashboardConfigPath()"));
    assert!(!provisioning.contains("argumentValue("));
    assert!(!provisioning.contains("\"--config\""));
    assert!(provisioning.contains("installedServiceHelperExecutable(serviceBundle)"));
    assert!(!provisioning.contains("Contents/MacOS/ICloudPDOptimizerApp"));
    assert!(provisioning.contains("serviceSMBCredentialSetupArguments("));
    assert!(provisioning.contains("bundledHelperProcess("));
    assert!(provisioning.contains("mode: .dashboard"));
    assert!(provisioning.contains("logEvents: false"));
    assert!(provisioning.contains("inheritParentEnvironment: false"));
    assert!(provisioning.contains("process.standardInput = FileHandle.standardInput"));
    assert!(provisioning.contains("process.standardOutput = FileHandle.standardOutput"));
    assert!(provisioning.contains("process.standardError = FileHandle.nullDevice"));
    assert!(provisioning.contains("process.waitUntilExit()"));
    assert!(provisioning.contains("exit(process.terminationStatus)"));
    assert!(provisioning.contains("emitSMBCredentialProvisioningIntegrityMismatchAndExit()"));
    for forbidden in [
        "WipeablePasswordBuffer",
        "Pipe()",
        "readData",
        "AppLogger.log",
        ...
        assert!(
            ...
            "headless provisioning must not use {forbidden}"
        );
    }

    let setup_args = swift_function_body(
        &app_source,
        "private func serviceSMBCredentialSetupArguments(",
    );
    assert!(setup_args.contains("[\"set-smb-credential\"]"));
    assert!(setup_args.contains("serviceSMBCredentialArguments(configPath: configPath)"));
    assert!(setup_args.contains("[\"--service-bundle\", serviceBundle.path, \"--json\"]"));
    let embedded_resolution = swift_function_body(
        &app_source,
        "private func resolveEmbeddedDashboardConfigPath() -> DashboardConfigResolution?",
    );
    assert!(embedded_resolution.contains("bundledConfigPath()"));
    assert!(!embedded_resolution.contains("launchAgentConfigPath()"));
    assert!(!embedded_resolution.contains("defaultConfigPath()"));
    let redacted_failure = swift_function_body(
        &app_source,
        "private func emitSMBCredentialProvisioningIntegrityMismatchAndExit() -> Never",
    );
    assert!(redacted_failure.contains("{\\\"status\\\":\\\"integrity_mismatch\\\"}\\n"));
    assert!(!redacted_failure.contains("error"));
...
#[test]
fn macos_signed_dashboard_smb_credential_fifo_provisioning_is_strict_and_fail_closed() {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let app_source =
        fs::read_to_string(repo_root.join("packaging/macos/ICloudPDOptimizerApp.swift"))
            .expect("macOS app source should be readable");
    ...
    assert!(app_source.contains(
        "let signedDashboardSMBCredentialFIFOProvisioningArgument = \"--signed-dashboard-set-smb-credential-fifo\""
    ));
    let fifo_attempt = swift_function_body(
        &app_source,
        "private func isSignedDashboardSMBCredentialFIFOProvisioningAttempt(_ args: [String]) -> Bool",
    );
    assert!(
        fifo_attempt.contains("args.first == signedDashboardSMBCredentialFIFOProvisioningArgument")
    );
    let invocation_parse =
        swift_function_body(&app_source, "static func parse(_ args: [String]) -> Self?");
    assert!(invocation_parse.contains("args.count == 3"));
    assert!(
        invocation_parse
            .contains("args[0] == signedDashboardSMBCredentialFIFOProvisioningArgument")
    );
    assert!(!invocation_parse.contains("argumentValue("));

    let run_directory = swift_function_body(
        &app_source,
    ...
    for component in [
        ...
        assert!(run_directory.contains(component));
    }
    let directory_validation = swift_function_body(
        &app_source,
        "private func validateSignedDashboardSMBCredentialRunDirectory(_ path: String) throws",
    );
    assert!(directory_validation.contains("path == expectedPath"));
    assert!(directory_validation.contains("signedDashboardSMBCredentialCanonicalPath(path)"));
    assert!(directory_validation.contains("canonicalPath == expectedPath"));
    assert!(directory_validation.contains("signedDashboardSMBCredentialLstat(path)"));
    assert!(directory_validation.contains("(value.st_mode & S_IFMT) == S_IFDIR"));
    assert!(directory_validation.contains("value.st_uid == geteuid()"));
    assert!(directory_validation.contains("mode_t(0o700)"));
    let canonical_path = swift_function_body(
        &app_source,
        "private func signedDashboardSMBCredentialCanonicalPath(_ path: String) -> String?",
    );
    assert!(canonical_path.contains("Darwin.realpath"));
    assert!(canonical_path.contains("Darwin.free"));

    let path_validation = swift_function_body(
        &app_source,
        "private func validateSignedDashboardSMBCredentialFIFOPath(",
    );
    assert!(path_validation.contains("let prefix = runDirectory + \"/\""));
    assert!(path_validation.contains("path.hasPrefix(prefix)"));
    assert!(path_validation.contains("path == prefix + basename"));
    assert!(path_validation.contains("signedDashboardSMBCredentialFIFOToken"));
    let token_validation = swift_function_body(
        &app_source,
        "private func signedDashboardSMBCredentialFIFOToken(",
    );
    assert!(token_validation.contains("smb-credential-\\(role.rawValue)-"));
    assert!(token_validation.contains(".fifo"));
    assert!(
        token_validation
            .contains("token.unicodeScalars.count == signedDashboardSMBCredentialFIFOTokenLength")
    );
    assert!(token_validation.contains("(48...57).contains(scalar.value)"));
    assert!(token_validation.contains("(97...102).contains(scalar.value)"));

    let fifo_stat = swift_function_body(
        &app_source,
        "private func validateSignedDashboardSMBCredentialFIFOStat(_ value: stat) throws",
    );
    assert!(fifo_stat.contains("(value.st_mode & S_IFMT) == S_IFIFO"));
    assert!(fifo_stat.contains("value.st_uid == geteuid()"));
    assert!(fifo_stat.contains("mode_t(0o600)"));
    assert!(fifo_stat.contains("value.st_nlink == 1"));
    let fifo_open = swift_function_body(
        &app_source,
        "private func signedDashboardSMBCredentialOpenFIFO(",
    );
    assert!(fifo_open.contains("let beforeOpen = try signedDashboardSMBCredentialLstat(path)"));
    assert!(fifo_open.contains("Darwin.open(path, accessMode | O_NOFOLLOW | O_CLOEXEC)"));
    assert!(fifo_open.contains("signedDashboardSMBCredentialFstat(fileDescriptor)"));
    assert!(
        fifo_open.contains("signedDashboardSMBCredentialFIFOIdentityMatches(beforeOpen, opened)")
    );
    assert!(fifo_open.contains("let beforeUnlink = try signedDashboardSMBCredentialLstat(path)"));
    assert!(fifo_open.contains("Darwin.unlink(path)"));

    let fifo_provisioning = swift_function_body(
        &app_source,
        "fileprivate func runSignedDashboardSMBCredentialFIFOProvisioningAndExit(",
    );
    assert!(fifo_provisioning.contains("resolveEmbeddedDashboardConfigPath()?.path"));
    assert!(!fifo_provisioning.contains("resolveDashboardConfigPath()"));
    assert!(!fifo_provisioning.contains("argumentValue("));
    assert!(
        fifo_provisioning
            .contains("input.basename != output.basename, input.token == output.token")
    );
    assert_swift_code_order(
        fifo_provisioning,
    ...
    assert!(
        fifo_provisioning.contains(
            "writeSignedDashboardSMBCredentialResult(helperResult.result, to: outputFIFO)"
        )
    );
    assert!(
        fifo_provisioning.contains(
            "writeSignedDashboardSMBCredentialResult(.integrityMismatch, to: outputFIFO)"
        )
    );
    for forbidden in [
        "AppLogger.log",
        ...
        assert!(
            !fifo_provisioning.contains(forbidden),
            "FIFO provisioning must not use {forbidden}"
        );
    }

    let fifo_helper = swift_function_body(
        &app_source,
        "private func runSignedDashboardSMBCredentialFIFOHelper(",
    );
    assert!(fifo_helper.contains("installedServiceHelperExecutable(serviceBundle)"));
    assert!(!fifo_helper.contains("Contents/MacOS/ICloudPDOptimizerApp"));
    assert!(fifo_helper.contains("serviceSMBCredentialSetupArguments("));
    assert!(fifo_helper.contains("mode: .dashboard"));
    assert!(fifo_helper.contains("logEvents: false"));
    assert!(fifo_helper.contains("inheritParentEnvironment: false"));
    assert!(fifo_helper.contains("fileDescriptor: inputFIFO.fileDescriptor"));
    assert!(fifo_helper.contains("inputFIFO.close()"));
    assert!(fifo_helper.contains("process.standardOutput = helperOutput"));
    assert!(fifo_helper.contains("process.standardError = FileHandle.nullDevice"));
    assert!(fifo_helper.contains("BoundedRedactedProcessOutputReader"));
    assert!(fifo_helper.contains("SMBCredentialActionResult.parse("));
    assert!(!fifo_helper.contains("FileHandle.standardOutput"));
    assert!(!fifo_helper.contains("AppLogger.log"));

    let output_write = swift_function_body(
        &app_source,
        "private func writeSignedDashboardSMBCredentialResult(",
    );
    assert!(output_write.contains("F_SETNOSIGPIPE"));
    assert!(output_write.contains("signedDashboardSMBCredentialCanonicalResultData"));
    let canonical_result = swift_function_body(
        &app_source,
        "private func signedDashboardSMBCredentialCanonicalResultData(",
    );
    assert!(canonical_result.contains("JSONSerialization.data"));
    assert!(canonical_result.contains(".sortedKeys"));
    assert!(canonical_result.contains("{\\\"status\\\":\\\"integrity_mismatch\\\"}"));
    assert!(!canonical_result.contains("stdout"));

    let proxy_args = swift_function_body(
        &app_source,
        "private func shouldProxyLaunchArguments(_ args: [String]) -> Bool",
    );
    assert!(proxy_args.contains("!isSignedDashboardSMBCredentialFIFOProvisioningAttempt(args)"));
    assert_swift_code_order(
        &app_source,
...
fn workflow_upload_verified_inherits_original_library_destination() {
    ...
    assert_eq!(
        record.proofs["upload"]["owner_record_name"],
        "_shared-owner"
    );
}
