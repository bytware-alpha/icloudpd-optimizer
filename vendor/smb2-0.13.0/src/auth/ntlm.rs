...
impl NtlmAuthenticator {
    ...
    pub fn authenticate(&mut self, challenge_bytes: &[u8]) -> Result<Vec<u8>, Error> {
        ...
        let nt_proof_str = Zeroizing::new({
            let mut mac =
        ...
        let session_base_key = Zeroizing::new({
            let mut mac =
                HmacMd5::new_from_slice(&ntlmv2_hash).expect("HMAC accepts any key length");
        ...
        let (exported_session_key, encrypted_random_session_key) = if key_exch {
            ...
            let encrypted = rc4_encrypt(&session_base_key, random_key.as_ref());
            (Zeroizing::new(random_key.to_vec()), encrypted)
        } else {
    ...
    pub fn session_key(&self) -> Option<&[u8]> {
        self.session_key.as_ref().map(|key| key.as_slice())
    }
    ...
    fn get_random_session_key(&self) -> Zeroizing<[u8; 16]> {
        #[cfg(test)]
        if let Some(rsk) = self.test_random_session_key {
            return Zeroizing::new(rsk);
        }
...
fn compute_nt_hash(password: &str) -> Zeroizing<Vec<u8>> {
    let unicode_password = Zeroizing::new(
        password
            .encode_utf16()
...
fn compute_ntlmv2_hash(nt_hash: &[u8], username: &str, domain: &str) -> Zeroizing<Vec<u8>> {
...
fn rc4_encrypt(key: &[u8], data: &[u8]) -> Vec<u8> {
    let mut s = Zeroizing::new((0..=255).collect::<Vec<u8>>());
    let mut j: u8 = 0;
    for i in 0..256 {
...
mod tests {
    ...
    #[test]
    fn ntlm_secret_owners_zeroize_on_drop() {
        fn assert_zeroize_on_drop<T: zeroize::ZeroizeOnDrop>() {}
        assert_zeroize_on_drop::<NtlmCredentials>();
        assert_zeroize_on_drop::<NtlmAuthenticator>();
        assert_zeroize_on_drop::<Zeroizing<Vec<u8>>>();
        assert_zeroize_on_drop::<Zeroizing<[u8; 16]>>();
    }
    ...
    fn ntlmv2_hash_computation() {
        ...
        assert_eq!(ntlmv2_hash.as_slice(), expected);
    }
