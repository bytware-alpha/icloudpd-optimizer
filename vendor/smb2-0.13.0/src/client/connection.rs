...
use crate::client::diagnostics::SessionDiagnostics;
...
struct CryptoState {
    ...
    established_session: Option<SessionDiagnostics>,
}
...
impl Connection {
    ...
    pub fn set_session_id(&mut self, id: SessionId) {
        let mut crypto = self.inner.crypto.lock().unwrap();
        crypto.session_id = id;
        crypto.established_session = None;
    }
    ...
    /// Bind diagnostics to a successfully established session.
    ///
    /// This is deliberately separate from `set_session_id`: the server sends
    /// a provisional ID before authentication and key activation complete.
    /// Publishing the snapshot only at the successful setup boundary prevents
    /// diagnostics from representing a partial session as authenticated.
    pub(crate) fn register_established_session(
        &mut self,
        session_id: SessionId,
        should_sign: bool,
        should_encrypt: bool,
        ...
        let mut crypto = self.inner.crypto.lock().unwrap();
        let signing_state_valid = crypto.should_sign == should_sign
            && (!should_sign
                || (crypto.signing_key.is_some()
                    && crypto.signing_algorithm == Some(signing_algorithm)));
        let encryption_state_valid = crypto.should_encrypt == should_encrypt
            && (!should_encrypt
                || (crypto.encryption_key.is_some()
                    && crypto.decryption_key.is_some()
                    && crypto.encryption_cipher.is_some()));
        if session_id == SessionId::NONE
            || crypto.session_id != session_id
            || !signing_state_valid
            || !encryption_state_valid
        {
            return Err(Error::invalid_data(
                "established session does not match active connection state",
            ));
        }
        crypto.established_session = Some(SessionDiagnostics {
            session_id,
            should_sign,
            should_encrypt,
            signing_algorithm,
    ...
    pub fn activate_signing(
        &mut self,
        key: Zeroizing<Vec<u8>>,
        algorithm: SigningAlgorithm,
    ) -> Result<()> {
        if key.len() != 16 {
            return Err(Error::invalid_data("invalid SMB signing key length"));
        }
        debug!(
    ...
    pub fn activate_encryption(
        ...
        if enc_key.len() != dec_key.len() || enc_key.len() != smb_encryption_key_length(cipher) {
            return Err(Error::invalid_data("invalid SMB encryption key length"));
        }
        debug!(
    ...
    pub fn diagnostics(&self) -> crate::client::diagnostics::ConnectionDiagnostics {
        ...
        let (signing, encryption, session) = {
            let c = self.inner.crypto.lock().unwrap();
            (
                SigningInfo {
...
fn smb_encryption_key_length(cipher: Cipher) -> usize {
    match cipher {
...
mod tests {
    ...
    async fn signing_applied_to_outgoing_messages() {
        ...
        // Activate signing.
        let key = Zeroizing::new(vec![0xAA; 16]);
    ...
    async fn phase3_decrypt_failure_errors_waiter_not_hangs() {
        ...
        let enc_key = Zeroizing::new(vec![0x42; 16]);
        let dec_key = Zeroizing::new(vec![0x99; 16]); // deliberately wrong decryption key
    ...
    async fn send_cancel_signs_message_when_signing_active() {
        ...
        let key = Zeroizing::new(vec![0xCC; 16]);
        conn.activate_signing(key, SigningAlgorithm::HmacSha256)
            .unwrap();
    ...
    async fn activate_encryption_sets_state() {
        ...
        conn.activate_encryption(
            Zeroizing::new(vec![0x42; 16]),
            Zeroizing::new(vec![0x42; 16]),
            Cipher::Aes128Gcm,
        )
        .unwrap();
    ...
    #[tokio::test]
    async fn crypto_state_accepts_only_bounded_zeroizing_key_copies() {
        ...
        assert!(conn
            .activate_signing(Zeroizing::new(vec![0x41; 15]), SigningAlgorithm::HmacSha256,)
            .is_err());
        assert!(conn
            .activate_signing(Zeroizing::new(vec![0x41; 32]), SigningAlgorithm::AesGmac,)
            .is_err());
        assert!(!conn.diagnostics().signing.active);
        assert!(conn
            .activate_encryption(
                Zeroizing::new(vec![0x42; 16]),
        ...
        assert!(conn
            .activate_encryption(
                Zeroizing::new(vec![0x42; 15]),
                Zeroizing::new(vec![0x43; 15]),
                Cipher::Aes128Gcm,
            )
            .is_err());
        assert!(!conn.should_encrypt());
    ...
    #[tokio::test]
    async fn established_session_diagnostics_require_exact_active_crypto_state() {
        let mock = Arc::new(MockTransport::new());
        let mut conn =
            Connection::from_transport(Box::new(mock.clone()), Box::new(mock), "test-server");
        let session_id = SessionId(0x5151);
        conn.set_session_id(session_id);

        assert!(conn
            .register_established_session(session_id, true, false, SigningAlgorithm::HmacSha256,)
            .is_err());
        assert!(conn.diagnostics().session.is_none());

        conn.activate_signing(Zeroizing::new(vec![0xA5; 16]), SigningAlgorithm::HmacSha256)
            .unwrap();
        assert!(conn
            .register_established_session(
                SessionId(0x6161),
                true,
                false,
        ...
        conn.register_established_session(session_id, true, false, SigningAlgorithm::HmacSha256)
            .unwrap();
        let diagnostic = conn.diagnostics().session.unwrap();
        assert_eq!(diagnostic.session_id, session_id);
        assert!(diagnostic.should_sign);
        assert!(!diagnostic.should_encrypt);
        assert_eq!(diagnostic.signing_algorithm, SigningAlgorithm::HmacSha256);

        conn.set_session_id(session_id);
        assert!(
            conn.diagnostics().session.is_none(),
            "a new setup round must invalidate the prior established snapshot"
        );
    }
