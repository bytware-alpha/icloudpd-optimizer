...
pub fn sp800_108_kdf(
    ...
    let mut result = Zeroizing::new(Vec::with_capacity((iterations * 32) as usize));
...
pub struct DerivedKeys {
    ...
    pub decryption_key: Zeroizing<Vec<u8>>,
}
...
pub fn derive_session_keys(
    ...
    assert!(
        matches!(key_length_bits, 128 | 256),
        "SMB derived key length must be 128 or 256 bits"
    );
    assert!(
        matches!(
            dialect,
    ...
    DerivedKeys {
        // SMB signing algorithms in this crate are all 128-bit even when the
        // negotiated encryption cipher uses a 256-bit key.
        signing_key: sp800_108_kdf(session_key, signing_label, signing_context, 128),
...
mod tests {
    ...
    #[test]
    fn derived_key_buffers_zeroize_on_drop() {
        fn assert_zeroize_on_drop<T: zeroize::ZeroizeOnDrop>() {}
        assert_zeroize_on_drop::<Zeroizing<Vec<u8>>>();
        let keys = derive_session_keys(&[0x42; 16], Dialect::Smb3_0, None, 128);
    ...
    #[test]
    #[should_panic(expected = "SMB derived key length must be 128 or 256 bits")]
    fn derived_key_length_is_bounded() {
        let _ = derive_session_keys(&[0x42; 16], Dialect::Smb3_0, None, 512);
    }
